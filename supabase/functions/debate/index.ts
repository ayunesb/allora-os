...
serve(async (req) => {
  ...
  try {
    ...
    const { action, topic, participants, messages, userMessage, userId, preferences, context } = await req.json();
    ...
    switch(action) {
      ...
      case 'generate_executive_responses': {
        ...
        const formattedMessages = messageData.map(msg => ({
          ...
          content: msg.content,
          name: msg.sender !== 'system' && msg.sender !== 'user' ? msg.sender.replace(/\s+/g, '_').toLowerCase() : undefined
        }));
        ...
        for (const exec of executives) {
          ...
          const lastSequence = messageData.length > 0 ? messageData[messageData.length - 1].sequence : -1;
      ...
      case 'generate_debate_summary': {
        ...
        const debateText = messageData.map(msg => 
          `${msg.sender}${msg.sender_role ? ` (${msg.sender_role})` : ''}: ${msg.content}`
        ).join('\n\n');
