...
serve(async (req) => {
  ...
  try {
    ...
    const { action, leadId, meetingData, communicationData } = await req.json();
    ...
    if (action === "create-zoom-meeting") {
      ...
      if (!tokenResponse.ok) {
        return new Response(JSON.stringify({ error: "Failed to get Zoom token", details: tokenData }), {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" }
        });
      }
      ...
      if (commError) {
        return new Response(JSON.stringify({ error: "Failed to save communication", details: commError }), {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" }
        });
      }
      ...
      const { communicationId, transcriptText } = await req.json();
      ...
      if (!openaiResponse.ok) {
        return new Response(JSON.stringify({ error: "Failed to generate summary", details: openaiData }), {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" }
        });
      }
