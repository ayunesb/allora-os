...
serve((req) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        // Get all tenants
        const { data: tenants, error: tenantsError } = yield supabase
            .from("tenants")
            .select("id, email_recipients");
        if (tenantsError) {
            throw tenantsError;
        }
        console.log(`Processing ${(tenants === null || tenants === void 0 ? void 0 : tenants.length) || 0} tenants`);
        for (const tenant of tenants || []) {
            ...
            const { data: kpis, error: kpisError } = yield supabase
                .from("kpi_metrics")
                .select("*")
      ...
      `;
            ...
            for (const email of tenant.email_recipients) {
                ...
                try {
                    ...
                    // Log the email
                    yield supabase.from("email_logs").insert({
                        tenant_id: tenant.id,
                        status: "sent",
                        recipient: email,
                        subject: "Your Weekly KPI Summary",
        ...
        return new Response(JSON.stringify({ success: true, message: "Weekly digest emails sent" }), {
            status: 200,
