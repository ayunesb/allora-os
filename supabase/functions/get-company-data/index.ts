  ...
  try {
    ...
    if (zyteData.webPage?.autoKeywords) {
      ...
      for (const keyword of keywords) {
        for (const [industryKey, industryValue] of Object.entries(
          industryKeywords,
        )) {
    ...
    if (zyteData.webPage?.product) {
      result.products = zyteData.webPage.product.map(
        (p: any) => p.name || "Unnamed Product",
      );
    }
    ...
    if (zyteData.webPage?.links) {
      ...
      const serviceLinks = zyteData.webPage.links.filter((link: any) =>
