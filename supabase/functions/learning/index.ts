  ...
  try {
    ...
    switch(action) {
      case 'track_feedback': {
        ...
        await updateLearningModel(supabase, botName, botRole, isPositive, metadata?.topic);
...
async function updateLearningModel(supabase, botName, botRole, isPositive, topic) {
  ...
  if (!model) {
    ...
    const newModel = {
      ...
      topics: topic ? { [topic]: { positive: isPositive ? 1 : 0, negative: isPositive ? 0 : 1 } } : {},
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString()
    };
    ...
    const updatedModel = {
      positive_feedback_count: isPositive ? model.positive_feedback_count + 1 : model.positive_feedback_count,
      negative_feedback_count: isPositive ? model.negative_feedback_count : model.negative_feedback_count + 1,
      updated_at: new Date().toISOString()
    };
...
async function getUserPreferences(supabase, userId) {
  ...
  const { data: actions, error } = await supabase.rpc('get_recent_user_actions', {
    p_user_id: userId,
    p_days: 90
  });
