...
serve((req) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const { action, userId, data } = yield req.json();
        switch (action) {
            case "track_feedback": {
                // Track user feedback on an AI response
                const { interactionId, messageId, botName, botRole, isPositive, comment, metadata, } = data;
                // Store the feedback
                ...
                yield updateLearningModel(supabase, botName, botRole, isPositive, metadata === null || metadata === void 0 ? void 0 : metadata.topic);
            ...
            case "get_learning_model": {
                ...
                const { data: model, error } = yield supabase
            ...
            case "get_user_preferences": {
                ...
                const { data: preferences, error } = yield supabase
                ...
                if (!preferences) {
                    ...
                    const userPreferences = yield getUserPreferences(supabase, userId);
                    return new Response(JSON.stringify({ preferences: userPreferences }), {
                        headers: Object.assign(Object.assign({}, corsHeaders), { "Content-Type": "application/json" }),
                    });
                }
            ...
            case "update_user_preferences": {
                ...
                const { data: existing, error: checkError } = yield supabase
                    .from("user_preferences")
                    .select("id")
                    .eq("user_id", userId)
                    .single();
                ...
                if (existing) {
                    ...
                    const result = yield supabase
                        .from("user_preferences")
                        .update(Object.assign(Object.assign({}, preferences), { last_updated: new Date().toISOString() }))
                        .eq("user_id", userId);
                    error = result.error;
                }
                else {
                    // Insert new preferences
                    const result = yield supabase.from("user_preferences").insert(Object.assign(Object.assign({ user_id: userId }, preferences), { last_updated: new Date().toISOString() }));
                    error = result.error;
                }
                if (error)
                    throw error;
...
function updateLearningModel(supabase, botName, botRole, isPositive, topic) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!model) {
            ...
            yield supabase.from("learning_models").insert(newModel);
        }
        else {
...
function getUserPreferences(supabase, userId) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const { data: actions, error } = yield supabase.rpc("get_recent_user_actions", {
            p_user_id: userId,
            p_days: 90,
        });
        if (error)
        ...
        actions.forEach((action) => {
            ...
            if ((_a = action.metadata) === null || _a === void 0 ? void 0 : _a.executive_name) {
            ...
            // Track risk appetite
            if ((_c = action.metadata) === null || _c === void 0 ? void 0 : _c.risk_level) {
        ...
        const maxRisk = Object.entries(riskLevels).reduce((max, [level, count]) => (count > max.count ? { level, count } : max), { level: "medium", count: 0 });
