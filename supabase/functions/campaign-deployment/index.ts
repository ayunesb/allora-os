...
serve(async (req) => {
  ...
  try {
    ...
    if (connectionError || !connection) {
      return new Response(
        JSON.stringify({ error: `No active ${campaign.ad_platform} connection found`, details: connectionError }),
        { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    if (new Date(connection.token_expires_at) < new Date()) {
      return new Response(
        JSON.stringify({ error: "Ad platform token expired. Please reconnect your account." }),
        { status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
