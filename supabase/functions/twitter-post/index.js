...
function generateOAuthSignature(method, url, params, consumerSecret, tokenSecret) {
    const signatureBaseString = `${method}&${encodeURIComponent(url)}&${encodeURIComponent(Object.entries(params)
        .sort()
        .map(([k, v]) => `${k}=${v}`)
        .join("&"))}`;
...
function generateOAuthHeader(method, url, accessToken, accessTokenSecret) {
    ...
    const signedOAuthParams = Object.assign(Object.assign({}, oauthParams), { oauth_signature: signature });
    return ("OAuth " +
...
Deno.serve((req) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const { tenant_id, message, queue = true } = yield req.json();
        ...
        if (queue) {
            const queueResult = yield queueTweet(tenant_id, message);
        ...
        const { data: tenant, error: tenantError } = yield supabase
            ...
            .single();
        if (tenantError ||
            !(tenant === null || tenant === void 0 ? void 0 : tenant.twitter_access_token) ||
            !(tenant === null || tenant === void 0 ? void 0 : tenant.twitter_access_token_secret)) {
            return new Response(JSON.stringify({ error: "Twitter not connected for this tenant" }), {
                status: 400,
                headers: Object.assign(Object.assign({}, corsHeaders), { "Content-Type": "application/json" }),
            });
        }
        // Post the tweet
        const result = yield postTweet(message, tenant.twitter_access_token, tenant.twitter_access_token_secret);
        // Update the last_tweet_at timestamp
        yield supabase
            .from("tenants")
            .update({ last_tweet_at: new Date().toISOString() })
            .eq("id", tenant_id);
        return new Response(JSON.stringify({ success: true, tweet_id: result.data.id }), { headers: Object.assign(Object.assign({}, corsHeaders), { "Content-Type": "application/json" }) });
    }
    catch (error) {
