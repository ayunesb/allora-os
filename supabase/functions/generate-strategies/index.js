...
serve((req) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        // Get request body
        const { industry, companySize, revenue, goals, riskTolerance, timeHorizon, challenges, userId, companyId, companyName, } = yield req.json();
      ...
      "strategies": [
        ...
        if (userId && companyId) {
            try {
                ...
                const { createClient } = yield import("https://esm.sh/@supabase/supabase-js@2.39.6");
                const supabaseUrl = Deno.env.get("SUPABASE_URL");
                const supabaseServiceKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY");
                const supabase = createClient(supabaseUrl, supabaseServiceKey);
                // Save the strategy generation to the database
                yield supabase.from("ai_strategy_logs").insert({
                    user_id: userId,
