...
serve((req) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        // Get request body
        const { userId, companyId, industry, riskAppetite, companyName, companyDetails, } = yield req.json();
        if (!userId || !companyId) {
        ...
        const strategies = yield generateStrategies(industry, riskAppetite, companyName, companyDetails);
        logStep("Strategies generated", { count: strategies.length });
        // Save strategies to database
        for (const strategy of strategies) {
            const { error: strategyError } = yield supabaseAdmin
                .from("strategies")
                .insert({
        ...
        const campaigns = yield generateCampaigns(industry, companyName, companyDetails);
        logStep("Campaigns generated", { count: campaigns.length });
        // Save campaigns to database
        for (const campaign of campaigns) {
            const { error: campaignError } = yield supabaseAdmin
                .from("campaigns")
                .insert({
                name: campaign.name,
        ...
        const debate = yield generateAIDebate(industry, riskAppetite, companyName, companyDetails);
        logStep("AI debate generated");
        // Save AI debate to database
        const { error: debateError } = yield supabaseAdmin
            .from("ai_debates")
            .insert({
            title: debate.title,
        ...
        const scripts = yield generateCallScripts(industry, companyName, companyDetails);
        logStep("Call scripts generated", { count: scripts.length });
        // Save call scripts to database
        for (const script of scripts) {
            const { error: scriptError } = yield supabaseAdmin
                .from("call_scripts")
                .insert({
                title: script.title,
        ...
        yield supabaseAdmin
            ...
            .eq("id", userId);
        // Record API usage
        yield recordApiUsage(supabaseAdmin, companyId, {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            newUsage.strategy_generations =
                (newUsage.strategy_generations || 0) + (usage.strategyGenerations || 0);
            newUsage.campaign_generations =
                (newUsage.campaign_generations || 0) + (usage.campaignGenerations || 0);
...
function generateStrategies(industry, riskAppetite, companyName, companyDetails) {
    return __awaiter(this, void 0, void 0, function* () {
