    ...
    try {
        ...
        if (action === "authorize") {
            ...
            if (!code || !state.companyId) {
                return new Response(JSON.stringify({ error: "Invalid callback request" }), {
                    status: 400,
                    headers: Object.assign(Object.assign({}, corsHeaders), { "Content-Type": "application/json" }),
                });
            }
            ...
            const longLivedTokenResponse = yield fetch(longLivedTokenUrl);
            const longLivedTokenData = yield longLivedTokenResponse.json();
            ...
            const adAccountsResponse = yield fetch(adAccountsUrl);
            const adAccountsData = yield adAccountsResponse.json();
            ...
            yield fetch(revokeUrl, { method: "DELETE" });
