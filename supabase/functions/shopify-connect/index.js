...
serve((req) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        // Parse request body
        const { userId, shopDomain, accessToken } = yield req.json();
        if (!userId || !shopDomain) {
            return new Response(JSON.stringify({ error: "userId and shopDomain are required" }), {
                status: 400,
        ...
        if (!companyId) {
            return new Response(JSON.stringify({ error: "User does not have an associated company" }), {
                status: 400,
                headers: Object.assign(Object.assign({}, corsHeaders), { "Content-Type": "application/json" }),
            });
        }
        // Store Shopify integration details
        const { data: integrationData, error: integrationError } = yield supabase
        ...
        if (integrationError) {
            ...
            return new Response(JSON.stringify({ error: "Failed to store Shopify integration" }), {
                status: 500,
                headers: Object.assign(Object.assign({}, corsHeaders), { "Content-Type": "application/json" }),
            });
        }
        ...
        if (accessToken) {
            try {
                const shopResponse = yield fetch(`https://${shopDomain}/admin/api/2023-07/shop.json`, {
                    headers: {
                        "X-Shopify-Access-Token": accessToken,
                    },
                });
                if (shopResponse.ok) {
                    const shopData = yield shopResponse.json();
