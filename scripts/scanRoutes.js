...
function scanRoutes() {
    return __awaiter(this, void 0, void 0, function* () {
        const files = yield glob(`${routesDir}/**/*.{ts,tsx}`);
        ...
        for (const file of files) {
            ...
            matches.forEach(([, routePath]) => {
                ...
                if (existing.length > 1)
                    duplicateRoutes.add(routePath);
            });
        }
        const isWildcardOrDynamic = (r) => r === "*" || r.includes(":") || r.startsWith("/:");
        ...
        // Output
        if (outputJson) {
            fs.writeFileSync("route-audit.json", JSON.stringify({ unusedRoutes, duplicateRoutes: duplicateRouteList }, null, 2));
            ...
            if (unusedRoutes.length > 0) {
                ...
                unusedRoutes.forEach(({ route, files }) => console.log(`   → ${route} (in: ${files.join(", ")})`));
            }
            else {
            ...
            if (duplicateRouteList.length > 0) {
                console.log(`\n⚠️ Duplicate Routes:`);
                duplicateRouteList.forEach(({ route, files }) => console.log(`   → ${route} (in: ${files.join(", ")})`));
            }
            else {
