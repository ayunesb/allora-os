 ...
 */
export async function createCampaign(campaignData: CreateCampaignInput): Promise<{ success: boolean; campaignId?: string; error?: string }> {
  try {
 ...
 */
export async function createCampaignCheckout(campaignId: string, cancelUrl?: string): Promise<{ success: boolean; url?: string; error?: string }> {
  try {
 ...
 */
export async function checkCampaignPaymentStatus(campaignId: string): Promise<{ success: boolean; status?: string; error?: string }> {
  try {
    const { data, error } = await supabase.functions.invoke('campaign-payment', {
      body: { 
 ...
 */
export async function deployCampaign(campaignId: string): Promise<{ success: boolean; error?: string }> {
  try {
 ...
 */
export async function syncCampaignData(campaignId: string): Promise<{ success: boolean; metrics?: any; error?: string }> {
  try {
    const { data, error } = await supabase.functions.invoke('campaign-deployment', {
      body: { 
        action: 'sync', 
