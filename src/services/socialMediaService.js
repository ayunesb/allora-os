 ...
 */
export async function fetchSocialMediaPosts(companyId, filters) {
    return apiRequest(wrapSupabaseQuery(async () => {
        let query = supabase
 ...
 */
export async function fetchSocialMediaPost(postId) {
 ...
 */
export async function createSocialMediaPost(companyId, postData) {
    try {
        ...
        const result = await apiRequest(wrapSupabaseQuery(async () => {
 ...
 */
export async function updateSocialMediaPost(postData) {
    try {
        // Validate the input data
        ...
        await apiRequest(wrapSupabaseQuery(async () => {
            const { data, error } = await supabase
                .from('social_media_posts')
                .update({
                title: validatedData.title,
 ...
 */
export async function deleteSocialMediaPost(postId) {
    try {
        // Get the current post data to determine company ID for cache invalidation
 ...
 * @returns Promise with success status or error message
 */
export async function approvePost(postId, notes) {
    try {
        // Get the current post data to determine company ID for cache invalidation
        const currentPost = await fetchSocialMediaPost(postId);
        if (!currentPost) {
        ...
        await apiRequest(wrapSupabaseQuery(async () => {
            const { data, error } = await supabase
                ...
                status: currentPost.status === 'Draft' ? 'Approved' : currentPost.status,
                updated_at: new Date().toISOString()
            })
                .eq('id', postId);
            return { data, error };
 ...
 */
export async function batchSchedulePosts(postIds) {
    if (!postIds.length) {
        return {
            success: true,
            processed: 0,
