...
const ExecutiveVotingResult: React.FC<ExecutiveVotingResultProps> = ({
  votes,
  executives
}) => {
  ...
  return (
    <motion.div 
      ...
      className="rounded-lg bg-gray-900 border border-gray-800 overflow-hidden mx-auto max-w-2xl"
    >
                ...
                {optionA.map((vote, index) => {
                  ...
                  return (
                    <div 
                      ...
                      className="relative" 
                      style={{ left: `${(index / Math.max(optionA.length - 1, 1)) * 90}%` }}
                    >
                ...
                {optionB.map((vote, index) => {
                  ...
                  return (
                    <div 
                      key={vote.executiveId} 
                      className="relative" 
                      style={{ left: `${(index / Math.max(optionB.length - 1, 1)) * 90}%` }}
                    >
                      <Avatar className="h-6 w-6 border border-gray-700">
                        <AvatarImage src={exec.avatar} alt={exec.name} />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {votes.map(vote => {
              ...
              return (
                <div 
                  key={vote.executiveId}
                  className="flex items-center gap-2 p-2 rounded-md bg-gray-800/50"
                >
