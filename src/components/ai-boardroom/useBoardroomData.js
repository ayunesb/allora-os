...
const sampleDebate = {
    ...
    discussion: [
        {
            speaker: "Elon Musk",
            message: "I strongly believe we should aggressively expand into Southeast Asia first. The digital transformation happening there presents a unique opportunity for our SaaS solution. We could capture significant market share before competitors realize the potential.",
        },
        {
            speaker: "Warren Buffett",
            message: "I'm concerned about the financial implications. These markets typically have lower price points, which could impact our margins. We should carefully analyze the unit economics before committing significant resources.",
        },
        {
            speaker: "Satya Nadella",
            message: "From an operational perspective, we'd need to consider localization requirements - not just language, but also regulatory compliance and payment methods. I suggest starting with a limited offering in 1-2 countries first.",
        },
        {
            speaker: "Sheryl Sandberg",
            message: "Marketing in these regions will require a different approach. We should leverage local partnerships and platforms rather than our usual channels. I'd recommend allocating budget for market research before full commitment.",
        },
    ],
...
export function useBoardroomData(companyId) {
    ...
    useEffect(() => {
        ...
        function fetchBoardroomDebate() {
            return __awaiter(this, void 0, void 0, function* () {
                if (!companyId && !(profile === null || profile === void 0 ? void 0 : profile.company_id)) {
                    setError("No company ID available. Please set up your company profile first.");
                    setIsLoading(false);
                    return;
                }
                const targetCompanyId = companyId || (profile === null || profile === void 0 ? void 0 : profile.company_id);
                setIsLoading(true);
                setError(null);
    ...
    }, [companyId, profile === null || profile === void 0 ? void 0 : profile.company_id]);
    return {
        topic,
        summary,
        executives,
