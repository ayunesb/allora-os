    ...
    return (<div className="bg-card border rounded-lg p-6">
      ...
      <Tabs defaultValue="ceo" onValueChange={setActiveRole}>
        <TabsList className="mb-4">
          {Object.keys(executiveBots).map(role => (<TabsTrigger key={role} value={role} className="flex items-center gap-2">
              {getRoleIcon(role)}
              <span className="capitalize">{role}</span>
            </TabsTrigger>))}
        </TabsList>
        
        {Object.entries(executiveBots).map(([role, names]) => (<TabsContent key={role} value={role}>
            <div className="bg-muted/30 rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {names.map(name => (<div key={name} className="bg-background border border-border/50 rounded-md p-3 flex items-center gap-2">
