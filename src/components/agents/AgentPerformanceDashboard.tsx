...
export const AgentPerformanceDashboard = ({
  ...
  const [pluginXpData, setPluginXpData] = useState<PluginXPPoint[]>([]);
  const [usageStreaks, setUsageStreaks] = useState<{ plugin_name: string; days_used: number }[]>([]);
  const [changelog, setChangelog] = useState<string>(""); // Added changelog state
  const [groupedLogs, setGroupedLogs] = useState<Record<string, EvolutionLog[]>>({});
  const [agentXpLogs, setAgentXpLogs] = useState<Record<string, number>>({});
  ...
  useEffect(() => {
    ...
    const fetchUsageStreaks = async () => {
      ...
      const calculateStreaks = (logs: { plugin_name: string; created_at: string }[]) => {
        const streaks: Record<string, Set<string>> = {};
  ...
  const handleRollback = async (pluginName: string, rollbackVersion: string) => {
    try {
      ...
      <div className="mt-4">
        ...
        <ul className="space-y-3">
          {pluginXpData.map((plugin) => (
            <li key={plugin.plugin_name} className="border rounded-lg p-3 bg-muted">
              <div className="font-bold">{plugin.plugin_name}</div>
              <div className="w-full h-2 bg-muted rounded-full overflow-hidden mt-1">
                <div
                  className="h-full bg-indigo-500 transition-all duration-500"
                  style={{ width: `${Math.min((plugin.total_xp / 100) * 100, 100)}%` }}
                />
              </div>
              ...
              {plugin.total_xp >= 100 && (
                <div>
                  ...
                  <button
                    onClick={() => handleRollback(plugin.plugin_name, "previous_version")}
        ...
        {Object.entries(groupedLogs).map(([trigger, logs]) => (
          <div key={trigger} className="mb-6">
            ...
            <ul className="space-y-2">
              {logs.map((log) => (
                <li key={log.id} className="border rounded p-3 text-sm bg-muted">
