    ...
    return (<div>
      ...
      <div className="mt-4">
        ...
        <ul className="space-y-3">
          {pluginXpData.map((plugin) => (<li key={plugin.plugin_name} className="border rounded-lg p-3 bg-muted">
              ...
              {plugin.total_xp >= 100 && (<Alert variant="success" className="mt-4">
      ...
      <div className="mt-4 flex gap-3">
        <button onClick={async () => {
            ...
            await supabase
                .from("agent_versions")
                .update({ prompt: prev.prompt, changelog: `Rolled back from ${current.version} to ${prev.version}` })
                .eq("id", current.id);
        ...
        <a href={`/PromptTuner.tsx?id=${current?.id}`} className="px-4 py-2 text-sm rounded bg-blue-600 text-white hover:bg-blue-700">
