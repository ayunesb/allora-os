            ...
            Object.values(totals).forEach(async (total_xp, index) => {
                if (total_xp >= 100) {
                    ...
                    const { data: last } = await supabase
                        ...
                        .limit(1)
                        .single();
                    const newVersion = "v" + ((parseInt(last?.version?.replace("v", "") || "1") + 1) || 2);
                    // Insert new version
    ...
    return (<div>
      ...
      <div className="mt-4">
        ...
        <ul className="grid grid-cols-2 gap-3">
          {usageStreaks.map((item) => (<li key={item.plugin_name} className="bg-muted rounded p-3">
      ...
      <div className="mt-4">
        ...
        <ul className="space-y-3">
          {pluginXpData.map((plugin) => (<li key={plugin.plugin_name} className="border rounded-lg p-3 bg-muted">
              <div className="font-bold">{plugin.plugin_name}</div>
              <div className="w-full h-2 bg-muted rounded-full overflow-hidden mt-1">
                <div className="h-full bg-indigo-500 transition-all duration-500" style={{ width: `${Math.min((plugin.total_xp / 100) * 100, 100)}%` }}/>
              </div>
              <p className="text-xs text-muted-foreground mt-1">
                {plugin.total_xp} / 100 XP to evolve
              </p>
              {plugin.total_xp >= 100 && (<div>
                  <Alert variant="success" className="mt-4">
                    🧬 Plugin Assistant ready to evolve! Next version unlocked.
                  </Alert>
                  <button onClick={() => handleRollback(plugin.plugin_name, "previous_version")} className="mt-2 bg-red-500 text-white px-3 py-1 rounded">
                    Rollback
                  </button>
                </div>)}
            </li>))}
      ...
      <div className="mt-4">
        <label className="block text-sm font-medium mb-1">Changelog Notes</label>
        <Textarea value={changelog} onChange={(e) => setChangelog(e.target.value)} rows={3} placeholder="Describe what changed in this version..."/>
      </div>
      ...
      <div className="mt-6">
        ...
        {Object.entries(groupedLogs).map(([trigger, logs]) => (<div key={trigger} className="mb-6">
            <h3 className="text-md font-bold capitalize mb-2">{trigger.replace(/_/g, " ")}</h3>
            <ul className="space-y-2">
              {logs.map((log) => (<li key={log.id} className="border rounded p-3 text-sm bg-muted">
      ...
      <ul className="grid grid-cols-2 gap-4 mt-6">
        {Object.entries(agentXpLogs).map(([type, xp]) => (<li key={type} className="bg-muted p-4 rounded">
            ...
            <div className="w-full h-2 bg-border rounded mt-2 overflow-hidden">
              <div className="h-full bg-indigo-500" style={{ width: `${Math.min(xp / 100, 1) * 100}%` }}/>
            </div>
          </li>))}
      </ul>
    </div>);
