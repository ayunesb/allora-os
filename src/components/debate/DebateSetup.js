...
debateTitle = '', debateObjective = '', debateDuration = '', isLoading = false, onTopicChange, onTitleChange, onObjectiveChange, onDurationChange, onStartDebate, onParticipantsChange, }) => {
    ...
    return (<Card>
      ...
      <CardContent className="space-y-4">
        <TopicSelector selectedTopic={selectedTopic} debateTopics={safeDebateTopics} onTopicChange={onTopicChange}/>
        
        <DebateParameters debateTitle={debateTitle} debateObjective={debateObjective} debateDuration={debateDuration} onTitleChange={onTitleChange} onObjectiveChange={onObjectiveChange} onDurationChange={onDurationChange}/>
        
        <ParticipantsList participants={participants} onEditParticipants={handleEditParticipants}/>
        
        <ExecutiveSelectionDialog isOpen={isDialogOpen} onClose={() => setIsDialogOpen(false)} selectedExecutives={participants} onExecutivesChange={handleParticipantsChange}/>
      </CardContent>
      <CardFooter>
        <Button onClick={handleStartDebate} disabled={!selectedTopic || !debateTitle || !debateObjective || isLoading} className="ml-auto">
