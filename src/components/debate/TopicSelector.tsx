      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className="w-full p-0">
          <Command>
            ...
            {Array.isArray(debateTopics) && debateTopics.length > 0 ? (<CommandGroup>
                <CommandList>
                  {debateTopics.map((topic) => (<CommandItem key={topic.id} value={topic.id} onSelect={() => {
                    onTopicChange(topic.id);
                    setOpen(false);
                }}>
                      <Check className={cn("mr-2 h-4 w-4", selectedTopic === topic.id ? "opacity-100" : "opacity-0")}/>
                      {topic.topic}
                    </CommandItem>))}
                </CommandList>
              </CommandGroup>) : (<div className="py-6 text-center text-sm">
