...
export function PostsDisplay({ view, posts, currentMonth, isLoading, error, onEditPost, onDeletePost, onSchedulePost, onApprovePost, onCreatePost, onRefresh, }) {
    ...
    if (isLoading) {
        ...
        <Loading center text="Loading posts..." tooltip="We're retrieving your social media posts. This should only take a moment."/>
      </div>);
    }
    // Handle error state
    if (error) {
    ...
    if (posts.length === 0) {
        ...
        <button onClick={onCreatePost} className="text-primary hover:underline">
          Create your first post
        </button>
      </div>);
    }
    // Render the appropriate view
    return view === 'calendar' ? (<CalendarView posts={posts} currentMonth={currentMonth} onCreatePost={onCreatePost} onEditPost={onEditPost} onDeletePost={onDeletePost} onSchedulePost={onSchedulePost} onApprovePost={onApprovePost} onRefresh={onRefresh}/>) : (<ListView posts={posts} onEditPost={onEditPost} onDeletePost={onDeletePost} onSchedulePost={onSchedulePost} onApprovePost={onApprovePost}/>);
}
