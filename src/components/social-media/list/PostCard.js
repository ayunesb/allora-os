...
export function PostCard({ post, onEdit, onDelete, onSchedule, onApprove, }) {
    ...
    const renderActionButtons = () => {
        switch (post.status.toLowerCase()) {
            ...
            default:
                return (<Button variant="outline" size="sm" onClick={onEdit}>
            <Edit className="mr-1 h-4 w-4"/>
            View
          </Button>);
        }
    ...
    return (<Card className="h-full flex flex-col">
      ...
      <CardContent className="px-4 py-3 flex-grow">
        ...
        <div className="text-xs text-muted-foreground mt-2">
          {post.scheduled_date ? (<p>Scheduled: {formatDate(`${post.scheduled_date}T${post.publish_time}:00`)}</p>) : (<p>Not scheduled</p>)}
          
          {post.published_at && (<p className="mt-1">Published: {formatDate(post.published_at)}</p>)}
        </div>
        
        {post.tags && post.tags.length > 0 && (<div className="flex flex-wrap gap-1 mt-2">
