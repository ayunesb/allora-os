          ...
          <CardContent className="p-4">
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className="md:w-1/2">
                ...
                {selectedDatePosts.length > 0 ? (<div className="space-y-3">
                    {selectedDatePosts.map(post => (<Card key={post.id} className="p-3">
                  ...
                  </div>) : (<p className="text-muted-foreground">No posts scheduled for this date.</p>)}
              </div>
            </div>
          </CardContent>
        </Card>)}
        ...
        <Card>
          <CardContent className="p-4">
            <div className="space-y-4">
              {posts.length > 0 ? (posts.map(post => (<Card key={post.id} className="p-4">
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className="font-medium">{post.title || 'Untitled Post'}</h4>
                        <p className="text-sm text-muted-foreground">
                  ...
                  </Card>))) : (<p className="text-muted-foreground text-center py-8">No posts available.</p>)}
            </div>
          </CardContent>
        </Card>)}
    </div>);
