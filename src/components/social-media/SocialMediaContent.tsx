...
export function SocialMediaContent() {
    ...
    const renderPosts = () => {
        ...
        {posts.map(post => (<div key={post.id} className="border rounded-lg p-4 shadow-sm">
            <h3 className="font-bold">{post.title}</h3>
            <p className="text-sm text-muted-foreground mb-2">{post.platform}</p>
    ...
    const SocialMediaPostForm = ({ post, onSubmit, isSubmitting, onClose }) => {
        ...
        <h2 className="text-lg font-bold">{post ? 'Edit Post' : 'Create Post'}</h2>
      ...
      <CardContent className="p-0 md:p-6">
        <div className="flex justify-between items-center p-4 border-b">
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => refreshPosts && refreshPosts()} disabled={loading}>
              <RefreshCw className="h-4 w-4 mr-1"/>
              Refresh
            </Button>
            <div className="flex border rounded-md">
              <Button variant={view === 'calendar' ? 'default' : 'ghost'} size="sm" className="rounded-r-none">
                <CalendarDays className="h-4 w-4"/>
              </Button>
              <Button variant={view === 'list' ? 'default' : 'ghost'} size="sm" className="rounded-l-none">
