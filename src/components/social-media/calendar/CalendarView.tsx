...
export function CalendarView({ 
  ...
  const DayCell = ({ day, isPlaceholder = false }: { day: Date, isPlaceholder?: boolean }) => {
          ...
          <div className="space-y-1 mt-1">
            {dayPosts.map((post) => (
              <div 
                ...
                className="text-xs p-1 rounded cursor-pointer hover:bg-muted flex flex-col"
              >
                <div className="flex justify-between items-center">
                  <Badge 
                    variant="outline" 
                    className={cn("text-[10px] px-1 py-0", getPlatformColor(post.platform))}
                  >
                    {post.platform}
                  </Badge>
                  <Badge 
                    variant="outline" 
                    className={cn("text-[10px] px-1 py-0", getStatusColor(post.status))}
                  >
                    {post.status}
                  </Badge>
                </div>
    ...
    <Card>
      <CardContent className="p-4">
        ...
        <div className="grid grid-cols-7 gap-2">
          ...
          {placeholders.map((day, index) => (
            <DayCell key={`placeholder-${index}`} day={day} isPlaceholder={true} />
          ))}
