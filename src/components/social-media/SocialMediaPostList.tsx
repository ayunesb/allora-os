...
export default function SocialMediaPostList({ posts, onEditPost, onDeletePost, onSchedulePost, onApprovePost, "aria-label": ariaLabel }) {
    ...
    return (<div className="space-y-4" role={screenReaderFriendly ? "list" : undefined} aria-label={ariaLabel}>
      {posts.map((post) => (<Card key={post.id} className="overflow-hidden hover:shadow-md transition-shadow" role={screenReaderFriendly ? "listitem" : undefined}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {post.is_approved && (<Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
              ...
              <div className="flex items-center gap-2 self-end sm:self-auto mt-2 sm:mt-0">
                {post.status === 'draft' && !post.is_approved && (<Button size="sm" variant="outline" onClick={() => onApprovePost(post.id)} aria-label={screenReaderFriendly ? `Approve post: ${post.title}` : undefined}>
                    Approve
                  </Button>)}
                
                {post.status === 'draft' && post.is_approved && (<Button size="sm" onClick={() => onSchedulePost(post.id)} aria-label={screenReaderFriendly ? `Schedule post: ${post.title}` : undefined}>
                    Schedule
                  </Button>)}
                
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button size="icon" variant="ghost" aria-label={screenReaderFriendly ? `More options for post: ${post.title}` : "More options"}>
