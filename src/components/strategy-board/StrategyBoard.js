...
export default function StrategyBoard() {
    ...
    return (<div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between gap-4">
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          <div className="relative w-full sm:w-80">
            ...
            <Input placeholder="Search strategies..." className="pl-9" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
          </div>
      ...
      {isLoading ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        </div>) : filteredAndSortedStrategies.length === 0 ? (<div className="bg-muted/30 border border-border rounded-lg p-8 text-center">
          ...
          {searchQuery || riskFilter !== "all" ? (<Button variant="outline" onClick={() => {
                    setSearchQuery("");
                    setRiskFilter("all");
                }}>
              Clear Filters
            </Button>) : (<Button onClick={handleNewStrategy}>
              <Plus className="mr-2 h-4 w-4"/>
              Create Strategy
            </Button>)}
        </div>) : (<StrategyGrid strategies={filteredAndSortedStrategies} onDebate={handleDebate} onExport={handleExport} onViewStrategy={handleViewStrategy}/>)}
      
      {selectedStrategy && (<Dialog open={showStrategyDialog} onOpenChange={setShowStrategyDialog}>
