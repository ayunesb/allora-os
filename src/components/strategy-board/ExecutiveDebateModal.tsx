      ...
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] bg-black/80 backdrop-blur-xl border border-white/10 overflow-hidden flex flex-col">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden">
          ...
          <TabsContent value="debate" className="flex-1 overflow-hidden flex flex-col">
            {isLoading ? (<div className="flex flex-col items-center justify-center py-10">
              ...
              </div>) : (<div className="space-y-6 overflow-y-auto py-4 pr-2 flex-1">
                <div className="space-y-6">
                  {debateMessages.map((message, index) => {
                const isEven = index % 2 === 0;
                return (<div key={message.id} className={`flex ${isEven ? 'justify-start' : 'justify-end'}`}>
                        <div className={`flex ${isEven ? 'flex-row' : 'flex-row-reverse'} gap-3 max-w-[80%]`}>
        ...
        <DialogFooter className="flex flex-col sm:flex-row gap-2 border-t border-white/10 pt-4">
          <div className="flex items-center gap-2 sm:mr-auto">
            <Button variant="ghost" size="sm" className="h-8 px-2" onClick={() => handleFeedback(true)}>
              <ThumbsUp className="h-4 w-4 text-green-400"/>
            </Button>
            <Button variant="ghost" size="sm" className="h-8 px-2" onClick={() => handleFeedback(false)}>
              <ThumbsDown className="h-4 w-4 text-red-400"/>
            </Button>
          </div>
          
          <Button variant="outline" className="bg-black/50 border-white/10" onClick={handleExportPDF}>
