      ...
      <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto bg-[#0c0f1f] border-gray-800 text-white">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-4">
          <TabsList className={`bg-gray-800/50 ${isMobileView ? 'w-full tabs-scrollable' : ''}`}>
            <TabsTrigger value="details" className={isMobileView ? 'text-xs px-2 py-1 tab-compact' : ''}>
              <FileText className="h-4 w-4 mr-2"/>
              Strategy Details
            </TabsTrigger>
            <TabsTrigger value="implementation" className={isMobileView ? 'text-xs px-2 py-1 tab-compact' : ''}>
          ...
          <TabsContent value="details" className="pt-4">
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {strategy.impact && (<div className="bg-gray-800/50 p-4 rounded-lg">
                    <h3 className="text-sm font-medium text-gray-300 mb-2">Expected Impact</h3>
                    <div className="text-gray-100">{strategy.impact}</div>
                  </div>)}
                
                {strategy.timeframe && (<div className="bg-gray-800/50 p-4 rounded-lg">
          ...
          <TabsContent value="implementation">
            <StrategyImplementationTools strategyId={strategy.id} strategyName={strategy.title}/>
          </TabsContent>
        </Tabs>
      </DialogContent>
    </Dialog>);
