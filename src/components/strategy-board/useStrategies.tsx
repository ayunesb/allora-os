  ...
  useEffect(() => {
    const fetchStrategies = async () => {
      ...
      try {
        ...
        const { data: realStrategies, error: fetchError } = await supabase
          .from("strategies")
          .select("*")
        ...
        const strategyInsights = insights.filter(
          (insight) => insight.type === ("strategy" as InsightType),
        );

        // Convert insights to strategy format
        const aiGeneratedStrategies: Strategy[] = strategyInsights.map(
          (insight) => {
        ...
        startTransition(() => {
          ...
          const newAiStrategies = aiGeneratedStrategies.filter(
