...
import { FeatureComparisonItem, pricingTiers } from './pricingData';
...
const PlanComparisonTable: React.FC<PlanComparisonTableProps> = ({ featureData }) => {
                      ...
                      <TableCell className="py-4">
                        <div className="flex items-start gap-2">
                          <div>
                            ...
                            {feature.description && (
                              <div className="text-muted-foreground text-xs mt-1">{feature.description}</div>
                            )}
                          </div>
                          {feature.description && (
                            <TooltipProvider>
...
const MobilePlanComparison: React.FC<PlanComparisonTableProps> = ({ featureData }) => {
    ...
    <div className="mt-8 space-y-8">
      <div className="flex justify-center space-x-2 mb-4">
        {pricingTiers.map((tier, index) => (
          <Badge 
            key={index} 
            variant={tier.title === selectedPlan.title ? "default" : "outline"}
      ...
      <div className="border rounded-lg overflow-hidden">
            ...
            <div key={categoryIndex}>
              ...
              <div className="divide-y">
                {category.features.map((feature, featureIndex) => {
                  ...
                  return (
                    <div key={`${categoryIndex}-${featureIndex}`} className="p-3 flex justify-between items-center">
                      <div>
                        <div className="font-medium">{feature.name}</div>
                        {feature.description && (
                          <div className="text-muted-foreground text-xs mt-1">{feature.description}</div>
        ...
        <div className="p-4 border-t">
          <Button 
            ...
            className="w-full"
            onClick={() => handlePlanSelect(selectedPlan.priceId, selectedPlan.title)}
          >
...
function getPlanValue(feature: any, planName: string): boolean | string {
  return feature[planName];
}

function handlePlanSelect(priceId?: string, planName?: string) {
  if (!priceId) {
    // For Enterprise plan
