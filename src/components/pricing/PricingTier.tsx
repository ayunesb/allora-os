...
const PricingTier: React.FC<PricingTierProps> = ({ 
  ...
  const Badge = ({ children, variant }: { children: React.ReactNode, variant: 'popular' | 'recommended' | 'current' }) => {
    const bgColor = 
      variant === 'popular' ? 'bg-primary' :
      variant === 'recommended' ? 'bg-green-600' : 'bg-blue-600';
    
    const textColor = 'text-white';
    
    return (
      <div className={`py-1 px-4 ${bgColor} ${textColor} text-center text-sm font-medium`}>
        {children}
      </div>
    );
  };
  ...
  return (
    <Card className={`flex flex-col ${popular ? 'border-primary shadow-lg' : isRecommended ? 'border-green-500 shadow-lg' : currentPlan ? 'border-blue-500 shadow-lg' : ''}`}>
