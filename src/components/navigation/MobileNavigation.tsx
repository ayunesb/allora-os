...
const MobileNavigation = ({ isOpen, setIsOpen, navItems, onSignOut, isSigningOut, userName, userEmail }) => {
    ...
    return (<Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger asChild>
        <Button variant="ghost" size="sm" className="mr-2 flex md:hidden touch-target" aria-label="Toggle Menu">
      ...
      <SheetContent side="left" className="pr-0 w-[85vw] max-w-[280px] sm:max-w-[320px]">
        <SheetHeader className="text-left">
          <div className="flex items-center mb-4">
            ...
            <div className="flex flex-col overflow-hidden">
              ...
              {userEmail && (<SheetDescription className="text-left text-xs mt-1 truncate">
                  {userEmail}
                </SheetDescription>)}
            </div>
          </div>
        ...
        <div className="py-4">
          {navItems.map((item) => (<Link key={item.name} to={item.href} onClick={() => setIsOpen(false)}>
        ...
        <div className="absolute bottom-0 left-0 right-0 border-t border-border/50 p-4">
          ...
          <Button variant="destructive" className="w-full justify-start" onClick={onSignOut} disabled={isSigningOut}>
