...
export default function NavigationFixer() {
    ...
    useEffect(() => {
        ...
        const isKnownValid = allValidPaths.some((path) => currentPath === path || currentPath.startsWith(`${path}/`));
        ...
        const isRestrictedPath = restrictedPaths.some((path) => currentPath.startsWith(path) && !isAuthenticated);
        if (isRestrictedPath) {
            // Log suspicious activity
            setSuspiciousRouteAttempts((prev) => {
                const count = (prev[currentPath] || 0) + 1;
                const newState = Object.assign(Object.assign({}, prev), { [currentPath]: count });
