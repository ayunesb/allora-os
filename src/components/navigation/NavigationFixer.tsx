...
export default function NavigationFixer() {
  ...
  const [suspiciousRouteAttempts, setSuspiciousRouteAttempts] = useState<
  ...
  useEffect(() => {
    ...
    const isKnownValid = allValidPaths.some(
    ...
    const isRestrictedPath = restrictedPaths.some(
      (path) => currentPath.startsWith(path) && !isAuthenticated,
    );
    ...
    if (isRestrictedPath) {
      ...
      // Redirect to login
      logger.info(
        `Unauthorized access attempt to restricted area: ${currentPath}`,
      );
    ...
    if (normalizedPath !== currentPath) {
      logger.info(
        `Fixing navigation: Redirecting from ${currentPath} to ${normalizedPath}`,
      );
    ...
    if (currentPath.includes("/legal/") && !isValidLegalRoute(currentPath)) {
      ...
      if (suggestedRoutes.length > 0) {
        toast.info(`Redirecting to ${suggestedRoutes[0].name}`);
        logger.info(
          `Suggested legal routes: ${JSON.stringify(suggestedRoutes)}`,
        );
        navigate(suggestedRoutes[0].path, {
          replace: true,
          state: {
