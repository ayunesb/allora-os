    ...
    return (<Card className="w-full">
      <CardHeader className="pb-2">
        <CardTitle className="text-lg flex items-center gap-2">
          Messages
          {messages.filter(m => !m.read).length > 0 && (<Badge variant="destructive" className="rounded-full h-5 min-w-5 flex items-center justify-center p-1">
      ...
      <CardContent>
        {isLoading ? (<div className="flex items-center justify-center p-6">
          ...
          </div>) : messages.length > 0 ? (<ScrollArea className="h-[320px] pr-4">
            <div className="space-y-4">
              {messages.map((message) => (<div key={message.id} className={`flex gap-3 ${!message.read ? 'bg-primary/5 p-2 rounded-md' : ''}`}>
          ...
          </ScrollArea>) : (<div className="text-center p-6 text-muted-foreground">
            <p>No messages yet</p>
