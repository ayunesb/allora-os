    ...
    return (<Card className="w-full">
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <select id="executive" className="w-full p-2 border rounded-md" value={selectedExecutive} onChange={(e) => setSelectedExecutive(e.target.value)}>
        ...
        <div className="space-y-2">
          ...
          <textarea id="prompt" rows={3} className="w-full p-2 border rounded-md" value={prompt} onChange={(e) => setPrompt(e.target.value)}/>
        </div>
        
        <div className="flex items-center space-x-4">
          <label className="flex items-center space-x-2 text-sm">
            <input type="checkbox" checked={saveToNotion} onChange={() => setSaveToNotion(!saveToNotion)}/>
            <span>Save to Notion</span>
          </label>
          
          <label className="flex items-center space-x-2 text-sm">
            <input type="checkbox" checked={addToAuditLog} onChange={() => setAddToAuditLog(!addToAuditLog)}/>
        ...
        {response && (<Tabs defaultValue="response" className="mt-6">
            ...
            <TabsContent value="response">
              <OutputStream text={response.aiResponse} executive={selectedExecutive}/>
            </TabsContent>
            
            <TabsContent value="tool-execution">
              {response.toolResponses && response.toolResponses.length > 0 ? (<div className="space-y-4">
                  {response.toolResponses.map((toolResponse, index) => (<Card key={index} className="overflow-hidden">
                      ...
                      <CardContent className="p-4">
                        <p className="mb-2 font-medium">{toolResponse.result}</p>
                        {toolResponse.data && (<pre className="bg-muted p-2 rounded text-xs overflow-auto">
