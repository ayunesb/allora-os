...
const RecentExecutiveMessages = ({ messages, isLoading = false, onViewMoreMessages }) => {
    if (isLoading) {
        ...
        <CardContent>
          <div className="space-y-4">
            {[1, 2, 3].map((i) => (<div key={i} className="flex items-start space-x-3">
    ...
    return (<Card>
      ...
      <CardContent>
        <div className="space-y-4">
          {normalizedMessages.slice(0, 5).map((message) => (<div key={message.id} className="flex items-start space-x-3">
              <div className="w-2 h-2 rounded-full bg-primary mt-2"/>
              <div className="space-y-1 flex-1">
                <div className="flex justify-between items-center">
                  {message.from_executive && (<Badge variant="outline" className="bg-primary/10 text-primary text-xs">
                      From {typeof message.from_executive === 'string' ? message.from_executive : 'Executive'}
                    </Badge>)}
                  {message.to_executive && (<Badge variant="outline" className="bg-secondary/10 text-secondary text-xs">
                      To {typeof message.to_executive === 'string' ? message.to_executive : 'Executive'}
                    </Badge>)}
                  {message.status && (<span className={`text-xs ${message.status === 'read' ? 'text-green-500' : 'text-amber-500'}`}>
        ...
        {messages.length > 5 && (<div className="mt-4 flex justify-center">
