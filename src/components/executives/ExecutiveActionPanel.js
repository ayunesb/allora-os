"use strict";
var __awaiter =
  (this && this.__awaiter) ||
  function (thisArg, _arguments, P, generator) {
    function adopt(value) {
      return value instanceof P
        ? value
        : new P(function (resolve) {
            resolve(value);
          });
    }
    return new (P || (P = Promise))(function (resolve, reject) {
      function fulfilled(value) {
        try {
          step(generator.next(value));
        } catch (e) {
          reject(e);
        }
      }
      function rejected(value) {
        try {
          step(generator["throw"](value));
        } catch (e) {
          reject(e);
        }
      }
      function step(result) {
        result.done
          ? resolve(result.value)
          : adopt(result.value).then(fulfilled, rejected);
      }
      step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
  };
var __generator =
  (this && this.__generator) ||
  function (thisArg, body) {
    var _ = {
        label: 0,
        sent: function () {
          if (t[0] & 1) throw t[1];
          return t[1];
        },
        trys: [],
        ops: [],
      },
      f,
      y,
      t,
      g = Object.create(
        (typeof Iterator === "function" ? Iterator : Object).prototype,
      );
    return (
      (g.next = verb(0)),
      (g["throw"] = verb(1)),
      (g["return"] = verb(2)),
      typeof Symbol === "function" &&
        (g[Symbol.iterator] = function () {
          return this;
        }),
      g
    );
    function verb(n) {
      return function (v) {
        return step([n, v]);
      };
    }
    function step(op) {
      if (f) throw new TypeError("Generator is already executing.");
      while ((g && ((g = 0), op[0] && (_ = 0)), _))
        try {
          if (
            ((f = 1),
            y &&
              (t =
                op[0] & 2
                  ? y["return"]
                  : op[0]
                    ? y["throw"] || ((t = y["return"]) && t.call(y), 0)
                    : y.next) &&
              !(t = t.call(y, op[1])).done)
          )
            return t;
          if (((y = 0), t)) op = [op[0] & 2, t.value];
          switch (op[0]) {
            case 0:
            case 1:
              t = op;
              break;
            case 4:
              _.label++;
              return { value: op[1], done: false };
            case 5:
              _.label++;
              y = op[1];
              op = [0];
              continue;
            case 7:
              op = _.ops.pop();
              _.trys.pop();
              continue;
            default:
              if (
                !((t = _.trys), (t = t.length > 0 && t[t.length - 1])) &&
                (op[0] === 6 || op[0] === 2)
              ) {
                _ = 0;
                continue;
              }
              if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) {
                _.label = op[1];
                break;
              }
              if (op[0] === 6 && _.label < t[1]) {
                _.label = t[1];
                t = op;
                break;
              }
              if (t && _.label < t[2]) {
                _.label = t[2];
                _.ops.push(op);
                break;
              }
              if (t[2]) _.ops.pop();
              _.trys.pop();
              continue;
          }
          op = body.call(thisArg, _);
        } catch (e) {
          op = [6, e];
          y = 0;
        } finally {
          f = t = 0;
        }
      if (op[0] & 5) throw op[1];
      return { value: op[0] ? op[1] : void 0, done: true };
    }
  };
Object.defineProperty(exports, "__esModule", { value: true });
exports.ExecutiveActionPanel = void 0;
var jsx_runtime_1 = require("react/jsx-runtime");
var react_1 = require("react");
var useExecutiveAgent_1 = require("@/hooks/useExecutiveAgent");
var button_1 = require("@/components/ui/button");
var card_1 = require("@/components/ui/card");
var lucide_react_1 = require("lucide-react");
var tabs_1 = require("@/components/ui/tabs");
var OutputStream_1 = require("./OutputStream");
var ExecutiveActionPanel = function () {
  var _a = (0, react_1.useState)(
      "Should we increase ad budget based on this month's revenue?",
    ),
    prompt = _a[0],
    setPrompt = _a[1];
  var _b = (0, react_1.useState)("CMO"),
    selectedExecutive = _b[0],
    setSelectedExecutive = _b[1];
  var _c = (0, react_1.useState)(true),
    saveToNotion = _c[0],
    setSaveToNotion = _c[1];
  var _d = (0, react_1.useState)(true),
    addToAuditLog = _d[0],
    setAddToAuditLog = _d[1];
  var _e = (0, useExecutiveAgent_1.useExecutiveAgent)(),
    executeQuery = _e.executeQuery,
    isLoading = _e.isLoading,
    response = _e.response,
    reset = _e.reset;
  var handleRun = function () {
    return __awaiter(void 0, void 0, void 0, function () {
      return __generator(this, function (_a) {
        switch (_a.label) {
          case 0:
            return [
              4 /*yield*/,
              executeQuery(prompt, selectedExecutive, {
                saveToNotion: saveToNotion,
                addToAuditLog: addToAuditLog,
                userContext: {
                  timestamp: new Date().toISOString(),
                },
              }),
            ];
          case 1:
            _a.sent();
            return [2 /*return*/];
        }
      });
    });
  };
  return (0, jsx_runtime_1.jsxs)(card_1.Card, {
    className: "w-full",
    children: [
      (0, jsx_runtime_1.jsx)(card_1.CardHeader, {
        children: (0, jsx_runtime_1.jsxs)(card_1.CardTitle, {
          className: "flex items-center gap-2",
          children: [
            (0, jsx_runtime_1.jsx)(lucide_react_1.Zap, {
              className: "h-5 w-5 text-amber-500",
            }),
            "Executive Action Panel",
          ],
        }),
      }),
      (0, jsx_runtime_1.jsxs)(card_1.CardContent, {
        className: "space-y-4",
        children: [
          (0, jsx_runtime_1.jsxs)("div", {
            className: "space-y-2",
            children: [
              (0, jsx_runtime_1.jsx)("label", {
                htmlFor: "executive",
                className: "text-sm font-medium",
                children: "Executive Role",
              }),
              (0, jsx_runtime_1.jsxs)("select", {
                id: "executive",
                className: "w-full p-2 border rounded-md",
                value: selectedExecutive,
                onChange: function (e) {
                  return setSelectedExecutive(e.target.value);
                },
                children: [
                  (0, jsx_runtime_1.jsx)("option", {
                    value: "CEO",
                    children: "CEO",
                  }),
                  (0, jsx_runtime_1.jsx)("option", {
                    value: "CMO",
                    children: "CMO",
                  }),
                  (0, jsx_runtime_1.jsx)("option", {
                    value: "CTO",
                    children: "CTO",
                  }),
                  (0, jsx_runtime_1.jsx)("option", {
                    value: "CFO",
                    children: "CFO",
                  }),
                  (0, jsx_runtime_1.jsx)("option", {
                    value: "COO",
                    children: "COO",
                  }),
                ],
              }),
            ],
          }),
          (0, jsx_runtime_1.jsxs)("div", {
            className: "space-y-2",
            children: [
              (0, jsx_runtime_1.jsx)("label", {
                htmlFor: "prompt",
                className: "text-sm font-medium",
                children: "Your Question",
              }),
              (0, jsx_runtime_1.jsx)("textarea", {
                id: "prompt",
                rows: 3,
                className: "w-full p-2 border rounded-md",
                value: prompt,
                onChange: function (e) {
                  return setPrompt(e.target.value);
                },
              }),
            ],
          }),
          (0, jsx_runtime_1.jsxs)("div", {
            className: "flex items-center space-x-4",
            children: [
              (0, jsx_runtime_1.jsxs)("label", {
                className: "flex items-center space-x-2 text-sm",
                children: [
                  (0, jsx_runtime_1.jsx)("input", {
                    type: "checkbox",
                    checked: saveToNotion,
                    onChange: function () {
                      return setSaveToNotion(!saveToNotion);
                    },
                  }),
                  (0, jsx_runtime_1.jsx)("span", {
                    children: "Save to Notion",
                  }),
                ],
              }),
              (0, jsx_runtime_1.jsxs)("label", {
                className: "flex items-center space-x-2 text-sm",
                children: [
                  (0, jsx_runtime_1.jsx)("input", {
                    type: "checkbox",
                    checked: addToAuditLog,
                    onChange: function () {
                      return setAddToAuditLog(!addToAuditLog);
                    },
                  }),
                  (0, jsx_runtime_1.jsx)("span", {
                    children: "Add to Audit Log",
                  }),
                ],
              }),
            ],
          }),
          (0, jsx_runtime_1.jsx)(button_1.Button, {
            onClick: handleRun,
            className: "w-full",
            disabled: isLoading,
            children: isLoading
              ? (0, jsx_runtime_1.jsxs)(jsx_runtime_1.Fragment, {
                  children: [
                    (0, jsx_runtime_1.jsx)(lucide_react_1.Loader2, {
                      className: "mr-2 h-4 w-4 animate-spin",
                    }),
                    "Processing...",
                  ],
                })
              : "Ask ".concat(selectedExecutive),
          }),
          response &&
            (0, jsx_runtime_1.jsxs)(tabs_1.Tabs, {
              defaultValue: "response",
              className: "mt-6",
              children: [
                (0, jsx_runtime_1.jsxs)(tabs_1.TabsList, {
                  className: "grid grid-cols-3 mb-4",
                  children: [
                    (0, jsx_runtime_1.jsx)(tabs_1.TabsTrigger, {
                      value: "response",
                      children: "Response",
                    }),
                    (0, jsx_runtime_1.jsx)(tabs_1.TabsTrigger, {
                      value: "tool-execution",
                      children: "Tool Execution",
                    }),
                    (0, jsx_runtime_1.jsx)(tabs_1.TabsTrigger, {
                      value: "raw",
                      children: "Raw Output",
                    }),
                  ],
                }),
                (0, jsx_runtime_1.jsx)(tabs_1.TabsContent, {
                  value: "response",
                  children: (0, jsx_runtime_1.jsx)(
                    OutputStream_1.OutputStream,
                    { text: response.aiResponse, executive: selectedExecutive },
                  ),
                }),
                (0, jsx_runtime_1.jsx)(tabs_1.TabsContent, {
                  value: "tool-execution",
                  children:
                    response.toolResponses && response.toolResponses.length > 0
                      ? (0, jsx_runtime_1.jsx)("div", {
                          className: "space-y-4",
                          children: response.toolResponses.map(
                            function (toolResponse, index) {
                              return (0, jsx_runtime_1.jsxs)(
                                card_1.Card,
                                {
                                  className: "overflow-hidden",
                                  children: [
                                    (0, jsx_runtime_1.jsx)(card_1.CardHeader, {
                                      className: "bg-muted py-2",
                                      children: (0, jsx_runtime_1.jsxs)(
                                        card_1.CardTitle,
                                        {
                                          className:
                                            "text-sm font-medium capitalize",
                                          children: [
                                            toolResponse.tool,
                                            " Tool",
                                          ],
                                        },
                                      ),
                                    }),
                                    (0, jsx_runtime_1.jsxs)(
                                      card_1.CardContent,
                                      {
                                        className: "p-4",
                                        children: [
                                          (0, jsx_runtime_1.jsx)("p", {
                                            className: "mb-2 font-medium",
                                            children: toolResponse.result,
                                          }),
                                          toolResponse.data &&
                                            (0, jsx_runtime_1.jsx)("pre", {
                                              className:
                                                "bg-muted p-2 rounded text-xs overflow-auto",
                                              children: JSON.stringify(
                                                toolResponse.data,
                                                null,
                                                2,
                                              ),
                                            }),
                                        ],
                                      },
                                    ),
                                  ],
                                },
                                index,
                              );
                            },
                          ),
                        })
                      : (0, jsx_runtime_1.jsx)("div", {
                          className: "p-4 text-center text-muted-foreground",
                          children: "No tools were executed for this query",
                        }),
                }),
                (0, jsx_runtime_1.jsx)(tabs_1.TabsContent, {
                  value: "raw",
                  children: (0, jsx_runtime_1.jsx)("pre", {
                    className:
                      "bg-muted p-4 rounded text-xs overflow-auto max-h-[300px]",
                    children: JSON.stringify(response, null, 2),
                  }),
                }),
              ],
            }),
        ],
      }),
    ],
  });
};
exports.ExecutiveActionPanel = ExecutiveActionPanel;
exports.default = exports.ExecutiveActionPanel;
