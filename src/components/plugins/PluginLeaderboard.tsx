...
const PluginLeaderboard: React.FC<PluginLeaderboardProps> = ({ children, variant = "default", size = "large" }) => {
    ...
    if (isLoading) {
        ...
        <CardContent>
          <div className="space-y-4">
            {[1, 2, 3].map((i) => (<div key={i} className="flex justify-between items-center animate-pulse">
    ...
    return (<Card className={className}>
      ...
      <CardContent>
        <div className="space-y-4">
          {pluginImpact.slice(0, 5).map((plugin, index) => (<div key={plugin.plugin_name} className="flex justify-between items-center">
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className="font-medium">{index + 1}.</span>
                  <span className="font-medium">{plugin.plugin_name}</span>
                  {index === 0 && (<Badge variant="outline" className="bg-amber-100 text-amber-800 border-amber-200">
                      Top ROI
                    </Badge>)}
                </div>
