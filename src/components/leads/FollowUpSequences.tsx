        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="predefined" className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              {lead && (<Button onClick={generateAISequence} disabled={generating} size="sm">
                  {generating ? (<>
            ...
            .map(sequence => (<div key={sequence.id} className="border rounded-md p-4">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <h4 className="font-medium flex items-center">
                          {sequence.name}
                          {sequence.aiGenerated && (<span className="ml-2 px-2 py-0.5 bg-blue-100 text-blue-800 text-xs rounded-full">
                              AI Generated
                            </span>)}
                        </h4>
                      ...
                      <Button variant="secondary" size="sm" onClick={() => handleApplySequence(sequence.id)}>
                        Apply
                      </Button>
                    </div>
                    
                    <div className="space-y-2 mt-4">
                      {sequence.steps.map((step, index) => (<div key={step.id} className="flex items-start gap-3 p-2 rounded-md bg-muted/50">
          ...
          <TabsContent value="custom" className="space-y-4">
            ...
            .map(sequence => (<div key={sequence.id} className="border rounded-md p-4">
              ...
              {sequences.filter(seq => !seq.aiGenerated).length === 0 && (<div className="flex flex-col items-center justify-center py-8 px-4 border rounded-md bg-muted/20">
            ...
            <div className="space-y-4">
              <div className="grid gap-4">
                <div className="space-y-2">
                  ...
                  <Input id="sequence-name" placeholder="e.g., Product Demo Follow-Up" value={newSequence.name} onChange={(e) => setNewSequence(prev => ({ ...prev, name: e.target.value }))}/>
                </div>
                
                <div className="space-y-2">
                  <Label htmlFor="sequence-description">Description</Label>
                  <Textarea id="sequence-description" placeholder="Describe what this sequence is for" value={newSequence.description} onChange={(e) => setNewSequence(prev => ({ ...prev, description: e.target.value }))}/>
                </div>
                
                <div className="space-y-2">
                  <Label htmlFor="target-audience">Target Audience</Label>
                  <Select value={newSequence.targetAudience} onValueChange={(value) => setNewSequence(prev => ({ ...prev, targetAudience: value }))}>
                    <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                {(newSequence.steps || []).length === 0 ? (<div className="flex flex-col items-center justify-center py-8 px-4 border rounded-md bg-muted/20">
                    ...
                    {(newSequence.steps || []).map((step, index) => (<div key={step.id} className="border rounded-md p-4 space-y-4">
                        <div className="flex justify-between items-center">
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="space-y-2">
                            ...
                            <Textarea id={`step-content-${step.id}`} placeholder="Enter the content or description of this step" value={step.content} onChange={(e) => handleUpdateStep(step.id, 'content', e.target.value)}/>
                          </div>
                          
                          <div className="space-y-2">
                            <Label htmlFor={`step-delay-${step.id}`}>Delay (days)</Label>
                            <div className="flex items-center gap-2">
                              <Input id={`step-delay-${step.id}`} type="number" min="0" value={step.delayDays} onChange={(e) => handleUpdateStep(step.id, 'delayDays', parseInt(e.target.value))}/>
