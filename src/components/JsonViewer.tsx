...
const JsonViewer = ({ data, expandLevel = 2 }) => {
    const formatJson = (obj, level = 0) => {
        ...
        if (typeof obj === 'boolean')
            return <span className="text-orange-600">{obj ? 'true' : 'false'}</span>;
        if (Array.isArray(obj)) {
            ...
            return (<div className={`ml-${level > 0 ? '4' : '0'}`}>
          <span className="text-gray-700">[</span>
          <div className="ml-4">
            {obj.map((item, i) => (<div key={i} className="flex">
        ...
        if (typeof obj === 'object') {
          ...
          <span className="text-gray-700">{'{'}</span>
          <div className="ml-4">
            {keys.map((key, i) => (<div key={key} className="flex">
    ...
    try {
        return (<pre className="text-xs font-mono whitespace-pre-wrap break-all">
        {formatJson(data)}
      </pre>);
    }
    catch (error) {
