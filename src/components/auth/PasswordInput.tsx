...
export default function PasswordInput({ form, name, label, showStrengthMeter = false, setShowTips }) {
    ...
    return (<FormField control={form.control} name={name} render={({ field }) => (<FormItem>
          ...
          <div className="relative">
            <FormControl>
              <Input type={showPassword ? "text" : "password"} placeholder="••••••••" {...field} onFocus={() => setShowTips && setShowTips(true)} onBlur={() => setShowTips && setShowTips(false)} className="pr-10"/>
            </FormControl>
          ...
          {showStrengthMeter && form.watch(name) && (<PasswordStrengthMeter password={form.watch(name)}/>)}
