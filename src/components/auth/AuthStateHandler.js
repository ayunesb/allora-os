...
export const AuthStateHandler = ({ isLoading, authError, onRetry, isRetrying, children }) => {
    ...
    if (loadingTimeout && isLoading) {
        return (<AuthErrorState error="Loading took too long. There might be an issue with the connection." onRetry={onRetry} isRetrying={isRetrying}/>);
    }
    if (authError) {
        return <AuthErrorState error={authError} onRetry={onRetry} isRetrying={isRetrying}/>;
    }
    return <>{children}</>;
};
