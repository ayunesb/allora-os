...
export default function PasswordStrengthMeter({ password }) {
    ...
    return (<div className="space-y-3">
      ...
      {password && (<div className="text-xs font-medium">
          Password strength: <span className="font-semibold">{getStrengthLabel(strength)}</span>
        </div>)}
      
      {password && (<div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
          {passwordRequirements.map((requirement) => {
                const isFulfilled = requirement.validator(password);
                return (<div key={requirement.id} className={`flex items-center gap-1.5 ${isFulfilled ? 'text-green-500' : 'text-muted-foreground'}`}>
                {isFulfilled ? (<Check className="h-3.5 w-3.5"/>) : (<X className="h-3.5 w-3.5"/>)}
