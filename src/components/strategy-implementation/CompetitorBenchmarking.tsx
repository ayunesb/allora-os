...
const initialFormState: CompetitorFormState = {
  competitorName: '',
  marketShare: 0,
  strengthScore: 5,
  weaknessScore: 5,
...
const CompetitorBenchmarking: React.FC<CompetitorBenchmarkingProps> = ({ strategyId }) => {
  ...
  const [formState, setFormState] = useState<CompetitorFormState>(initialFormState);
  ...
  const handleSliderChange = (name: 'strengthScore' | 'weaknessScore', value: number[]) => {
    setFormState(prev => ({
      ...prev,
      [name]: value[0]
    }));
    ...
    try {
      if (formState.id) {
        ...
        setCompetitors(prev => prev.map(c => 
          c.id === formState.id ? { ...c, ...formState } : c
        ));
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Slider 
                ...
                onValueChange={(value) => handleSliderChange('strengthScore', value)} 
              />
            </div>
            ...
            <div>
              ...
              <Slider 
                ...
                onValueChange={(value) => handleSliderChange('weaknessScore', value)} 
              />
            </div>
            ...
            {competitors.some(c => c.notes) && (
              <div className="mt-4">
                ...
                <div className="space-y-3">
                  {competitors.filter(c => c.notes).map(competitor => (
                    <Card key={`notes-${competitor.id}`}>
