    ...
    return (<Card className="shadow-md">
      ...
      <CardContent>
        {isLoading ? (<div className="py-6 text-center text-muted-foreground">
            Loading milestones...
          </div>) : milestones.length === 0 ? (<div className="py-12 text-center text-muted-foreground">
      ...
      {isDialogOpen && (<MilestoneDialog isOpen={isDialogOpen} onClose={() => setIsDialogOpen(false)} strategyId={strategyId} milestone={currentMilestone} onSave={handleMilestoneSaved} onDelete={handleMilestoneDeleted}/>)}
    </Card>);
};
