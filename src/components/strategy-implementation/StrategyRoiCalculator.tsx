        ...
        {isLoading ? (<div className="py-6 text-center text-muted-foreground">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input id="projectedRevenue" type="number" value={projectedRevenue} onChange={(e) => setProjectedRevenue(Number(e.target.value))}/>
              </div>
              
              <div>
                <Label htmlFor="timeframeMonths">Timeframe (Months)</Label>
                <Input id="timeframeMonths" type="number" value={timeframeMonths} onChange={(e) => setTimeframeMonths(Number(e.target.value))}/>
              </div>
              
              <div>
                <Label htmlFor="annualCosts">Annual Ongoing Costs ($)</Label>
                <Input id="annualCosts" type="number" value={annualCosts} onChange={(e) => setAnnualCosts(Number(e.target.value))}/>
              </div>
              
              <div className="pt-4 border-t">
                <Label htmlFor="actualRevenue">Actual Revenue ($) (Optional)</Label>
                <Input id="actualRevenue" type="number" value={actualRevenue === undefined ? '' : actualRevenue} onChange={(e) => setActualRevenue(e.target.value ? Number(e.target.value) : undefined)} placeholder="Enter once strategy is implemented"/>
              </div>
              
              <div>
                <Label htmlFor="actualCosts">Actual Costs ($) (Optional)</Label>
                <Input id="actualCosts" type="number" value={actualCosts === undefined ? '' : actualCosts} onChange={(e) => setActualCosts(e.target.value ? Number(e.target.value) : undefined)} placeholder="Enter once strategy is implemented"/>
              </div>
            ...
            <div className="space-y-4">
              ...
              {actualROI !== undefined && (<div className="bg-muted p-4 rounded-md">
              ...
              <div className="h-[300px] mt-4">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={getChartData()} cx="50%" cy="50%" labelLine={false} outerRadius={80} fill="#8884d8" dataKey="value" label={({ name, percent }) => `${name}: ${(percent * 100).toFixed(0)}%`}>
