...
export default function AuditLogTable({ logs }) {
    ...
    if (isMobileView) {
        const columns = [
            {
                key: 'timestamp',
                title: 'Timestamp',
                render: (log) => (<span className="font-mono text-xs">{new Date(log.timestamp).toLocaleString()}</span>)
            },
            {
                key: 'user',
                title: 'User',
            },
            {
                key: 'action',
                title: 'Action',
                render: (log) => (<span className={cn("px-2 py-1 rounded-full text-xs", {
        ...
        const emptyState = (<div className="text-center py-8 bg-muted/20 rounded-md">
    ...
    return (<div className="rounded-md border">
      <Table>
        ...
        <TableBody>
          {logs.length > 0 ? (logs.map((log) => (<TableRow key={log.id}>
