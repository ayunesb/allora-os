...
export default function DocumentListItem({ id, name, currentVersion, lastUpdated, status, nextUpdateDue, autoUpdatesEnabled, onUpdate, onToggleAutoUpdate }) {
    return (<div className={`p-4 border rounded-lg transition-colors ${status === "outdated"
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <div className="flex flex-col text-sm text-muted-foreground mt-1 gap-1">
            ...
            {nextUpdateDue && (<div className="flex items-center">
        ...
        <div className="flex flex-wrap gap-2 ml-auto">
          {status === "update-available" && (<Button size="sm" onClick={() => onUpdate(id)} className="bg-amber-500 hover:bg-amber-600">
              Update Now
            </Button>)}
          
          <Button variant="outline" size="sm" onClick={() => onToggleAutoUpdate(id)}>
            {autoUpdatesEnabled ? (<>
