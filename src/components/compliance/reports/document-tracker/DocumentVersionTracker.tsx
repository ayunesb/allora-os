    ...
    return (<Card>
      ...
      <CardContent>
        <div className="space-y-4">
          {compliance.isCheckingUpdates ? (<div className="space-y-2">
              <Skeleton className="h-4 w-3/4"/>
              <Skeleton className="h-4 w-1/2"/>
              <Skeleton className="h-4 w-2/3"/>
            </div>) : (<div className="flex items-center justify-between border-b pb-2 mb-2">
              <div>
                <p className="text-sm font-medium">Last document check</p>
                <div className="flex items-center text-xs text-muted-foreground">
                  <Clock className="h-3 w-3 mr-1"/>
                  {compliance.lastChecked ? (formatRelativeTime(compliance.lastChecked)) : ('Never checked')}
                </div>
              </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center">
              <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" className="sr-only peer" checked={compliance.autoUpdate || false} onChange={(e) => compliance.setAutoUpdate && compliance.setAutoUpdate(e.target.checked)}/>
