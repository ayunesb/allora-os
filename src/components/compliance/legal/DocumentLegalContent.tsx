...
export default function DocumentLegalContent() {
    ...
    if (needsUpdate) {
      ...
      notificationToast.innerHTML = `
        <div class="flex items-start">
          ...
          <div>
            <h5 class="font-medium text-amber-800">Document Update Available</h5>
            <p class="text-sm text-amber-700 mt-1">This document needs to be updated to meet the latest compliance requirements.</p>
          </div>
        </div>
      `;
      ...
      notificationTimer = setInterval(() => {
        if (needsUpdate) {
          ...
          reminderToast.className = 'fixed top-4 right-4 bg-amber-50 border border-amber-200 p-4 rounded shadow-md z-50 animate-in fade-in slide-in-from-top-5 duration-300';
          reminderToast.innerHTML = `
            <div class="flex items-start">
              ...
              <div>
                <h5 class="font-medium text-amber-800">Reminder: Document Update Available</h5>
                <p class="text-sm text-amber-700 mt-1">Please update this document to stay compliant with latest regulations.</p>
              </div>
            </div>
          `;
          ...
          setTimeout(() => {
            if (document.body.contains(reminderToast)) {
              ...
              setTimeout(() => {
                if (document.body.contains(reminderToast)) {
                  document.body.removeChild(reminderToast);
                }
              }, 300);
            }
          }, 5000);
  ...
  }, [checkForUpdates, documentId, needsUpdate]);
          ...
          {needsUpdate && (
            <div className="mb-4 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded">
              <div className="flex">
                ...
                <div>
                  <h3 className="font-medium text-amber-800 dark:text-amber-300">Regulatory Update Available</h3>
                  <p className="text-sm text-amber-700 dark:text-amber-400">
                    This document needs to be updated to comply with the latest regulatory requirements.
                  </p>
                </div>
              </div>
            </div>
