...
export function DecisionTable({ decisions, loading, error }) {
    ...
    if (error) {
        return (<Card className="p-6 bg-red-50 border border-red-200 text-red-800">
        <p>{error}</p>
      </Card>);
    }
        ...
        <TableBody>
          {decisions.map((decision) => (<TableRow key={decision.id}>
              <TableCell className="font-medium">
                <div>
                  <Link to={`/dashboard/executives/${encodeURIComponent(decision.executiveName)}`} className="hover:underline hover:text-primary">
              ...
              <TableCell>
                {decision.priority && (<Badge className={getPriorityColor(decision.priority)}>
                    {decision.priority}
                  </Badge>)}
              </TableCell>
