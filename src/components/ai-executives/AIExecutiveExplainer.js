...
export function AIExecutiveExplainer() {
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {executiveRoles.map((role) => (<Card key={role.role} className="cursor-pointer hover:border-primary/50 transition-all">
            ...
            <CardContent>
              ...
              <ul className="text-sm text-muted-foreground space-y-1">
                {role.capabilities.slice(0, 3).map((capability, index) => (<li key={index} className="flex items-start">
            ...
            <CardFooter>
              <Button variant="outline" className="w-full" onClick={() => handleOpenRoleDialog(role)}>
      ...
      {selectedRole && (<Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="sm:max-w-[500px]">
            ...
            <div className="py-4">
              ...
              <ul className="space-y-1 mb-4">
                {selectedRole.capabilities.map((capability, index) => (<li key={index} className="flex items-start text-sm">
                    <span className="mr-2">•</span>
                    <span>{capability}</span>
                  </li>))}
              </ul>
              ...
              <ul className="space-y-1">
                {selectedRole.examples.map((example, index) => (<li key={index} className="flex items-start text-sm">
                    <span className="mr-2">•</span>
                    <span>{example}</span>
                  </li>))}
              </ul>
