...
export function UpgradeAllExecutives({ onUpgradeComplete }) {
    ...
    return (<Card className="border border-primary/20 shadow-sm">
      <CardHeader className="pb-2">
        <CardTitle className="text-lg font-medium">Executive OS Integration</CardTitle>
        <CardDescription>
      ...
      <CardContent className="pb-2">
        {isComplete ? (<div className="space-y-4">
            <div className="flex justify-center">
              {failedCount === 0 ? (<div className="flex items-center gap-2 text-primary">
                  <CheckCircle className="h-8 w-8"/>
                  <span className="text-xl font-medium">Complete</span>
                </div>) : (<div className="flex items-center gap-2 text-amber-500">
                  <AlertCircle className="h-8 w-8"/>
                  <span className="text-xl font-medium">Partially Complete</span>
                </div>)}
            </div>
            
            <div className="grid grid-cols-2 gap-4 text-center">
              <div>
                <div className="text-2xl font-bold text-primary">{upgradedCount}</div>
            ...
            <div className="space-y-1 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">3x3 Priorities, Eisenhower Matrix</span>
              </div>
              <div className="flex justify-between">
      ...
      <CardFooter className="pt-2">
        {isComplete ? (<Button className="w-full" variant="outline" onClick={resetUpgrade}>
            Reset
          </Button>) : (<Button className="w-full" variant="default" disabled={isLoading} onClick={handleUpgradeAll}>
            {isLoading ? (<>
