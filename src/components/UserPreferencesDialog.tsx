    ...
    return (<Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <motion.div whileTap={{ scale: 0.97 }} transition={{ duration: 0.1 }}>
          <Button variant={triggerVariant || "outline"} className="flex items-center gap-2">
            {triggerLabel || (<>
      ...
      <DialogContent className="max-w-2xl">
        ...
        <Tabs defaultValue="models" className="mt-4">
          ...
          <TabsContent value="models">
            <AIModelPreferences preferences={preferences} updatePreference={handleUpdatePreference}/>
          </TabsContent>
          
          <TabsContent value="style">
            <ResponseStylePreferences preferences={preferences} updatePreference={handleUpdatePreference}/>
          </TabsContent>
          
          <TabsContent value="learning">
            <LearningPreferences preferences={preferences} updatePreference={handleUpdatePreference}/>
          </TabsContent>
        </Tabs>
        
        <DialogFooter className="mt-6">
          <Button variant="outline" onClick={handleCancel} className="transition-all duration-200 hover:bg-destructive/10">
            Cancel
          </Button>
          <Button variant="outline" onClick={handleReset} disabled={isSaving || isLoading} className="transition-all duration-200 hover:bg-amber-500/10">
            Reset Defaults
          </Button>
          <Button onClick={handleSave} disabled={isSaving || isLoading} className="min-w-24 relative group">
            {(isSaving || isLoading) ? (<>
                <Loader2 className="h-4 w-4 mr-2 animate-spin"/>
                Saving...
              </>) : (<motion.span initial={{ opacity: 1 }} whileTap={{ scale: 0.97 }} transition={{ duration: 0.1 }}>
                Save Preferences
              </motion.span>)}
          </Button>
        </DialogFooter>
