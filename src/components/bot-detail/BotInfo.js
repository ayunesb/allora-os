...
const BotInfo = ({ bot }) => {
    return (<div className="flex items-start gap-4">
      ...
      <div className="overflow-hidden">
        ...
        <CardDescription className="flex items-center gap-1 mt-1 truncate" aria-labelledby={`bot-${bot.name.replace(/\s+/g, '-').toLowerCase()}`}>
          <Briefcase className="h-3.5 w-3.5 flex-shrink-0" aria-hidden="true"/>
          <span className="truncate">{bot.title}</span>
        </CardDescription>
        <div className="flex items-center gap-1 text-sm text-muted-foreground mt-1 truncate" aria-label={`${bot.name}'s expertise: ${bot.expertise}`}>
