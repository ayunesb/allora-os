    ...
    return (<div className="space-y-4 p-4 border rounded-md border-border/30 bg-muted/20">
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <Input id="github-webhook-url" placeholder="https://api.github.com/repos/username/repo/hooks/..." value={githubWebhook} onChange={handleInputChange} className={!isValid && githubWebhook ? "border-red-500" : ""}/>
          <Button type="button" variant="outline" onClick={testGitHubWebhook} disabled={isTestLoading || !githubWebhook || isValid !== true}>
            {isTestLoading ? (<>
      ...
      {!isValid && githubWebhook && (<Alert variant="destructive">
      ...
      <div className="flex flex-col sm:flex-row gap-2 justify-between items-center pt-2">
        <Button variant="ghost" size="sm" onClick={() => window.open("https://docs.github.com/en/developers/webhooks-and-events/webhooks", "_blank")}>
          GitHub Docs
        </Button>
        
        <div className="flex gap-2">
          <Button variant="default" size="sm" onClick={handleDirectGitHubConnect} disabled={isConnecting} className="gap-2">
            {isConnecting ? (<>
          ...
          <Button variant="outline" size="sm" onClick={() => onConfigureWebhook('github')}>
            Advanced Config
          </Button>
        </div>
      </div>
