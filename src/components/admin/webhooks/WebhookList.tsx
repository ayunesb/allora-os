...
export default function WebhookList({ webhooks, isLoading, onAddWebhook, onEditWebhook, onDeleteWebhook, onTestWebhook }) {
    if (isLoading) {
        ...
        {[1, 2, 3].map((i) => (<Skeleton key={i} className="h-[180px] w-full"/>))}
      </div>);
    }
    return (<div className="space-y-4">
      <div className="flex justify-between items-center">
      ...
      {webhooks.length === 0 ? (<div className="text-center py-12 border rounded-lg">
          ...
          {webhooks.map((webhook) => (<WebhookCard key={webhook.id} webhook={webhook} onEdit={onEditWebhook} onDelete={onDeleteWebhook} onTest={onTestWebhook}/>))}
        </div>)}
    </div>);
}
