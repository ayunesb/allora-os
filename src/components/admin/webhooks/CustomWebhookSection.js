...
const CustomWebhookSection = ({ customWebhook, onCustomWebhookChange, onTestWebhook, isTestLoading, webhookName = "Custom" }) => {
    ...
    return (<div className="space-y-4 p-4 border rounded-md border-border/30 bg-muted/20">
      ...
      <WebhookInput id="custom-webhook" label={`${webhookName} Webhook URL`} placeholder="https://api.example.com/webhooks/your-endpoint" value={customWebhook} onChange={handleCustomWebhookChange} isValid={isValid} errorMessage="Invalid webhook URL" validMessage="Valid webhook URL" validationMessage={validationMessage} description={`Enter your ${webhookName.toLowerCase()} webhook URL to integrate with external services.`}/>
      
      <div className="flex gap-2">
        <Button variant="outline" size="sm" onClick={onTestWebhook} disabled={isTestLoading || !customWebhook || isValid !== true}>
