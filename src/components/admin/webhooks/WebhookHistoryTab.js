...
const WebhookHistoryTab = ({ events, onRefresh, isLoading }) => {
    return (<Card>
      ...
      <CardContent>
        {isLoading ? (<div className="text-center py-8">Loading webhook events...</div>) : events.length === 0 ? (<div className="text-center py-8">
            ...
            {events.map((event) => (<div key={event.id} className="p-4 border rounded-lg">
                <div className="flex justify-between mb-2">
                  ...
                  <span className={`text-sm ${event.status === 'success' ? 'text-green-500' :
