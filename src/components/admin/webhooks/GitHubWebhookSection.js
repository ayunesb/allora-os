    ...
    return (<div className="space-y-4 p-4 border rounded-md border-border/30 bg-muted/20">
      <div className="flex items-center justify-between">
        ...
        {isConnected ? (<Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
            Connected
          </Badge>) : (<Badge variant="outline" className="bg-orange-50 text-orange-700 border-orange-200">
            Not Connected
          </Badge>)}
      </div>
      
      <WebhookInput id="github-webhook" label="GitHub Webhook URL" placeholder="https://api.github.com/repos/username/repo/hooks/..." value={githubWebhook} onChange={handleGithubWebhookChange} isValid={isValid} errorMessage="Invalid GitHub webhook URL" validMessage="Valid GitHub webhook URL" validationMessage={validationMessage} description="Enter your GitHub webhook URL to receive repository events like pushes, pull requests, etc."/>
      
      <div className="flex flex-wrap gap-2">
        <Button variant="outline" size="sm" onClick={onTestWebhook} disabled={isTestLoading || !githubWebhook || isValid !== true}>
          {isTestLoading ? "Testing..." : "Test Webhook"}
        </Button>
        <Button variant="outline" size="sm" onClick={() => window.open("https://docs.github.com/en/developers/webhooks-and-events/webhooks", "_blank")}>
          GitHub Docs
        </Button>
        <Button variant="default" size="sm" className="gap-2" onClick={handleDirectGitHubConnect}>
