...
export default function LaunchReadinessCheck() {
    ...
    return (<Card className="relative overflow-hidden border-primary/10">
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-xl md:text-2xl">Launch Readiness Check</CardTitle>
          <Button variant="secondary" size="sm" onClick={performReadinessCheck} disabled={isChecking} className="h-8 gap-1">
      ...
      <CardContent className="space-y-6">
        <Alert variant="default" className={`${overallStatusClasses} border`}>
          <div className="flex items-center gap-2">
            {readinessStatus.overallStatus === 'ready' ? (<ShieldCheck className="h-5 w-5"/>) : readinessStatus.overallStatus === 'warning' ? (<AlertTriangle className="h-5 w-5"/>) : (<X className="h-5 w-5"/>)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div className="space-y-2 bg-secondary/20 p-4 rounded-lg border border-border">
              ...
              {readinessStatus.database.message && (<div className="text-xs text-muted-foreground">{readinessStatus.database.message}</div>)}
            </div>
          </div>
      ...
      <CardFooter className="flex flex-col sm:flex-row justify-between gap-4 border-t pt-6">
        ...
        {readinessStatus.overallStatus === 'ready' && (<Button className="w-full sm:w-auto gap-2">
            <ExternalLink className="h-4 w-4"/>
            Launch to Production
          </Button>)}
        
        {readinessStatus.overallStatus === 'warning' && (<Button variant="secondary" className="w-full sm:w-auto gap-2">
            <AlertTriangle className="h-4 w-4"/>
            Launch with Warnings
          </Button>)}
        
        {readinessStatus.overallStatus === 'not_ready' && (<Button variant="secondary" className="w-full sm:w-auto gap-2" disabled>
            <X className="h-4 w-4"/>
