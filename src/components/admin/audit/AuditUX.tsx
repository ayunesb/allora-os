...
import { CheckCircle2, XCircle, AlertCircle, Loader2, Palette, Smartphone, Monitor, EyeOff } from 'lucide-react';
...
export function AuditUX({ status, onStatusChange }: AuditComponentProps) {
  ...
  const checkBrandConsistency = async () => {
    ...
    try {
      ...
      setItems(prev => prev.map(item => 
        item.id === 'ux-2' ? { ...item, status: brandingConsistent ? 'passed' : 'failed' } : item
      ));
  ...
  const checkResponsiveness = async () => {
    ...
    try {
      ...
      setItems(prev => prev.map(item => 
        item.id === 'ux-1' ? { ...item, status: isResponsive ? 'passed' : 'failed' } : item
      ));
  ...
  const checkEmptyStates = async () => {
    ...
    try {
      ...
      setItems(prev => prev.map(item => 
        item.id === 'ux-3' ? { ...item, status: hasEmptyStates ? 'passed' : 'pending' } : item
      ));
