...
import { InfoIcon, AlertTriangle, CheckCircle2, Database, Shield, FileText } from 'lucide-react';
...
export default function AuditDashboard() {
  ...
  const warningIssues = auditResults?.issues?.filter((issue: any) => issue.severity === 'warning') || [];
            ...
            <div className="space-y-6">
              {criticalIssues.length > 0 && (
                <div className="space-y-4">
                  ...
                  {criticalIssues.map((issue: any, idx: number) => (
                    <div key={idx} className="p-4 rounded-md bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800">
                      <div className="flex items-center gap-2">
              ...
              {warningIssues.length > 0 && (
                <div className="space-y-4">
                  ...
                  {warningIssues.map((issue: any, idx: number) => (
                    <div key={idx} className="p-4 rounded-md bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
                      <div className="flex items-center gap-2">
                        <AlertTriangle className="h-5 w-5 text-amber-500" />
                        <div className="font-medium">{issue.name}</div>
                        <div className="ml-auto text-xs px-2 py-1 rounded-full uppercase font-bold tracking-wider bg-black/10 dark:bg-white/10">
