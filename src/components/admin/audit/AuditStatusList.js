...
export function AuditStatusList({ items }) {
    ...
    return (<div className="grid grid-cols-1 gap-2 mb-6">
      {items.map((item) => (<div key={item.id} className="flex justify-between items-center p-3 border rounded-md">
          <div className="flex items-center gap-2">
            {getStatusIcon(item.status)}
            <span className="font-medium">{item.label}</span>
          </div>
          <div className="text-sm text-muted-foreground">
            {item.status === 'passed' ? item.passedMessage :
                item.status === 'failed' ? item.failedMessage :
                    item.status === 'in-progress' ? 'Checking...' : item.pendingMessage}
          </div>
        </div>))}
    </div>);
}
