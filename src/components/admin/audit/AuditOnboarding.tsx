    ...
    return (<Card>
      <CardHeader className="pb-2">
        <div className="flex justify-between items-center">
          ...
          <Button onClick={runTest} disabled={isRunning || isLiveChecking} size="sm">
        ...
        <div className="space-y-4">
          {items.map((item) => (<div key={item.id} className="flex items-start space-x-2">
              ...
              <div className="ml-auto flex items-center">
                <Checkbox id={item.id} checked={item.status === 'passed'} disabled={isRunning || isLiveChecking || isProductionMode} onCheckedChange={(checked) => {
                // Only allow manual changes in development mode
                if (isProductionMode)
                    return;
                setItems(prev => prev.map(i => i.id === item.id ? { ...i, status: checked ? 'passed' : 'pending' } : i));
