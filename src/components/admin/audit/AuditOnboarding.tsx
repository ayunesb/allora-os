...
export function AuditOnboarding({ status, onStatusChange }) {
  ...
  const checkForRealData = async () => {
    ...
    try {
      ...
      if (!validationResults.success) {
        ...
        if (isProductionMode) {
          toast.error(
            `Data validation failed with ${validationResults.errors.length} errors`,
          );
          ...
          if (hasCompanyErrors) {
            const companyItem = updatedItems.find(
              (item) => item.id === "onb-3",
            );
            if (companyItem) {
      ...
      if (companies && companies.length > 0) {
        ...
        if (hasValidCompanyData && hasValidProfiles) {
          ...
          setItems((prev) =>
            prev.map((item) => ({ ...item, status: "passed" })),
          );
          onStatusChange("passed");
  ...
  const checkProfiles = async () => {
    try {
      ...
      if (profiles && profiles.length > 0) {
        ...
        const hasCompleteProfiles = profiles.some(
          (profile) =>
  ...
  const runTest = async () => {
    ...
    try {
      ...
      setItems((prev) =>
        prev.map((item) => ({ ...item, status: "in-progress" })),
      );
      ...
      if (isProductionMode) {
        ...
        const isPassing =
          hasRealCompanies && hasRealProfiles && validationResults.success;
                ...
                <Checkbox
                  ...
                  onCheckedChange={(checked) => {
                    ...
                    setItems((prev) =>
                      prev.map((i) =>
                        i.id === item.id
                          ? { ...i, status: checked ? "passed" : "pending" }
                          : i,
                      ),
                    );
