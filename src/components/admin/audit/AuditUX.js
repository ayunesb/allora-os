...
export function AuditUX({ status, onStatusChange }) {
    ...
    const checkBrandConsistency = async () => {
        ...
        setItems(prev => prev.map(item => item.id === 'ux-2' ? { ...item, status: 'in-progress' } : item));
        try {
            // Get brand elements from the page
            const logoElements = document.querySelectorAll('img[alt*="Allora"], img[alt*="allora"], img[alt*="logo"]');
            const brandColors = {
                primary: getComputedStyle(document.documentElement).getPropertyValue('--primary').trim() || '#7E69AB',
                brand: getComputedStyle(document.documentElement).getPropertyValue('--brand').trim() || '#9b87f5'
            };
            ...
            setItems(prev => prev.map(item => item.id === 'ux-2' ? { ...item, status: brandingConsistent ? 'passed' : 'failed' } : item));
            return brandingConsistent;
        }
        catch (error) {
            console.error('Error checking brand consistency:', error);
            setItems(prev => prev.map(item => item.id === 'ux-2' ? { ...item, status: 'failed' } : item));
            return false;
        }
    };
    const checkResponsiveness = async () => {
        // Set the responsive design check to in-progress
        setItems(prev => prev.map(item => item.id === 'ux-1' ? { ...item, status: 'in-progress' } : item));
        try {
            ...
            setItems(prev => prev.map(item => item.id === 'ux-1' ? { ...item, status: isResponsive ? 'passed' : 'failed' } : item));
            return isResponsive;
        }
        catch (error) {
            console.error('Error checking responsiveness:', error);
            setItems(prev => prev.map(item => item.id === 'ux-1' ? { ...item, status: 'failed' } : item));
            return false;
        }
    };
    const checkEmptyStates = async () => {
        // Set the empty states check to in-progress
        setItems(prev => prev.map(item => item.id === 'ux-3' ? { ...item, status: 'in-progress' } : item));
        try {
            ...
            const emptyTextElements = Array.from(document.querySelectorAll('p, div, span')).filter(el => el.textContent?.includes('No data') ||
            ...
            setItems(prev => prev.map(item => item.id === 'ux-3' ? { ...item, status: hasEmptyStates ? 'passed' : 'pending' } : item));
    ...
    const simulateOtherChecks = async () => {
        ...
        setItems(prev => prev.map(item => otherIds.includes(item.id) ? { ...item, status: 'in-progress' } : item));
        await new Promise(resolve => setTimeout(resolve, 500));
        // For demo purposes, we'll mark these as passed
        // In a real audit, we'd check for error boundaries, loading states, etc.
        setItems(prev => prev.map(item => otherIds.includes(item.id) ? { ...item, status: 'passed' } : item));
        return true;
    };
    const runTest = async () => {
        setIsRunning(true);
        ...
        <div className="space-y-4">
          {items.map((item) => (<div key={item.id} className="flex items-start space-x-2">
              ...
              <div className="space-y-1">
                ...
                {item.id === 'ux-1' && (<div className="flex mt-1 gap-2">
                    <Button variant="outline" size="sm" className="h-7 text-xs flex items-center gap-1" onClick={() => {
