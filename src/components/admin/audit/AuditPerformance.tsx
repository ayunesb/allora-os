...
export function AuditPerformance({ status, onStatusChange }) {
    ...
    const checkImageOptimization = async () => {
        ...
        setItems(prev => prev.map(item => item.id === 'perf-2' ? { ...item, status: 'in-progress' } : item));
        try {
            ...
            setItems(prev => prev.map(item => item.id === 'perf-2' ? { ...item, status: allOptimized ? 'passed' : 'failed' } : item));
            return allOptimized;
        }
        catch (error) {
            console.error('Error checking image optimization:', error);
            setItems(prev => prev.map(item => item.id === 'perf-2' ? { ...item, status: 'failed' } : item));
            return false;
        }
    };
    const runSimulatedTests = async () => {
        ...
        for (const id of idsToTest) {
            setItems(prev => prev.map(item => item.id === id ? { ...item, status: 'in-progress' } : item));
            await new Promise(resolve => setTimeout(resolve, 500));
            // For demo purposes, we'll mark these as passed
            setItems(prev => prev.map(item => item.id === id ? { ...item, status: 'passed' } : item));
        }
        return true;
    };
    const runTest = async () => {
    ...
    return (<div className="space-y-4">
      <Card>
        ...
        <CardContent>
          {pageLoadTime !== null && (<div className="mb-4 space-y-1.5">
              <div className="flex justify-between items-center">
                <div className="text-sm font-medium">Page Load Time</div>
                <div className="text-sm font-medium">{pageLoadTime.toFixed(2)}s</div>
              </div>
              <Progress value={Math.min(100, (2 - pageLoadTime) * 50)} className={pageLoadTime < 2 ? "bg-green-100" : "bg-red-100"}/>
