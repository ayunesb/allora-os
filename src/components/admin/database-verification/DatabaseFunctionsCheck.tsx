...
export function DatabaseFunctionsCheck({ functions }) {
    ...
    return (<div className="rounded-md border border-border/60 overflow-hidden">
      <div className="bg-muted/30 px-4 py-3 font-medium border-b border-border/60 flex justify-between items-center">
        ...
        {functionIssues > 0 ? (<span className="text-xs px-2 py-1 bg-red-100 text-red-800 rounded-full">
      ...
      <div className="divide-y divide-border/60">
        {functions.map((func) => (<div key={func.name} className="px-4 py-3">
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center gap-2">
                {func.exists ? (<CheckCircle2 className="h-5 w-5 text-green-500"/>) : (<XCircle className="h-5 w-5 text-red-500"/>)}
            ...
            {func.exists && (<div className="flex items-center pl-7 mt-1">
                <div className="flex items-center gap-2">
                  {func.isSecure ? (<CheckCircle2 className="h-4 w-4 text-green-500"/>) : (<AlertTriangle className="h-4 w-4 text-amber-500"/>)}
