"use strict";
var __awaiter =
  (this && this.__awaiter) ||
  function (thisArg, _arguments, P, generator) {
    function adopt(value) {
      return value instanceof P
        ? value
        : new P(function (resolve) {
            resolve(value);
          });
    }
    return new (P || (P = Promise))(function (resolve, reject) {
      function fulfilled(value) {
        try {
          step(generator.next(value));
        } catch (e) {
          reject(e);
        }
      }
      function rejected(value) {
        try {
          step(generator["throw"](value));
        } catch (e) {
          reject(e);
        }
      }
      function step(result) {
        result.done
          ? resolve(result.value)
          : adopt(result.value).then(fulfilled, rejected);
      }
      step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
  };
var __generator =
  (this && this.__generator) ||
  function (thisArg, body) {
    var _ = {
        label: 0,
        sent: function () {
          if (t[0] & 1) throw t[1];
          return t[1];
        },
        trys: [],
        ops: [],
      },
      f,
      y,
      t,
      g = Object.create(
        (typeof Iterator === "function" ? Iterator : Object).prototype,
      );
    return (
      (g.next = verb(0)),
      (g["throw"] = verb(1)),
      (g["return"] = verb(2)),
      typeof Symbol === "function" &&
        (g[Symbol.iterator] = function () {
          return this;
        }),
      g
    );
    function verb(n) {
      return function (v) {
        return step([n, v]);
      };
    }
    function step(op) {
      if (f) throw new TypeError("Generator is already executing.");
      while ((g && ((g = 0), op[0] && (_ = 0)), _))
        try {
          if (
            ((f = 1),
            y &&
              (t =
                op[0] & 2
                  ? y["return"]
                  : op[0]
                    ? y["throw"] || ((t = y["return"]) && t.call(y), 0)
                    : y.next) &&
              !(t = t.call(y, op[1])).done)
          )
            return t;
          if (((y = 0), t)) op = [op[0] & 2, t.value];
          switch (op[0]) {
            case 0:
            case 1:
              t = op;
              break;
            case 4:
              _.label++;
              return { value: op[1], done: false };
            case 5:
              _.label++;
              y = op[1];
              op = [0];
              continue;
            case 7:
              op = _.ops.pop();
              _.trys.pop();
              continue;
            default:
              if (
                !((t = _.trys), (t = t.length > 0 && t[t.length - 1])) &&
                (op[0] === 6 || op[0] === 2)
              ) {
                _ = 0;
                continue;
              }
              if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) {
                _.label = op[1];
                break;
              }
              if (op[0] === 6 && _.label < t[1]) {
                _.label = t[1];
                t = op;
                break;
              }
              if (t && _.label < t[2]) {
                _.label = t[2];
                _.ops.push(op);
                break;
              }
              if (t[2]) _.ops.pop();
              _.trys.pop();
              continue;
          }
          op = body.call(thisArg, _);
        } catch (e) {
          op = [6, e];
          y = 0;
        } finally {
          f = t = 0;
        }
      if (op[0] & 5) throw op[1];
      return { value: op[0] ? op[1] : void 0, done: true };
    }
  };
Object.defineProperty(exports, "__esModule", { value: true });
var jsx_runtime_1 = require("react/jsx-runtime");
var react_1 = require("react");
var supabase_1 = require("@/backend/supabase");
var lucide_react_1 = require("lucide-react");
var AdminSettingsProvider = function (_a) {
  var children = _a.children;
  var _b = (0, react_1.useState)(null),
    companyId = _b[0],
    setCompanyId = _b[1];
  var _c = (0, react_1.useState)(true),
    isLoading = _c[0],
    setIsLoading = _c[1];
  var _d = (0, react_1.useState)({
      stripe: "",
      twilio_sid: "",
      twilio_token: "",
      heygen: "",
    }),
    apiKeys = _d[0],
    setApiKeys = _d[1];
  var _e = (0, react_1.useState)({
      twoFactorEnabled: false,
      extendedSessionTimeout: false,
      strictContentSecurity: false,
      enhancedApiProtection: false,
    }),
    securitySettings = _e[0],
    setSecuritySettings = _e[1];
  // Fetch the settings data
  (0, react_1.useEffect)(function () {
    var fetchData = function () {
      return __awaiter(void 0, void 0, void 0, function () {
        var session,
          profileData,
          currentCompanyId,
          companyData,
          details,
          companies,
          details,
          _a,
          securityData,
          securityError,
          error_1;
        return __generator(this, function (_b) {
          switch (_b.label) {
            case 0:
              _b.trys.push([0, 8, 9, 10]);
              setIsLoading(true);
              return [4 /*yield*/, supabase_1.supabase.auth.getSession()];
            case 1:
              session = _b.sent().data.session;
              if (!session) return [3 /*break*/, 6];
              return [
                4 /*yield*/,
                supabase_1.supabase
                  .from("profiles")
                  .select("company_id")
                  .eq("id", session.user.id)
                  .single(),
              ];
            case 2:
              profileData = _b.sent().data;
              currentCompanyId =
                profileData === null || profileData === void 0
                  ? void 0
                  : profileData.company_id;
              setCompanyId(currentCompanyId);
              if (!currentCompanyId) return [3 /*break*/, 4];
              return [
                4 /*yield*/,
                supabase_1.supabase
                  .from("companies")
                  .select("details")
                  .eq("id", currentCompanyId)
                  .single(),
              ];
            case 3:
              companyData = _b.sent().data;
              if (
                companyData === null || companyData === void 0
                  ? void 0
                  : companyData.details
              ) {
                details =
                  typeof companyData.details === "string"
                    ? JSON.parse(companyData.details)
                    : companyData.details;
                // Set API keys if they exist
                if (details.api_keys) {
                  setApiKeys({
                    stripe: details.api_keys.stripe || "",
                    twilio_sid: details.api_keys.twilio_sid || "",
                    twilio_token: details.api_keys.twilio_token || "",
                    heygen: details.api_keys.heygen || "",
                  });
                }
              }
              return [3 /*break*/, 6];
            case 4:
              return [
                4 /*yield*/,
                supabase_1.supabase
                  .from("companies")
                  .select("id, details")
                  .limit(1),
              ];
            case 5:
              companies = _b.sent().data;
              if (companies && companies.length > 0) {
                setCompanyId(companies[0].id);
                // Load existing API keys if they exist
                if (companies[0].details) {
                  details =
                    typeof companies[0].details === "string"
                      ? JSON.parse(companies[0].details)
                      : companies[0].details;
                  if (details.api_keys) {
                    setApiKeys({
                      stripe: details.api_keys.stripe || "",
                      twilio_sid: details.api_keys.twilio_sid || "",
                      twilio_token: details.api_keys.twilio_token || "",
                      heygen: details.api_keys.heygen || "",
                    });
                  }
                }
              }
              _b.label = 6;
            case 6:
              return [
                4 /*yield*/,
                supabase_1.supabase.rpc("get_security_settings"),
              ];
            case 7:
              (_a = _b.sent()),
                (securityData = _a.data),
                (securityError = _a.error);
              if (securityError) {
                console.error(
                  "Error fetching security settings:",
                  securityError,
                );
              } else {
                setSecuritySettings(
                  securityData || {
                    twoFactorEnabled: false,
                    extendedSessionTimeout: false,
                    strictContentSecurity: false,
                    enhancedApiProtection: false,
                  },
                );
              }
              return [3 /*break*/, 10];
            case 8:
              error_1 = _b.sent();
              console.error("Error fetching settings data:", error_1);
              return [3 /*break*/, 10];
            case 9:
              setIsLoading(false);
              return [7 /*endfinally*/];
            case 10:
              return [2 /*return*/];
          }
        });
      });
    };
    fetchData();
  }, []);
  if (isLoading) {
    return (0, jsx_runtime_1.jsx)("div", {
      className: "min-h-screen flex items-center justify-center bg-background",
      children: (0, jsx_runtime_1.jsx)(lucide_react_1.Loader2, {
        className: "h-8 w-8 animate-spin text-primary",
      }),
    });
  }
  return (0, jsx_runtime_1.jsx)(jsx_runtime_1.Fragment, {
    children: children({
      companyId: companyId,
      isLoading: isLoading,
      apiKeys: apiKeys,
      securitySettings: securitySettings,
    }),
  });
};
exports.default = AdminSettingsProvider;
