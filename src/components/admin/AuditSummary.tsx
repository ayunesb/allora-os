    ...
    return (<Card className="w-full">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-6">
          {auditData.map((category, idx) => (<div key={idx} className="space-y-2">
              <h3 className="text-lg font-medium">{category.name}</h3>
              <div className="rounded-md border bg-card">
                {category.items.map((item, itemIdx) => (<div key={itemIdx} className={`flex justify-between p-3 ${itemIdx !== category.items.length - 1 ? "border-b" : ""}`}>
                    <div className="space-y-1">
                      ...
                      {item.recommendation && (<p className="text-xs text-primary">
