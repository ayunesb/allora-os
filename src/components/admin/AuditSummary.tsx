    ...
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="flex justify-between items-center">
          ...
          <Badge
            className={`text-sm ${passPercentage >= 80 ? "bg-green-500/10 text-green-500" : passPercentage >= 60 ? "bg-yellow-500/10 text-yellow-500" : "bg-red-500/10 text-red-500"}`}
          >
            ...
            <div key={idx} className="space-y-2">
              ...
              <div className="rounded-md border bg-card">
                {category.items.map((item, itemIdx) => (
                  <div
                    key={itemIdx}
                    className={`flex justify-between p-3 ${itemIdx !== category.items.length - 1 ? "border-b" : ""}`}
                  >
