...
export function EnhancedVerificationChecklist() {
    ...
    useEffect(() => {
        ...
        const completed = defaultCategories.reduce((sum, category) => sum + category.items.filter(item => item.status === 'completed').length, 0);
        setTotalItems(total);
    ...
    const handleVerify = async () => {
        ...
        try {
            ...
            const completed = categories.reduce((sum, category) => sum + category.items.filter(item => item.status === 'completed').length, 0);
            setCompletedItems(completed);
        }
        catch (error) {
    ...
    return (<Card>
      <CardHeader className="pb-3">
        <div className="flex justify-between items-center">
          ...
          <Button variant="outline" size="sm" onClick={handleVerify} disabled={isLoading} className="gap-2">
            {isLoading ? (<>
      ...
      <CardContent>
        ...
        <div className="mt-6 space-y-6">
          {categories.map(category => (<ChecklistCategory key={category.id} category={category}/>))}
        </div>
      </CardContent>
    </Card>);
}
