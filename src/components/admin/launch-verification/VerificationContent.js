...
export function VerificationContent({ results, isChecking }) {
    ...
    const coreValidationKeys = Object.keys(results).filter(key => !['databaseTables', 'databaseIndexes', 'rlsPolicies', 'databaseFunctions', 'overallStatus'].includes(key));
    ...
    return (<div className="space-y-3">
      ...
      {results.databaseIndexes && Array.isArray(results.databaseIndexes) && (<DatabaseChecksSection title="Database Indexes Check" items={results.databaseIndexes}/>)}
      
      {results.rlsPolicies && Array.isArray(results.rlsPolicies) && (<DatabaseChecksSection title="RLS Policies Check" items={results.rlsPolicies}/>)}
      
      {results.databaseFunctions && Array.isArray(results.databaseFunctions) && (<DatabaseChecksSection title="Database Functions Check" items={results.databaseFunctions}/>)}
    </div>);
}
