...
export function ChecklistCategory({ category }) {
    ...
    return (<div className="border rounded-lg overflow-hidden">
      <div className="flex justify-between items-center p-4 bg-secondary/40 cursor-pointer" onClick={toggleExpanded}>
        <div>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 text-sm">
            ...
            {warningCount > 0 && (<span className="ml-2 px-1.5 py-0.5 bg-yellow-100 text-yellow-800 rounded text-xs">
                {warningCount} warning{warningCount !== 1 ? 's' : ''}
              </span>)}
            
            {errorCount > 0 && (<span className="ml-2 px-1.5 py-0.5 bg-red-100 text-red-800 rounded text-xs">
      ...
      {isExpanded && (<div className="divide-y">
          {category.items.map(item => (<ChecklistItem key={item.id} item={item}/>))}
        </div>)}
    </div>);
}
