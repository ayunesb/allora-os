...
export default function APIKeysTab({ companyId, initialApiKeys, isLoading }) {
    ...
    return (<Card>
      ...
      <CardContent className="space-y-6">
        <APIKeyInput id="stripe-key" label="Stripe API Key" placeholder="sk_test_..." value={stripeKey} onChange={setStripeKey}/>
        
        <APIKeyInput id="twilio-sid" label="Twilio Account SID" placeholder="AC..." value={twilioSid} onChange={setTwilioSid} isSecret={false}/>
        
        <APIKeyInput id="twilio-token" label="Twilio Auth Token" value={twilioToken} onChange={setTwilioToken}/>
        
        <APIKeyInput id="heygen-key" label="Heygen API Key" value={heygenKey} onChange={setHeygenKey}/>
        
        <Button onClick={handleSaveApiConfiguration} disabled={isSaving || !companyId}>
          {isSaving ? (<>
