...
export function ProductionDataStatus({ validationResults, isValidating, onValidate }) {
    ...
    return (<Card className={validationResults.success ?
            "border-green-200 bg-green-50/50 dark:bg-green-900/10" :
            "border-amber-200 bg-amber-50/50 dark:bg-amber-900/10"}>
      <CardHeader className="pb-2">
        <CardTitle className="text-lg flex items-center justify-between">
          <div className="flex items-center">
            {validationResults.success ? (<CheckCircle2 className="h-4 w-4 mr-2 text-green-500"/>) : (<AlertCircle className="h-4 w-4 mr-2 text-amber-500"/>)}
            Production Data Status
          </div>
          <Badge variant={validationResults.success ? "outline" : "outline"} className={validationResults.success ?
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          {(validationResults.errors.length > 0 || validationResults.warnings.length >
            0) && (<Accordion type="single" collapsible className="w-full">
              {validationResults.errors.length > 0 && (<AccordionItem value="errors">
                  ...
                  <AccordionContent>
                    <ul className="text-xs space-y-1 mt-2 max-h-40 overflow-y-auto">
                      {validationResults.errors.map((error, index) => (<li key={index} className="text-red-600 bg-red-50 p-1 rounded flex items-start">
              ...
              {validationResults.warnings.length > 0 && (<AccordionItem value="warnings">
                  <AccordionTrigger className="text-sm text-amber-600">
                    {validationResults.warnings.length} Warnings
                  </AccordionTrigger>
                  <AccordionContent>
                    <ul className="text-xs space-y-1 mt-2 max-h-40 overflow-y-auto">
                      {validationResults.warnings.map((warning, index) => (<li key={index} className="text-amber-600 bg-amber-50 p-1 rounded flex items-start">
