      ...
      <DialogContent className={`${isMobileView ? "w-[calc(100%-32px)] p-4" : "sm:max-w-md"} bg-[#1A1F2C] text-white border-white/10`}>
        ...
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            ...
            <Input id="email" placeholder="user@example.com" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-[#0F1729] border-white/10 text-white"/>
          </div>
          <div className="space-y-2">
            <Label htmlFor="company" className="text-gray-300">Company</Label>
            <Select value={company} onValueChange={setCompany} disabled={loadingCompanies}>
              ...
              <SelectContent className="bg-[#1A1F2C] border-white/10 text-white">
                ...
                {companies.length === 0 && !loadingCompanies && (<SelectItem value="no-companies" disabled>No companies available</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
        ...
        <DialogFooter className={isMobileView ? "flex-col space-y-2" : ""}>
          <Button type="submit" onClick={handleInviteUser} disabled={isSubmitting || !email || !company} className={`${isMobileView ? "w-full" : "w-full sm:w-auto"} bg-[#5A67D8] hover:bg-[#4C5BC7] text-white`}>
            {isSubmitting ? (<>
