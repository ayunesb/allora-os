...
export const UserTable = ({ users, isLoading = false, onUpdateUser, onDeleteUser }) => {
    ...
    if (isLoading) {
        return (<div className="text-center py-8 border border-white/10 rounded-md text-gray-400 bg-[#111827]">
        Loading users...
      </div>);
    }
    if (users.length === 0) {
    ...
    if (isMobileView) {
        ...
        {users.map((user) => (<div key={user.id} className="border border-white/10 rounded-md p-4 bg-[#111827] shadow-sm">
            ...
            <div className="flex gap-2 mt-2">
              ...
              <Button variant="outline" size="sm" className="flex-1 h-8 px-2 text-red-500 border-white/10 bg-[#1E293B] hover:bg-[#2D3A4F]" onClick={() => onDeleteUser(user.id, user.name || user.email || '')}>
    ...
    return (<div className="rounded-md border border-white/10 overflow-x-auto bg-[#111827]">
      <Table>
        ...
        <TableBody>
          {users.map((user) => (<TableRow key={user.id} className="border-white/10 hover:bg-[#1E293B]">
              ...
              <TableCell>
                ...
                toast.success(`User role updated to ${newRole}`);
                  ...
                  <Button variant="outline" size="sm" className="h-8 px-2 flex items-center gap-1 text-red-500 border-white/10 bg-[#1E293B] hover:bg-[#2D3A4F]" onClick={() => onDeleteUser(user.id, user.name || user.email || '')}>
