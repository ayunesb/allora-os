  ...
  return (
    ...
    >
      <div className="flex justify-between items-start">
        <div
          className={`p-2 rounded-full w-fit ${highlight ? "bg-primary/20" : "bg-secondary"}`}
        >
          {icon}
        </div>
        {count && (
          <span className="text-sm font-semibold bg-secondary/80 px-2 py-1 rounded">
            {count}
          </span>
        )}
      </div>
      <h3
        className={`text-lg font-medium mt-4 ${highlight ? "text-primary" : ""}`}
      >
        {title}
      </h3>
