...
export function CrmIntegrationsForm({ companyDetails, updateCompanyDetails }) {
  ...
  const [selectedDocTypes, setSelectedDocTypes] = useState(
    companyDetails.documentGenerationTypes || [],
  );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Connect your CRM, marketing platforms, and enable document generation
                ...
                companyDetails.crmSystem !== "none" && (
                  <div className="mt-4">
                    <Button variant="outline" size="sm" className="text-xs">
                      ...
                      {
                        crmSystems.find(
                          (c) => c.value === companyDetails.crmSystem,
                        )?.label
                      }
                    </Button>
                  </div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="marketing-platform">
                  Current Marketing Platform
                </Label>
                <Select
                  value={companyDetails.marketingPlatform || ""}
                  onValueChange={(value) =>
                    handleChange("marketingPlatform", value)
                  }
                >
                    ...
                    <Button variant="outline" size="sm" className="text-xs">
                      ...
                      Connect{" "}
                      {
                        marketingPlatforms.find(
                          (m) => m.value === companyDetails.marketingPlatform,
                        )?.label
                      }
                    </Button>
                  </div>
      ...
      {showApiKeyField && (
        <div className="space-y-2 pt-2 pl-4 border-l-2 border-muted">
          ...
          <Input
            ...
            onChange={(e) => handleChange("integrationApiKey", e.target.value)}
          />
          <p className="text-xs text-muted-foreground mt-1">
            This key will be securely stored and used for integration purposes
            only.
          </p>
        </div>
      )}
      ...
      <div className="space-y-4">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Select which types of documents you'd like Allora AI to help you
          generate.
        </p>
        ...
        <div className="flex flex-wrap gap-2 pt-2">
          {documentTypes.map((docType) => (
            <Badge
              key={docType.value}
              variant={
                selectedDocTypes.includes(docType.value) ? "default" : "outline"
              }
