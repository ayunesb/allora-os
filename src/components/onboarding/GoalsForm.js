      ...
      <div className="space-y-4">
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {businessGoals.map((item) => (<div key={item.id} className="flex items-center space-x-2">
                <Checkbox id={item.id} checked={goals.includes(item.id)} onCheckedChange={() => toggleGoal(item.id)}/>
                <label htmlFor={item.id} className="text-sm leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                  {item.label}
                </label>
              </div>))}
          </div>

          {error && (<Alert variant="destructive" className="mt-4 py-2">
        ...
        <div className="space-y-2 pt-2">
          ...
          <Select value={companyDetails?.timeHorizon || ""} onValueChange={handleTimeHorizonChange}>
            <SelectTrigger id="time-horizon">
              <SelectValue placeholder="Select time horizon"/>
            </SelectTrigger>
            <SelectContent>
              {timeHorizons.map((horizon) => (<SelectItem key={horizon.value} value={horizon.value}>
                  {horizon.label}
                </SelectItem>))}
            </SelectContent>
          </Select>
