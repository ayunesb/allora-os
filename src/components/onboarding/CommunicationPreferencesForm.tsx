    ...
    return (<div className="space-y-6">
      ...
      <div className="space-y-4">
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3" role={screenReaderFriendly ? "group" : undefined} aria-label={screenReaderFriendly ? "Communication channel options" : undefined}>
            {communicationChannels.map((channel) => (<div key={channel.id} className="flex items-center space-x-2">
                <Checkbox id={channel.id} checked={!!companyDetails[channel.key]} onCheckedChange={() => toggleChannel(channel.key)} aria-label={`Enable ${channel.label}`}/>
                <label htmlFor={channel.id} className={`text-sm leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 ${highContrast ? 'text-foreground' : ''}`}>
                  {channel.label}
                </label>
              </div>))}
          </div>
        ...
        <div className="space-y-2 pt-2">
          ...
          <Select value={companyDetails.salesStylePreference || ""} onValueChange={handleSalesStyleChange} aria-label="Select your sales style preference">
            <SelectTrigger id="sales-style">
              <SelectValue placeholder="Select sales style"/>
            </SelectTrigger>
            <SelectContent>
              {salesStyles.map((style) => (<SelectItem key={style.value} value={style.value}>
                  {style.label}
                </SelectItem>))}
            </SelectContent>
          </Select>
          
          {!companyDetails.salesStylePreference && (<p className="text-xs text-amber-500 mt-1">
