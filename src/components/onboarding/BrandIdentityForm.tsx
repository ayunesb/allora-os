      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-4">
            <div className="w-24 h-24 border-2 border-dashed border-muted rounded-md flex items-center justify-center overflow-hidden bg-muted/10">
              {previewLogo ? (<img src={previewLogo} alt="Logo preview" className="max-w-full max-h-full object-contain"/>) : (<ImageIcon className="h-8 w-8 text-muted-foreground"/>)}
            </div>
            <div>
              <Input id="logo-upload" type="file" accept="image/*" className="hidden" onChange={handleLogoUpload}/>
              <div className="flex space-x-2">
                <Button variant="outline" onClick={() => document.getElementById("logo-upload")?.click()} type="button" size="sm">
                  <Upload className="h-4 w-4 mr-2"/>
                  Upload Logo
                </Button>
                {previewLogo && (<Button variant="outline" onClick={handleClearLogo} type="button" size="sm" className="text-destructive hover:text-destructive">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Input id="primary-color" type="color" value={companyDetails.primaryColor || "#000000"} onChange={(e) => handleColorChange("primaryColor", e.target.value)} className="w-12 h-10 p-1"/>
              <Input value={companyDetails.primaryColor || "#000000"} onChange={(e) => handleColorChange("primaryColor", e.target.value)} placeholder="#000000" className="flex-1"/>
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Input id="secondary-color" type="color" value={companyDetails.secondaryColor || "#ffffff"} onChange={(e) => handleColorChange("secondaryColor", e.target.value)} className="w-12 h-10 p-1"/>
              <Input value={companyDetails.secondaryColor || "#ffffff"} onChange={(e) => handleColorChange("secondaryColor", e.target.value)} placeholder="#ffffff" className="flex-1"/>
            </div>
            <p className="text-xs text-muted-foreground">
        ...
        <div className="space-y-2 pt-2">
          ...
          <Select value={companyDetails.brandTone || ""} onValueChange={(value) => updateCompanyDetails({ ...companyDetails, brandTone: value })}>
            <SelectTrigger id="brand-tone">
              <SelectValue placeholder="Select brand tone"/>
            </SelectTrigger>
            <SelectContent>
              {brandTones.map((tone) => (<SelectItem key={tone.value} value={tone.value}>
                  {tone.label}
                </SelectItem>))}
            </SelectContent>
          </Select>
        ...
        <div className="space-y-2">
          ...
          <Card className="overflow-hidden">
            <div className="h-2" style={{ backgroundColor: companyDetails.primaryColor || "#4F46E5" }}></div>
            <CardContent className="p-4">
              <div className="flex items-center space-x-2 mb-2">
                {previewLogo ? (<img src={previewLogo} alt="Logo" className="h-8 w-8 object-contain"/>) : (<div className="h-8 w-8 rounded flex items-center justify-center" style={{ backgroundColor: companyDetails.secondaryColor || "#10B981" }}>
