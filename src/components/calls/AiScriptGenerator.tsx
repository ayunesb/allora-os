      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <Button type="button" variant={scriptType === 'call' ? 'default' : 'outline'} className="flex-1 flex items-center justify-center gap-2" onClick={() => setScriptType('call')}>
              <Phone className="h-4 w-4"/>
              Call Script
            </Button>
            <Button type="button" variant={scriptType === 'whatsapp' ? 'default' : 'outline'} className="flex-1 flex items-center justify-center gap-2" onClick={() => setScriptType('whatsapp')}>
        ...
        <div className="space-y-2">
          <label htmlFor="script-purpose" className="text-sm font-medium">Script Purpose</label>
        ...
        <div className="space-y-2">
          <label htmlFor="lead-info" className="text-sm font-medium">Lead Information</label>
          <Textarea id="lead-info" placeholder="Enter lead details including company name, industry, pain points, etc." value={leadInfo} onChange={(e) => setLeadInfo(e.target.value)} className="min-h-[100px]"/>
        </div>
        
        <Button onClick={generateScript} className="w-full" disabled={isGenerating || !leadInfo.trim()}>
          {isGenerating ? (<>
        ...
        {generatedScript && (<div className="space-y-3">
            <div className="p-3 border rounded-md bg-muted/30">
              <pre className="whitespace-pre-wrap text-sm">{generatedScript}</pre>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" className="flex-1" onClick={regenerateScript}>
