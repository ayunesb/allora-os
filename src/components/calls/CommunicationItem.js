...
export default function CommunicationItem({ communication, isUpcoming }) {
    ...
    return (<div className={`border rounded-lg p-3 ${communication.status === "scheduled" ? "border-blue-200 bg-blue-50/30" :
      ...
      <Collapsible open={isOpen} onOpenChange={setIsOpen}>
        <div className="flex items-start justify-between">
          <div className="flex items-start space-x-3">
            <div className={`p-2 rounded-full ${communication.type === "phone" ? "bg-blue-100 text-blue-700" :
        ...
        <CollapsibleContent className="pt-3 mt-3 border-t">
          {isUpcoming && communication.status === "scheduled" && (<div className="flex items-center justify-between mb-4">
              <div className="space-x-2">
                {communication.meeting_link && (<Button size="sm" variant="outline" asChild>
                ...
                {communication.type === "whatsapp" && communication.leads?.phone && (<Button size="sm" variant="outline" asChild>
                    <a href={`https://wa.me/${communication.leads.phone.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer">
          ...
          {!isUpcoming && (<CommunicationStatusSelector communicationId={communication.id} currentStatus={communication.status} currentOutcome={communication.outcome}/>)}
          
          <div className="mt-4">
            <div className="flex items-center justify-between mb-2">
              <h5 className="text-sm font-medium">Notes</h5>
              {!isEditingNotes ? (<Button variant="ghost" size="sm" onClick={() => setIsEditingNotes(true)}>
            ...
            {!isEditingNotes ? (<p className="text-sm text-muted-foreground">
                {communication.notes || "No notes available"}
              </p>) : (<Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Enter notes about this communication..." className="w-full" rows={3}/>)}
          </div>
          
          {communication.ai_summary && (<div className="mt-4 p-3 bg-primary-foreground/50 rounded-md border border-primary/10">
