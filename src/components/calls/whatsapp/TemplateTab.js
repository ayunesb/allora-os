...
export default function TemplateTab({ phoneNumber, selectedLeadId, templates, isLoadingTemplates, onMessageSent, isLoadingMutation }) {
    ...
    const handleSendTemplate = async () => {
        ...
        setIsSending(true);
        try {
            const sentViaApi = await sendWhatsAppTemplate(formattedNumber, selectedTemplate, templateVariables, selectedLeadId);
            if (sentViaApi) {
    ...
    return (<div className="space-y-4">
      <div className="space-y-2">
        ...
        <Select value={selectedTemplate} onValueChange={setSelectedTemplate} disabled={isLoadingTemplates}>
          ...
          <SelectContent>
            {templates.map((template) => (<SelectItem key={template.sid} value={template.name}>
                {template.name}
              </SelectItem>))}
          </SelectContent>
        </Select>
      ...
      {selectedTemplate && (<div className="space-y-2 border p-3 rounded-md bg-muted/20">
          <h4 className="font-medium">Template Variables</h4>
          
          <div className="space-y-2">
            <Label htmlFor="variable-1">Name</Label>
            <Input id="variable-1" placeholder="Enter customer name" value={templateVariables.name || ""} onChange={(e) => setTemplateVariables({
                ...templateVariables,
                name: e.target.value
            })}/>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input id="variable-2" placeholder="Enter company name" value={templateVariables.company || ""} onChange={(e) => setTemplateVariables({
                ...templateVariables,
      ...
      <Button onClick={handleSendTemplate} disabled={isSending || isLoadingMutation || !phoneNumber || !selectedTemplate} className="w-full">
        {isSending ? (<>
