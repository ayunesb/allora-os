...
export default function AiCallScriptFeedback({ id, title, type, primaryBot }) {
    ...
    return (<div className="flex items-center justify-end space-x-2 mt-2">
      <span className="text-xs text-muted-foreground mr-1">Helpful?</span>
      <Button variant="outline" size="sm" className="h-7 w-7 p-0" onClick={() => handleFeedback(true)} disabled={isSubmitting}>
        {isSubmitting ? (<Loader2 className="h-3 w-3 animate-spin"/>) : (<ThumbsUp className="h-3 w-3"/>)}
      </Button>
      <Button variant="outline" size="sm" className="h-7 w-7 p-0" onClick={() => handleFeedback(false)} disabled={isSubmitting}>
