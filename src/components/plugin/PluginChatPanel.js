...
export default function GalaxyExplorer() {
    ...
    return (<div className="flex h-screen bg-background text-foreground">
      ...
      <div className="w-1/4 border-l border-border">
        {selectedPlugin ? (<>
            <button onClick={() => setSelectedPlugin(null)} className="mb-2 px-3 py-1 text-sm bg-secondary text-secondary-foreground rounded-lg">
              Reset Selection
            </button>
      ...
      <div className="flex flex-col h-full border border-muted rounded-xl p-3 overflow-hidden">
        <div className="flex-1 overflow-y-auto space-y-2 pr-2">
          {messages.map((msg) => (<div className="flex items-start gap-2" key={msg.id}>
              {msg.sender === "agent" && (<div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center text-xs font-bold">
                  🤖
                </div>)}
              ...
              {msg.sender === "user" && (<div className="w-8 h-8 rounded-full bg-accent flex items-center justify-center text-xs font-bold">
                  🧑
                </div>)}
            </div>))}
          {loading && (<div className="mr-auto text-xs text-muted-foreground animate-pulse">
              AI is thinking...
            </div>)}
          <div ref={chatEndRef}/>
        </div>

        {showXpBoost && (<div className="absolute top-4 right-4 animate-bounce text-green-500 text-sm font-semibold">
            +5 XP 💡
          </div>)}
        ...
        <form className="mt-3 flex items-center gap-2" onSubmit={(e) => {
          ...
          <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask the plugin something..." className="flex-1 border border-input rounded-lg px-3 py-2 text-sm bg-background"/>
          <button type="submit" disabled={loading} className="px-3 py-2 text-sm bg-primary text-primary-foreground rounded-lg">
            Send
          </button>
        </form>
      </div>
