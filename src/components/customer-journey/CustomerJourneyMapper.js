...
export function CustomerJourneyMapper() {
    ...
    const handleSaveStage = (formData) => {
        if (editingStage?.id) {
            ...
            setJourneyStages(prev => prev.map(stage => stage.id === editingStage.id ? { ...stage, ...formData } : stage));
    ...
    const handleSaveTouchpoint = (formData) => {
        ...
        if (editingTouchpoint?.id) {
            ...
            setJourneyStages(prev => prev.map(stage => stage.id === editingStage.id
                ? {
                    ...stage,
                    touchpoints: stage.touchpoints.map(tp => tp.id === updatedTouchpoint.id ? updatedTouchpoint : tp)
                }
                : stage));
          ...
          <div className="p-6">
            ...
            <TabsContent value="flow" className="mt-0">
              <div className="grid grid-cols-1 gap-8">
                {journeyStages.map((stage, index) => (<div key={stage.id} className="relative">
                    {/* Stage details */}
                    <div className="flex justify-between items-start bg-muted/30 p-4 rounded-lg mb-2">
                      ...
                      <div className="flex space-x-2">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => handleAddTouchpoint(stage.id)}>
                    ...
                    <div className="grid grid-cols-1 gap-3 pl-8">
                      {stage.touchpoints.map(touchpoint => (<Card key={touchpoint.id} className={`relative border-l-4 ${touchpoint.status === 'active'
                    ? 'border-l-green-500'
                    : touchpoint.status === 'draft'
                            ...
                            <div className="flex justify-between items-start">
                              <div>
                                ...
                                <div className="flex flex-wrap gap-1 mt-2">
                                  {touchpoint.metrics.map((metric, i) => (<span key={i} className="text-xs px-2 py-0.5 bg-primary/10 text-primary rounded-full">
                                      {metric}
                                    </span>))}
                                </div>
                              </div>
                              <div className="flex space-x-1">
                                <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => handleEditTouchpoint(stage.id, touchpoint)}>
                                  <Edit className="h-3.5 w-3.5"/>
                                </Button>
                                <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => handleDeleteTouchpoint(stage.id, touchpoint.id)}>
                      ...
                      {stage.touchpoints.length === 0 && (<Button variant="outline" className="border-dashed" onClick={() => handleAddTouchpoint(stage.id)}>
                    ...
                    {index < journeyStages.length - 1 && (<div className="flex justify-center my-4">
                        <ArrowDown className="h-6 w-6 text-muted-foreground"/>
                      </div>)}
                  </div>))}
                
                {/* Add new stage button */}
                <Button variant="outline" className="mt-4 border-dashed" onClick={handleAddStage}>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {journeyStages.map((stage, index) => (<div key={stage.id}>
                          ...
                          <div className="w-full bg-muted rounded-full h-2.5">
                            <div className="bg-primary h-2.5 rounded-full" style={{ width: index === 0 ? '100%' : index === 1 ? '72%' : index === 2 ? '45%' : '28%' }}></div>
                          </div>
                          <div className="grid grid-cols-3 gap-2 mt-2">
                            {stage.touchpoints.map(touchpoint => (<div key={touchpoint.id} className="text-xs flex justify-between bg-muted/40 p-2 rounded">
          ...
          <form onSubmit={(e) => {
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <Input id="title" name="title" placeholder="e.g. Awareness, Consideration" defaultValue={editingStage?.title || ''} required/>
              </div>
              <div className="space-y-2">
                <Label htmlFor="description">Description</Label>
                <Textarea id="description" name="description" placeholder="Describe this stage in the customer journey" defaultValue={editingStage?.description || ''} required/>
              </div>
            </div>
            <DialogFooter>
              <Button type="submit">
          ...
          <form onSubmit={(e) => {
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <Input id="title" name="title" placeholder="e.g. Welcome Email, Product Demo" defaultValue={editingTouchpoint?.title || ''} required/>
              </div>
              <div className="space-y-2">
                <Label htmlFor="channel">Channel</Label>
                <Select name="channel" defaultValue={editingTouchpoint?.channel || ''} required>
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Textarea id="content" name="content" placeholder="What message does this touchpoint deliver?" defaultValue={editingTouchpoint?.content || ''} required/>
              </div>
              <div className="space-y-2">
                <Label htmlFor="metrics">Key Metrics (comma separated)</Label>
                <Input id="metrics" name="metrics" placeholder="e.g. Open Rate, CTR, Conversion Rate" defaultValue={editingTouchpoint?.metrics?.join(', ') || ''} required/>
              </div>
              <div className="space-y-2">
                <Label htmlFor="status">Status</Label>
                <Select name="status" defaultValue={editingTouchpoint?.status || 'draft'} required>
                  <SelectTrigger>
