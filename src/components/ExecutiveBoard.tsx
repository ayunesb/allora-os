  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {executives.map((executive) => (
        <Card
          key={executive.id}
          className={`cursor-pointer transition-shadow hover:shadow-md ${executive.status === "inactive" ? "opacity-70" : ""}`}
          onClick={() => onSelectExecutive && onSelectExecutive(executive.id)}
        >
              ...
              <div className="flex-1 space-y-1">
                ...
                {executive.specialties && (
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {executive.specialties.slice(0, 2).map((specialty) => (
                      <Badge
                        key={specialty}
                        variant="outline"
...
function StatusBadge({ status }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${variants[status].className}`}
    >
