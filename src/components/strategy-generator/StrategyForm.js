...
const formSchema = z.object({
    ...
    goals: z.string().min(10, { message: "Business goals must be at least 10 characters" }),
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <FormField control={form.control} name="goals" render={({ field }) => (<FormItem>
                    ...
                    <FormControl>
                      <Textarea placeholder="Describe your primary business goals and targets..." className="min-h-[100px]" {...field}/>
                    </FormControl>
                    <FormMessage />
                  </FormItem>)}/>
              ...
              <FormField control={form.control} name="challenges" render={({ field }) => (<FormItem>
                    ...
                    <FormControl>
                      <Textarea placeholder="Describe any current challenges or obstacles..." className="min-h-[100px]" {...field}/>
                    </FormControl>
                    <FormMessage />
                  </FormItem>)}/>
              ...
              <FormField control={form.control} name="riskTolerance" render={({ field }) => (<FormItem>
                    ...
                    <FormControl>
                      <div className="flex items-center space-x-4">
                        ...
                        <Slider min={1} max={10} step={1} defaultValue={[field.value]} onValueChange={(values) => field.onChange(values[0])} className="w-full"/>
      ...
      <CardContent>
        <Tabs defaultValue={`strategy-0`}>
          <TabsList className="mb-4">
            {strategies.map((strategy, index) => (<TabsTrigger key={index} value={`strategy-${index}`}>
                Option {index + 1}
              </TabsTrigger>))}
          </TabsList>
          
          {strategies.map((strategy, index) => (<TabsContent key={index} value={`strategy-${index}`} className="space-y-6">
              <div>
              ...
              <div>
                ...
                <ol className="list-decimal pl-5 space-y-1 mt-2">
                  {strategy.implementationSteps.map((step, i) => (<li key={i}>{step}</li>))}
                </ol>
              </div>
            </TabsContent>))}
        </Tabs>
...
async function generateStrategies(params) {
    try {
        ...
        return { data: null, error: err.message || 'Failed to generate strategies' };
    }
}
