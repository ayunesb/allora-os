...
import { Check, Info, Save, Settings } from "lucide-react";
import { toast } from "sonner";
import { BotPersonalitySettings, PersonalityTrait, ResponseStyle, TechnicalLevel, FocusArea } from '@/types/aiSettings';
...
const personalityDescriptions: Record<PersonalityTrait, string> = {
...
const responseStyleDescriptions: Record<ResponseStyle, string> = {
...
export function PersonalitySettings({ botPersonalities, onUpdatePersonality }: PersonalitySettingsProps) {
  ...
  const handleSavePersonality = async (botId: string, settings: Partial<BotPersonalitySettings>) => {
    try {
  ...
  return (
    <Card className="w-full">
      ...
      <Tabs 
        defaultValue={botPersonalities[0]?.botId} 
        value={activeBot} 
      ...
      >
        ...
        {botPersonalities.map(bot => (
          <TabsContent key={bot.botId} value={bot.botId} className="m-0">
            <CardContent className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="pt-2">
                  <Slider
                    id={`personality-${bot.botId}`}
                    defaultValue={[getPersonalityValue(bot.personalityTrait)]}
                    max={3}
                    step={1}
              ...
              <div className="space-y-2 pt-4">
                ...
                <Select 
                  ...
                  onValueChange={(value) => handleSavePersonality(bot.botId, { 
              ...
              <div className="space-y-2 pt-4">
                ...
                <Select 
                  ...
                  onValueChange={(value) => handleSavePersonality(bot.botId, { 
                    technicalLevel: value as TechnicalLevel 
                  })}
                >
              ...
              <div className="flex items-center space-x-2 pt-4">
                <Switch 
                  id={`sources-${bot.botId}`}
                  checked={bot.showSources}
                  onCheckedChange={(checked) => handleSavePersonality(bot.botId, { 
                    showSources: checked 
                  })}
                />
              ...
              <div className="space-y-2 pt-4">
                ...
                <Textarea 
                  id={`custom-${bot.botId}`}
                  placeholder="Add specific instructions for how this executive should behave..."
                  value={bot.customInstructions || ''}
                  onChange={(e) => handleSavePersonality(bot.botId, { 
...
function getPersonalityValue(trait: PersonalityTrait): number {
  switch (trait) {
...
function getPersonalityFromValue(value: number): PersonalityTrait {
  switch (value) {
