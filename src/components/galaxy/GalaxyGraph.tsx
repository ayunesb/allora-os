...
const GalaxyGraph = ({ nodes, links, onNodeClick }) => {
    ...
    return (<div className="h-[600px] w-full">
      <ForceGraph3D ref={fgRef} graphData={{ nodes, links }} backgroundColor="#0A0A23" nodeLabel={(node) => `${node.id} (${node.total_xp} XP)`} nodeAutoColorBy="group" linkDirectionalParticles={2} linkDirectionalParticleSpeed={(d) => d.value * 0.001 || 0.01} onNodeClick={onNodeClick} // Pass onNodeClick to ForceGraph3D
