...
export default function BotInsightsSection() {
    ...
    if (error) {
        ...
        <CardHeader className="text-destructive">Error loading insights</CardHeader>
        <CardContent>
          <p className="text-sm">
    ...
    if (insights.length === 0) {
        return (<Card className="border-dotted">
          ...
          {filteredInsights.length === 0 ? (<Card>
              <CardContent className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <p className="text-muted-foreground max-w-md">
                  We couldn't find any {activeTab !== "all" ? activeTab + " " : ""}insights. 
                  Try selecting a different filter.
                </p>
              </CardContent>
            </Card>) : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredInsights.map((insight) => (<BotInsightCard key={insight.id} insight={insight} onViewDetails={handleViewDetails}/>))}
            </div>)}
        </TabsContent>
      </Tabs>
      
      <InsightDetailsDialog insight={selectedInsight} open={detailsOpen} onOpenChange={setDetailsOpen}/>
    </div>);
}
