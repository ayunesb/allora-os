...
export default function BotInsightCard({ insight, onViewDetails }) {
    ...
    return (<Card className="transition-all duration-200 hover:shadow-md overflow-hidden">
        ...
        <CardDescription>
          <div className="flex items-center mt-1">
            <Avatar className="h-6 w-6 mr-2">
              <AvatarImage src={insight.primaryBot.avatar || `/avatars/${insight.primaryBot.name.toLowerCase().replace(/\s+/g, '-')}.png`} alt={insight.primaryBot.name}/>
      ...
      <CardFooter className="pt-2 flex justify-between">
        <Button variant="ghost" size="sm" className="text-xs" onClick={() => onViewDetails(insight)}>
