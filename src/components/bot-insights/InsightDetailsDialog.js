        ...
        <div className="space-y-6 mt-4">
          ...
          <div className="flex items-start gap-3">
            <Avatar className="h-12 w-12">
              <AvatarImage src={insight.primaryBot.avatar} alt={insight.primaryBot.name}/>
          ...
          {/* Executive Summary */}
          {detailedInsight?.executiveSummary && (<div>
              <h4 className="font-medium mb-2">Executive Summary</h4>
              <p className="text-sm">{detailedInsight.executiveSummary}</p>
            </div>)}
          
          {/* Key Points */}
          {detailedInsight?.keyPoints && detailedInsight.keyPoints.length > 0 && (<div>
              <h4 className="font-medium mb-2">Key Points</h4>
              <ul className="space-y-2">
                {detailedInsight.keyPoints.map((point, index) => (<li key={index} className="flex items-start gap-2">
          ...
          {/* Contributors */}
          {detailedInsight?.contributors && detailedInsight.contributors.length > 0 && (<div>
              <h4 className="font-medium mb-3">Contributing Executives</h4>
              <div className="space-y-3">
                {detailedInsight.contributors.map((contributor, index) => (<ContributorItem key={index} contributor={contributor}/>))}
              </div>
            </div>)}
        </div>
      </DialogContent>
...
function ContributorItem({ contributor }) {
    return (<div className="flex items-start gap-3">
      <div className="mt-0.5">
        {contributor.opinion === 'positive' && (<Check className="h-4 w-4 text-green-500"/>)}
        {contributor.opinion === 'negative' && (<X className="h-4 w-4 text-red-500"/>)}
        {contributor.opinion === 'neutral' && (<Minus className="h-4 w-4 text-yellow-500"/>)}
      </div>
