...
export default function ExecutiveCards() {
    return (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {Object.entries(executiveBots).map(([role, names]) => (<Card key={role} className="border-primary/10 shadow-md">
          ...
          <CardContent>
            <ul className="space-y-1">
              {names.map(name => (<li key={name} className="text-muted-foreground">
                  {name}
                </li>))}
            </ul>
          </CardContent>
