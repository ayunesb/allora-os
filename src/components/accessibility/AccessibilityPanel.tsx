...
export function AccessibilityPanel() {
    ...
    return (<Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="space-y-6 py-4">
          <div className="space-y-2">
            ...
            <Slider value={[Number(fontSize)]} min={12} max={24} step={1} onValueChange={handleFontSizeChange} className="w-full"/>
          </div>
          
          <div className="space-y-4">
            {toggles.map((toggle) => (<div key={toggle.id} className="flex flex-col space-y-1">
      ...
      <DialogTrigger asChild>
        <Button variant="outline" size="icon" className="fixed bottom-4 right-4 rounded-full z-50 bg-primary text-primary-foreground" aria-label="Accessibility Settings">
