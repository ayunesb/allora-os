...
export const AiBotsTabsList = ({ isMobileView: propsMobileView, activeTab, onTabChange, tabIcons = {} }) => {
    ...
    if (!ScrollableTabs) {
        return (<TabsList className="w-full mb-6 py-2 overflow-x-auto bg-black/30 border border-white/10 backdrop-blur-md rounded-lg">
        {tabs.map((tab) => (<TabsTrigger key={tab.id} value={tab.id} onClick={() => onTabChange && onTabChange(tab.id)} className={`
            ...
            `}>
            {activeTab === tab.id && (<motion.div layoutId="active-tab-background" className="absolute inset-0 bg-primary/10 rounded-sm -z-10" initial={false} transition={{ type: "spring", duration: 0.5 }}/>)}
    ...
    return (<div className="w-full max-w-full overflow-hidden">
      <ScrollableTabs tabs={tabs} activeTab={activeTab || tabs[0].id} onTabChange={onTabChange || (() => { })} className="mb-6 py-2 bg-black/30 border border-white/10 backdrop-blur-md rounded-lg" variant="default"/>
    </div>);
};
