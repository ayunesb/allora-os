...
export const LinkedInIntegration: React.FC<LinkedInIntegrationProps> = ({ children, variant = "default", size = "medium" }) => {
    ...
    const toggleConnectionSelection = (id) => {
        setSelectedConnections(prev => prev.includes(id)
            ? prev.filter(c => c !== id)
            : [...prev, id]);
    };
    ...
    return (<div className="space-y-6">
      <Card>
        ...
        <CardContent>
          {!isAuthenticated ? (<div className="flex flex-col items-center gap-4 py-6">
              <p>Connect your LinkedIn account to import connections as leads</p>
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-muted-foreground">Search and import your connections</p>
                </div>
              ...
              <div className="space-y-4">
                <div className="flex gap-2">
                  <Input placeholder="Search connections..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="flex-1"/>
                ...
                {connections.length > 0 && (<div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <Button variant="ghost" size="sm" onClick={selectAllConnections}>
                    ...
                    <div className="border rounded-md">
                      {connections.map((connection) => (<div key={connection.id} className="flex items-center gap-3 p-3 border-b last:border-b-0 hover:bg-muted/50">
                          <Checkbox checked={selectedConnections.includes(connection.id)} onCheckedChange={() => toggleConnectionSelection(connection.id)}/>
                          <div className="flex-1">
                            <p className="font-medium">{connection.name}</p>
                            {connection.title && connection.company && (<p className="text-sm text-muted-foreground">
                    ...
                    <div className="flex flex-col gap-4">
                      ...
                      <Button onClick={handleImport} disabled={isImporting || selectedConnections.length === 0 || !selectedCampaign}>
                        {isImporting ? (<>
                            <Loader2 className="mr-2 h-4 w-4 animate-spin"/>
                            Importing...
                          </>) : (<>
