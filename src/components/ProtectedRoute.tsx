...
const AuthLoadingState = () => (<div className="flex flex-col items-center justify-center min-h-screen p-4">
    <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mb-4"></div>
    <p className="text-lg text-center text-muted-foreground">Verifying your access...</p>
  </div>);
// Auth error state component
const AuthErrorState = ({ error, onRetry, isRetrying }) => (<div className="flex flex-col items-center justify-center min-h-screen p-4">
    <div className="p-6 bg-destructive/10 rounded-lg max-w-md">
      ...
      <button onClick={onRetry} disabled={isRetrying} className="bg-primary text-white px-4 py-2 rounded hover:bg-primary/90 transition-colors">
        {isRetrying ? 'Retrying...' : 'Retry'}
      </button>
    </div>
  </div>);
// Verification required state component
const VerificationRequiredState = ({ onRefresh, onResendVerification, isResending }) => (<div className="flex flex-col items-center justify-center min-h-screen p-4">
    <div className="p-6 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg max-w-md">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <button onClick={onRefresh} className="bg-primary text-white px-4 py-2 rounded hover:bg-primary/90 transition-colors">
          I've Verified My Email
        </button>
        <button onClick={onResendVerification} disabled={isResending} className="bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 px-4 py-2 rounded hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">
...
export default function ProtectedRoute({ children, roleRequired, adminOnly, requireVerified = false }) {
    ...
    // Handle auth errors
    if (auth.authError) {
        return <AuthErrorState error={auth.authError} onRetry={async () => {
    ...
    if (requireVerified && !auth.isEmailVerified && auth.hasInitialized) {
        return <VerificationRequiredState onRefresh={async () => {
                if (auth.refreshSession) {
                    await auth.refreshSession();
                }
