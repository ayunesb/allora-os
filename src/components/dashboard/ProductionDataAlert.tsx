...
export default function ProductionDataAlert() {
    ...
    return (<Alert variant={isProductionReady ? "default" : "destructive"} className={isProductionReady
            ? "border-risk-low-light bg-risk-low-light/20 text-risk-low-DEFAULT dark:text-risk-low-dark"
            : "border-amber-300 bg-amber-50 text-amber-900"}>
      <div className="flex justify-between items-center">
        <div className="flex items-start gap-3">
          {isProductionReady ? (<CheckCircle2 className="h-5 w-5 text-green-500 mt-0.5"/>) : (<AlertCircle className="h-5 w-5 text-amber-600 mt-0.5"/>)}
          <div>
        ...
        <div className="flex gap-2">
          {!isProductionReady && (<Button variant="outline" size="sm" onClick={handleSetupProduction} className="border-amber-400 bg-amber-100 hover:bg-amber-200 text-amber-900">
              Setup Production
            </Button>)}
          <Button variant="outline" size="sm" onClick={handleForceProduction} className="border-green-400 bg-green-100 hover:bg-green-200 text-green-900">
            Use Real Data
          </Button>
          <Button variant="ghost" size="sm" onClick={handleDismiss} className="text-amber-900 hover:bg-amber-100">
            Dismiss
          </Button>
        </div>
      </div>
