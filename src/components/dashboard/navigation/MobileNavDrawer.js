    ...
    return (<Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent>
        ...
        <div className="py-4 max-h-[70vh] overflow-y-auto">
          {tabs.map((tab) => (<Link to={tab.path} key={tab.name} className={`flex items-center space-x-2 p-3 hover:bg-accent rounded-md transition-colors ${isActive(tab.path) ? "font-medium" : ""}`} onClick={() => onOpenChange(false)}>
              {tab.icon}
              <span>{tab.name}</span>
            </Link>))}
        </div>
