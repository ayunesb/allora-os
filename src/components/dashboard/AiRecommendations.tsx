...
const AiRecommendations: React.FC<AiRecommendationsProps> = ({ children, variant = "default", size = "expanded" }) => {
    ...
    return (<>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
        {recommendations.map((rec, index) => (<Card key={index} className="border-primary/10 hover:border-primary/30 transition-all">
            ...
            <CardContent>
              <div className="mb-4 flex items-start gap-3">
                <Avatar className="h-8 w-8 border border-primary/20">
                  <AvatarImage src={`/avatars/${rec.executiveBot.name.toLowerCase().replace(/\s+/g, '-')}.png`} alt={rec.executiveBot.name}/>
            ...
            <CardFooter className="flex justify-between border-t pt-4">
              {feedbackState[index] === 'liked' ? (<Button variant="outline" size="sm" className="text-green-500" disabled>
                  ...
                  <Button variant="ghost" size="sm" onClick={() => handleFeedback(index, false)} className="text-muted-foreground">
