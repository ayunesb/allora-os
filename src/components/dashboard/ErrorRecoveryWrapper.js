...
export function ErrorRecoveryWrapper({ children, fallbackComponent, onReset, errorTitle = "Something went wrong", errorMessage = "We encountered an error loading this component. Your data is safe, and you can try again.", componentName = "unknown component" }) {
    ...
    if (hasError) {
        ...
        return (<Card className="w-full border border-red-200 bg-red-50/50 dark:bg-red-950/10 dark:border-red-900/50">
        <CardHeader>
        ...
        <CardContent>
          ...
          {process.env.NODE_ENV !== 'production' && error && (<div className="bg-muted p-3 rounded-md overflow-auto max-h-32 text-xs">
              <p className="font-mono text-red-500">{error.toString()}</p>
              {errorInfo && (<pre className="mt-2 text-muted-foreground">
    ...
    return (<ErrorBoundaryClass onError={handleCatchError}>
      {children}
    </ErrorBoundaryClass>);
}
