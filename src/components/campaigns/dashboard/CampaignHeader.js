...
export function CampaignHeader({ onRefresh, onCreateCampaign, isRefreshing }) {
    return (<div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
      <div>
      ...
      <div className="flex gap-2">
        <Button variant="outline" onClick={onRefresh} disabled={isRefreshing} aria-label={isRefreshing ? "Refreshing campaign data" : "Refresh campaign data"}>
          <RefreshCcw className={`mr-2 h-4 w-4 ${isRefreshing ? 'animate-spin' : ''}`}/>
          {isRefreshing ? 'Refreshing...' : 'Refresh Data'}
        </Button>
        
        <Button onClick={onCreateCampaign} aria-label="Create new campaign">
