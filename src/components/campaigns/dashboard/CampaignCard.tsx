    ...
    return (<Card className="overflow-hidden">
      <CardContent className="p-4">
        <div className="flex justify-between items-start mb-3">
          ...
          <Button variant="outline" size="sm" onClick={() => onViewAnalytics(id)}>
      ...
      <CardFooter className="px-4 py-3 border-t bg-muted/30 flex justify-between">
        {onStatusChange && (<div className="flex space-x-2">
            {isDraft && (<Button variant="outline" size="sm" onClick={() => onStatusChange(id, 'active')}>
                <PlayCircle className="h-4 w-4 mr-1"/>
                Activate
              </Button>)}
            
            {isActive && (<Button variant="outline" size="sm" onClick={() => onStatusChange(id, 'paused')}>
                <PauseCircle className="h-4 w-4 mr-1"/>
                Pause
              </Button>)}
            
            {isPaused && (<Button variant="outline" size="sm" onClick={() => onStatusChange(id, 'active')}>
                <PlayCircle className="h-4 w-4 mr-1"/>
                Resume
              </Button>)}
            
            {!isCompleted && (<Button variant="outline" size="sm" onClick={() => onStatusChange(id, 'completed')}>
        ...
        <div className="flex space-x-2">
          ...
          <Button variant="ghost" size="sm" onClick={() => onDelete(id)} className="text-destructive">
