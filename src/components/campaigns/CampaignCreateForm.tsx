...
const formSchema = z.object({
    ...
    targetingAudience: z.string().min(5, { message: 'Please describe your target audience' }),
    targetingLocation: z.string().min(2, { message: 'Please specify a location' }),
    adTitle: z.string().min(5, { message: 'Ad title must be at least 5 characters' }),
    adDescription: z.string().min(10, { message: 'Ad description must be at least 10 characters' }),
});
export default function CampaignCreateForm() {
    ...
    const onSubmit = async (values) => {
        ...
        try {
            ...
            const checkoutResult = await createCampaignCheckout(campaignId, window.location.href);
    ...
    const renderFormStep = () => {
        switch (step) {
            ...
            <FormField control={form.control} name="budget" render={({ field }) => (<FormItem>
                  ...
                  <FormControl>
                    <Input type="number" min="100" step="100" placeholder="1000" {...field} onChange={(e) => {
                            field.onChange(e);
            ...
            case 2:
                return (<>
            <FormField control={form.control} name="targetingAudience" render={({ field }) => (<FormItem>
                  <FormLabel>Target Audience</FormLabel>
                  <FormControl>
                    <Textarea placeholder="E.g., Males and females, ages 25-45, interested in fitness and nutrition" className="min-h-[100px]" {...field}/>
                  </FormControl>
                  <FormMessage />
                </FormItem>)}/>
            
            <FormField control={form.control} name="targetingLocation" render={({ field }) => (<FormItem>
                  <FormLabel>Target Location</FormLabel>
                  <FormControl>
                    <Input placeholder="E.g., United States, Canada" {...field}/>
                  </FormControl>
                  <FormMessage />
                </FormItem>)}/>
          </>);
            case 3:
                return (<>
            <FormField control={form.control} name="adTitle" render={({ field }) => (<FormItem>
            ...
            <FormField control={form.control} name="adDescription" render={({ field }) => (<FormItem>
                  <FormLabel>Ad Description</FormLabel>
                  <FormControl>
                    <Textarea placeholder="E.g., Don't miss our biggest sale of the year! Get 50% off all products until July 31." className="min-h-[100px]" {...field}/>
                  </FormControl>
                  <FormMessage />
                </FormItem>)}/>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="bg-muted p-4 rounded-md space-y-2 mt-2">
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <span>{form.getValues('platform') === 'meta' ? 'Meta (Facebook/Instagram)' : 'TikTok'}</span>
                    ...
                    <span className="font-medium col-span-2 pt-2">Location:</span>
                    <span className="col-span-2">{form.getValues('targetingLocation')}</span>
                    
                    <span className="font-medium col-span-2 pt-2">Target Audience:</span>
                    <span className="col-span-2">{form.getValues('targetingAudience')}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-primary/10 p-4 rounded-md">
                <p className="text-sm">
                  By clicking "Create Campaign & Checkout", you agree to pay the total amount of <strong>${totalAmount}</strong> which includes your ad budget (${form.getValues('budget')}) and the 10% management fee (${managementFee}).
                </p>
              </div>
            </div>
          </>);
    ...
    return (<Card className="w-full max-w-2xl mx-auto">
      ...
      <CardFooter className="flex justify-between">
        {step > 1 && (<Button type="button" variant="outline" onClick={handlePreviousStep} disabled={isSubmitting}>
            Previous
          </Button>)}
        
        <div className="ml-auto">
          {step < 3 ? (<Button type="button" onClick={handleNextStep} disabled={isSubmitting}>
              Next
            </Button>) : (<Button type="submit" onClick={form.handleSubmit(onSubmit)} disabled={isSubmitting}>
