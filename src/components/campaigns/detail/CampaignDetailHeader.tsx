    ...
    return (<div className="flex items-center mb-8">
      ...
      <div>
        ...
        <div className="flex items-center text-muted-foreground mt-1">
          {getPlatform() === 'meta' ? (<Facebook className="h-4 w-4 mr-1 text-blue-600"/>) : (<TikTokIcon className="h-4 w-4 mr-1"/>)}
          <span>{getPlatform() === 'meta' ? 'Meta Ads' : 'TikTok Ads'}</span>
        </div>
      </div>
      
      <div className="ml-auto flex gap-2">
        {campaign.payment_status === 'paid' && campaign.deployment_status === 'ready' && (<Button onClick={onDeploy} disabled={isDeploying}>
            {isDeploying ? (<>
