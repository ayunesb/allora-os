        ...
        <TableBody>
          {data.map((channel, index) => (
            <TableRow key={index}>
              ...
              <TableCell className="text-right">
                {formatNumber(channel.metrics.clicks)}
              </TableCell>
              <TableCell className="text-right">
                {(channel.metrics.ctr * 100).toFixed(2)}%
              </TableCell>
              <TableCell className="text-right">
              ...
              <TableCell className="text-right">
                {formatCurrency(channel.metrics.cost)}
              </TableCell>
              <TableCell className="text-right">
                {formatCurrency(channel.metrics.revenue)}
              </TableCell>
              <TableCell className="text-right">
                {(channel.metrics.roi * 100).toFixed(0)}%
              </TableCell>
            </TableRow>
          ))}
          ...
          {totalMetrics && (
            <TableRow className="font-bold bg-muted/50">
              ...
              <TableCell className="text-right">
                {formatNumber(totalMetrics.totalImpressions)}
              </TableCell>
              <TableCell className="text-right">
              ...
              <TableCell className="text-right">
                {(
                  (totalMetrics.totalClicks / totalMetrics.totalImpressions) *
                  100
                ).toFixed(2)}
                %
              </TableCell>
              ...
              <TableCell className="text-right">
                {(
                  (totalMetrics.totalConversions / totalMetrics.totalClicks) *
                  100
                ).toFixed(2)}
                %
              </TableCell>
              ...
              <TableCell className="text-right">
                {(
                  (totalMetrics.totalRevenue / totalMetrics.totalCost - 1) *
                  100
                ).toFixed(0)}
                %
              </TableCell>
