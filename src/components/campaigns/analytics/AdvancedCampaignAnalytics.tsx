...
export function AdvancedCampaignAnalytics({ campaign }) {
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {performanceData.slice(0, 4).map((item, index) => (<Card key={index}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {performanceData.slice(4).map((item, index) => (<Card key={index}>
            <CardContent className="p-4 text-center">
              <h3 className="text-sm font-medium text-muted-foreground mb-1">{item.name}</h3>
              <p className="text-2xl font-bold">
                {item.format ? item.format(item.value) : item.value.toLocaleString()}
        ...
        <CardContent>
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="overview">
              <div className="h-80">
                {dayMetrics.length > 0 ? (<ResponsiveContainer width="100%" height="100%">
                    <LineChart data={dayMetrics}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Line yAxisId="left" type="monotone" dataKey="impressions" stroke="#8884d8" name="Impressions" dot={false}/>
                      <Line yAxisId="right" type="monotone" dataKey="clicks" stroke="#82ca9d" name="Clicks" dot={false}/>
                    </LineChart>
            ...
            <TabsContent value="channels">
              <div className="h-80">
                {channelBreakdown.length > 0 ? (<div className="flex flex-col md:flex-row gap-6">
                    <ResponsiveContainer width="50%" height={300}>
                      <PieChart>
                        <Pie data={channelBreakdown} cx="50%" cy="50%" labelLine={false} label={({ name, percent }) => `${name}: ${(percent * 100).toFixed(0)}%`} outerRadius={80} fill="#8884d8" dataKey="value">
                          {channelBreakdown.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]}/>))}
                        </Pie>
            ...
            <TabsContent value="devices">
              <div className="h-80">
                {deviceBreakdown.length > 0 ? (<div className="flex flex-col md:flex-row gap-6">
                    <ResponsiveContainer width="50%" height={300}>
                      <PieChart>
                        <Pie data={deviceBreakdown} cx="50%" cy="50%" labelLine={false} label={({ name, percent }) => `${name}: ${(percent * 100).toFixed(0)}%`} outerRadius={80} fill="#82ca9d" dataKey="value">
                          {deviceBreakdown.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]}/>))}
                        </Pie>
                        <Tooltip formatter={(value) => [`${value}`, 'Value']}/>
                      </PieChart>
                    </ResponsiveContainer>
