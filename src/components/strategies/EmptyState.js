...
const EmptyState = ({ onCreateNew, isLoading = false, error = null, onRetry }) => {
    ...
    if (error) {
        return (<div className="bg-secondary/40 border border-border/50 rounded-lg p-4 sm:p-6 text-center mb-10">
        ...
        {onRetry && (<Button onClick={onRetry} variant="outline" size={isMobile ? "sm" : "default"}>
    ...
    return (<div className="bg-secondary/40 border border-border/50 rounded-lg p-4 sm:p-6 text-center mb-10">
      ...
      <Button onClick={onCreateNew} className="allora-button" size={isMobile ? "sm" : "default"} disabled={isLoading}>
