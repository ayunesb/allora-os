...
const StrategyCard = ({ strategy, onEdit, onDelete, onView }) => {
    ...
    return (<div key={strategy.id} className="dashboard-card flex flex-col cursor-pointer" data-testid={`strategy-card-${strategy.id}`} onClick={handleCardClick}>
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 sm:gap-0 mb-3 sm:mb-4">
        ...
        <span className={`self-start sm:self-auto px-3 py-1 rounded-full text-xs font-medium ${getRiskColor(riskLevel)}`}>
          {riskLevel} Risk
        </span>
      </div>
        ...
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="outline" size={isMobile ? "sm" : "sm"} onClick={(e) => {
            e.stopPropagation();
            onEdit(strategy.id);
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <AlertDialogTrigger asChild>
                  <Button variant="destructive" size={isMobile ? "sm" : "sm"} onClick={(e) => e.stopPropagation()} aria-label={`Delete ${strategy.title}`}>
