...
const COLORS = ["#0088FE", "#00C49F", "#FFBB28", "#FF8042", "#8884D8", "#4BC0C0", "#36A2EB", "#9966FF"];
export function EnhancedVisualization({ type, data, title, description, config = {} }) {
    const renderVisualization = () => {
        switch (type) {
            ...
            <FunnelChart>
              <Tooltip />
              <Funnel dataKey="value" data={data} isAnimationActive fill={COLORS[0]}>
            ...
            <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
              ...
              <ZAxis type="number" dataKey="z" range={[60, 400]} name={config.zAxisName || "Z"}/>
              <Tooltip cursor={{ strokeDasharray: '3 3' }}/>
              {data.map((group, index) => (<Scatter key={index} name={group.name} data={group.data} fill={COLORS[index % COLORS.length]}/>))}
            </ScatterChart>
          </ResponsiveContainer>);
            case "heatmap":
                // Render a simple heatmap visualization (using a styled grid of divs)
                return (<div className="w-full h-[350px] overflow-auto">
            <div className="grid grid-cols-10 gap-1">
              {data.map((cell, index) => (<div key={index} className="aspect-square rounded" style={{
