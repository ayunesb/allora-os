...
export default function ProfileDiagnostics() {
    ...
    if (!user) {
        return (<Card className="mb-6 bg-red-50 dark:bg-red-900/10">
        <CardHeader className="pb-3">
          <CardTitle className="text-red-600 dark:text-red-400">Not Authenticated</CardTitle>
          <CardDescription>
    ...
    return (<Card className="mb-6 bg-green-50 dark:bg-green-900/10">
      <CardHeader className="pb-3">
        <CardTitle className="text-green-600 dark:text-green-400">Authentication Status</CardTitle>
        <CardDescription>
        ...
        <div className="text-sm space-y-1.5">
          ...
          {session && (<div className="mt-2">
              ...
              <pre className="bg-slate-100 dark:bg-slate-800 p-2 rounded text-xs overflow-x-auto">
                {JSON.stringify({
                provider: session?.provider_token ? 'OAuth provider' : 'Email/Password',
                expires_at: session?.expires_at ? new Date(session.expires_at * 1000).toLocaleString() : 'Unknown',
                last_accessed: session?.last_sign_in_at ? new Date(session.last_sign_in_at).toLocaleString() : 'Unknown'
            }, null, 2)}
              </pre>
            </div>)}
        </div>
