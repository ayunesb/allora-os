...
export default function ExecutiveBoard({ executives, onSelectExecutive }: ExecutiveBoardProps) {
  return (
                ...
                {executive.specialties && (
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {executive.specialties.slice(0, 2).map((specialty) => (
                      <Badge key={specialty} variant="outline" className="text-[0.65rem] px-1.5 py-0">
                        {specialty}
                      </Badge>
                    ))}
...
function StatusBadge({ status }: { status: 'active' | 'inactive' | 'learning' }) {
  const variants = {
