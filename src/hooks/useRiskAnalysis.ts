...
export function useRiskAnalysis() {
   ...
   */
  const calculateRiskScore = useCallback((factors: { [key: string]: number }): number => {
    let totalScore = 0;
    let totalWeight = 0;
   ...
   */
  const updateRiskFactors = useCallback((factors: { [key: string]: number }) => {
    setRiskFactors(prev => 
      prev.map(factor => ({
        ...factor,
        score: factors[factor.name] !== undefined ? factors[factor.name] : factor.score
      }))
    );
  }, []);
   ...
   */
  const getRiskAppetiteFromScore = useCallback((score: number): RiskAppetite => {
   ...
   */
  const getStrategyRecommendation = useCallback((riskAppetite: RiskAppetite): string => {
    switch (riskAppetite) {
      case 'low':
