...
export function useAiDebate() {
  ...
  const initiateDebate = useCallback(async (
    topic: string,
  ...
  const generateExecutiveResponses = useCallback(async () => {
    ...
    try {
      ...
      const newMessages = data.responses.map((resp: any) => ({
  ...
  const fetchDebateMessages = useCallback(async (existingDebateId: string) => {
    if (!user?.id) {
      return false;
    }
