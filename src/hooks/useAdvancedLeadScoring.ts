...
export function useAdvancedLeadScoring() {
   ...
   */
  const calculateAdvancedScore = useCallback((lead: Lead): LeadScoreBreakdown => {
    ...
    if (daysSinceCreation < 3) timeFactorScore = 20; // Very recent
    else if (daysSinceCreation < 7) timeFactorScore = 15; // Within a week
    else if (daysSinceCreation < 14) timeFactorScore = 10; // Within two weeks
    else if (daysSinceCreation < 30) timeFactorScore = 5; // Within a month
    
    // Calculate total score
    const totalScore = baseScore + engagementScore + demographicScore + behavioralScore + timeFactorScore;
   ...
   */
  const getLeadScoreCategory = useCallback((lead: Lead): LeadScoreCategory => {
  ...
  const getNextBestAction = useCallback((lead: Lead): string => {
    ...
    if (scoreCategory === 'hot') {
      if (lead.status === 'negotiation') return 'Schedule follow-up call within 24 hours';
      if (lead.status === 'proposal') return 'Send personalized case study and check in on proposal';
      if (lead.status === 'qualified') return 'Prepare tailored proposal with specific pricing options';
      return 'Prioritize for immediate contact by senior sales rep';
    }
    
    if (scoreCategory === 'warm') {
      if (lead.status === 'contacted') return 'Schedule detailed qualification call to understand needs';
      if (lead.status === 'new') return 'Send personalized introduction email with relevant content';
      return 'Follow up within 3 days with industry-specific information';
    }
    
    // Cold leads
    if (lead.status === 'new') return 'Add to automated email nurture sequence';
    if (lead.status === 'contacted') return 'Include in relevant webinar invitation';
   ...
   */
  const getLeadPriority = useCallback((lead: Lead): number => {
    const { totalScore } = calculateAdvancedScore(lead);
