...
export function useAiLearning() {
    ...
    const submitFeedback = useCallback(async (botName, botRole, isPositive, interactionId, messageId, comment, topics) => {
        if (!user?.id)
            return false;
        ...
        try {
            ...
            await updateLearningModel(botName, botRole, isPositive ? 'positive' : 'negative', topics);
            return true;
        }
        catch (error) {
    ...
    const updateLearningModel = useCallback(async (botName, botRole, feedbackType, topics) => {
        try {
    ...
    const getLearningModel = useCallback(async (botName, botRole) => {
        try {
            const { data, error } = await supabase
                .from('learning_models')
                .select('*')
    ...
    const getFeedbackHistory = useCallback(async (botName, botRole, limit = 10) => {
        if (!user?.id)
            return [];
        try {
            let query = supabase
    ...
    const trackFeedback = useCallback(async (interactionId, messageId, botName, botRole, isPositive, comment, metadata) => {
        return submitFeedback(botName, botRole, isPositive, interactionId, messageId, comment, metadata?.topic ? [metadata.topic] : undefined);
    }, [submitFeedback]);
    return {
        isSubmitting,
        isLoading,
