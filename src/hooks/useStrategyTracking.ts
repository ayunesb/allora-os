...
export function useStrategyTracking() {
  ...
  const trackApproval = useCallback(
    async (strategyId: string, strategyTitle: string) => {
  ...
  const trackRejection = useCallback(
    async (strategyId: string, reason: string) => {
      if (!user?.id) {
  ...
  const trackShare = useCallback(
    async (strategyId: string, shareDetails: object) => {
      if (!user?.id) {
  ...
  const trackStrategyUpdate = useCallback(
    async (strategyId: string, title: string, riskLevel: string) => {
      if (!user?.id) {
  ...
  const trackStrategyFilter = useCallback(
