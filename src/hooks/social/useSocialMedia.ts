...
const useSocialMedia = () => {
  ...
  const createPost = useCallback(async (postData: Partial<CreatePostInput>) => {
    try {
  ...
  const updatePost = useCallback(async (postId: string, updatedData: Partial<SocialMediaPost>) => {
    try {
      const { error } = await supabase
        .from('social_media_posts')
        .update(updatedData)
