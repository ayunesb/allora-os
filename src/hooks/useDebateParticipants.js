...
export default function useDebateParticipants() {
            ...
            names.forEach((name) => {
                if (executiveProfiles[name]) {
                    allExecutives.push({
                        ...
                        name: name,
                        role: executiveProfiles[name].role ||
    ...
    useEffect(() => {
        ...
        const enhancedParticipants = initialParticipants.map((participant) => {
            ...
            if (executiveInfo) {
                return Object.assign(Object.assign({}, participant), { expertise: executiveInfo.expertise || [], background: executiveInfo.background || "", avatar: executiveInfo.avatar || "/avatars/default.jpg" });
            }
            return participant;
        });
