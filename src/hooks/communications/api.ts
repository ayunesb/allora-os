...
export async function createZoomMeeting(leadId: string, meetingData: ZoomMeetingData) {
  try {
...
// Log a communication
export async function logCommunication(leadId: string, communicationData: CommunicationData) {
  try {
    const { data: result, error } = await supabase.functions.invoke('communications', {
      body: { 
...
export async function generateAISummary(communicationId: string, transcriptText: string) {
  try {
    const { data: result, error } = await supabase.functions.invoke('communications', {
      body: { 
