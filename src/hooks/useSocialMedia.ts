...
import { SocialMediaPost, CreatePostInput, SocialMediaCalendarFilters, SocialPlatform, ContentType, PostStatus } from '@/types/unified-types';
...
export const useSocialMedia = () => {
      ...
      setTimeout(() => {
        const filteredPosts = mockPosts.filter(post => {
          if (filters?.search_query && 
             !post.title.toLowerCase().includes(filters.search_query.toLowerCase()) && 
             !post.content.toLowerCase().includes(filters.search_query.toLowerCase())) {
            return false;
          }
  ...
  const refreshPosts = useCallback(async () => {
    await fetchPosts({ search_query: searchQuery, platform: selectedPlatform as SocialPlatform, status: selectedStatus as PostStatus });
