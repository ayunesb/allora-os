...
export function useCallScriptTracking() {
   ...
   */
  const trackScriptView = useCallback((scriptId: string, title: string, type: 'call' | 'message') => {
    if (!user?.id) return;
   ...
   */
  const trackScriptUse = useCallback((scriptId: string, title: string, type: 'call' | 'message', primaryBot?: any) => {
    if (!user?.id) return;
