...
export function useAdvancedLeadScoring() {
     ...
     */
    const calculateAdvancedScore = useCallback((lead) => {
        ...
        const daysSinceCreation = Math.floor((new Date().getTime() - new Date(lead.created_at).getTime()) / (1000 * 60 * 60 * 24));
        if (daysSinceCreation < 3)
            ...
            timeFactorScore = 5; // Within a month
     ...
     */
    const getLeadScoreCategory = useCallback((lead) => {
     ...
     */
    const getNextBestAction = useCallback((lead) => {
     ...
     */
    const getLeadPriority = useCallback((lead) => {
        const { totalScore } = calculateAdvancedScore(lead);
        if (totalScore >= 90)
            return 5;
        if (totalScore >= 70)
