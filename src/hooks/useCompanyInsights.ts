...
import {
  BotInsight,
  InsightType,
} from "@/components/bot-insights/BotInsightCard";
...
export function useCompanyInsights() {
  ...
  useEffect(() => {
    ...
    const generateInsights = async () => {
      try {
        ...
        const newInsights = generateCompanySpecificInsights(
          companyName,
          industry,
          companyGoals,
          riskAppetite,
        );
        const detailedInsightsMap = generateDetailedInsightData(
          newInsights,
          companyName,
          industry,
          companyGoals,
  ...
  const getDetailedInsight = (insightId: string): DetailedInsight | null => {
...
function generateCompanySpecificInsights(
  ...
  const generateId = (prefix: string) =>
    `${prefix}-${companyName.toLowerCase().replace(/\s+/g, "-")}`;
  ...
  const campaignBots = getBotsForRole(
    "marketing",
    "sales_business_development",
    "cmo",
  );
  const callScriptBots = getBotsForRole(
    "lead_qualification",
    "sales_business_development",
    "cold_calling",
  );
  ...
  return [
    // Strategy insight
    {
      ...
      title: strategyTitle,
      description: `A comprehensive growth strategy tailored for ${companyName} in the ${industry} sector, focusing on ${
      ...
      description: `Proposal for a high-ROI ${
        goals.includes("revenue")
          ...
          : goals.includes("acquisition")
            ...
            : "brand awareness"
      } campaign targeting decision-makers in the ${industry} space with personalized messaging and content strategy.`,
    ...
    // Call script insight
    {
      ...
      description: `High-conversion call script designed to communicate ${companyName}'s value proposition to ${
        goals.includes("enterprise")
          ? "enterprise-level"
          : goals.includes("startup")
            ? "startup"
            : "mid-market"
      } prospects in the ${industry} industry.`,
...
function generateDetailedInsightData(
  ...
  const detailedInsights: Record<string, DetailedInsight> = {};

  insights.forEach((insight) => {
    let keyPoints: string[] = [];
    let reasoning = "";
    let executiveSummary = "";
    let contributors: InsightContributor[] = [];
    ...
    if (insight.type === "strategy") {
      ...
      reasoning = `This strategy was developed with a primary focus on ${companyName}'s stated goal of ${
        goals.length > 0 ? goals[0] : "growth"
      } within the ${industry} sector. Given the ${riskAppetite.toLowerCase()} risk appetite, we've prioritized ${
        riskAppetite === "High"
      ...
      executiveSummary = `A ${riskAppetite.toLowerCase()}-risk strategy focused on ${
        goals.length > 0 ? goals.join(" and ") : "growth and innovation"
      } within the ${industry} space, designed to position ${companyName} for sustainable competitive advantage.`;
      ...
      reasoning = `This campaign was designed to align with ${companyName}'s ${
      ...
      } based on industry benchmarks for ${industry} companies.`;
      ...
      reasoning = `This call script was crafted using proven sales methodologies from top sales experts, with specific adaptations for the ${industry} sector. The script emphasizes ${
        goals.includes("enterprise")
...
function getBotsForRole(...roles: string[]) {
  ...
  const primaryBotName =
    executiveBots[primaryRole as keyof typeof executiveBots]?.[0] ||
    "AI Assistant";
  ...
  const collaborators = roles.slice(1).map((role) => {
    ...
    return {
      ...
      role,
      contribution:
        contributions[role as keyof typeof contributions] || "Advisory support",
    };
  });
