...
export function useAiMemory() {
  ...
  const storeInteraction = useCallback(
    async (
      botName: string,
      botRole: string,
      userMessage: string,
      ...
      try {
        ...
        const { error } = await supabase.from("bot_interactions").insert([
          {
            user_id: user.id,
            bot_name: botName,
            bot_role: botRole,
    ...
    async (
      ...
      try {
        ...
        const { data: embedding, error: embeddingError } =
          await supabase.functions.invoke("get-embedding", {
  ...
  const getBotInteractions = useCallback(
    async (botName: string, botRole?: string, limit: number = 20) => {
      if (!user?.id) return [];
  ...
  const clearBotMemory = useCallback(
