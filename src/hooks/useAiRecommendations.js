...
export function useAiRecommendations(companyDetails, analytics, profile, riskAppetite) {
    const [aiRecommendations, setAiRecommendations] = useState([]);
    // Generate AI recommendations based on company profile and analytics
    const generateAiRecommendations = () => {
        var _a;
        const industry = (profile === null || profile === void 0 ? void 0 : profile.industry) || "Technology";
        const companySize = companyDetails.companySize || "Small";
        // Get most active consultation topics if available
        const topTopics = ((_a = analytics === null || analytics === void 0 ? void 0 : analytics.consultationAnalytics) === null || _a === void 0 ? void 0 : _a.consultationsByTopic) || {};
        const topTopicNames = Object.entries(topTopics)
            .sort((a, b) => b[1] - a[1])
            .map((entry) => entry[0]);
        // Helper function to get a random executive from a specific role
        const getRandomExecutive = (role) => {
            const executives = executiveBots[role] || [];
        ...
        if (riskAppetite === "low") {
            ...
            filteredRecommendations = recommendations.filter((rec) => rec.type !== "high-risk" && rec.expectedImpact < 80);
        }
