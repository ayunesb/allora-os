...
export type ExecutiveRole = 'ceo' | 'coo' | 'cfo' | 'cio' | 'cmo' | 'chro' | 'strategy' | 'operations' | 'research' | 'sales';
...
export function useExecutiveBoard() {
  const [executives, setExecutives] = useState<ExecutiveBot[]>([]);
  const [filteredExecutives, setFilteredExecutives] = useState<ExecutiveBot[]>([]);
  ...
  useEffect(() => {
    const allBots: ExecutiveBot[] = Object.entries(executiveBots).flatMap(([role, names]) => 
      names.map((name, index) => {
        // Custom specializations based on the AI system documentation
        let specialization: string[] = [];
        let optimization = '';
        let riskAppetite: RiskLevel = 'Medium';
        
        // Assign specializations based on documented data
        switch(name) {
          case 'Elon Musk':
            specialization = ['Disruptive innovation', 'AI/space', 'Sustainability'];
            optimization = 'Pushes high-risk bold strategies';
            riskAppetite = 'High';
            break;
          case 'Jeff Bezos':
            specialization = ['Customer obsession', 'Data-driven scaling'];
            optimization = 'Focused on operational excellence + customer retention';
            riskAppetite = 'Medium';
            break;
          case 'Satya Nadella':
          ...
          case 'Gwynne Shotwell':
            ...
            break;
          case 'Warren Buffett':
            specialization = ['Investment strategy', 'Value assessment', 'Risk management'];
            optimization = 'Allocates capital efficiently based on long-term value';
            riskAppetite = 'Low';
            break;
          case 'Ruth Porat':
  ...
  const getExecutivesByRole = useCallback((role: string) => {
    return executives.filter(exec => exec.role === role);
  }, [executives]);
  
  // Get executives by risk appetite
  const getExecutivesByRisk = useCallback((risk: RiskLevel) => {
