...
export function useExecutiveBoard() {
  ...
  useEffect(() => {
    const allBots: ExecutiveBot[] = Object.entries(executiveBots).flatMap(
      ([role, names]) =>
  ...
  const getExecutivesByRole = useCallback(
    (role: string) => {
  ...
  const getExecutivesByRisk = useCallback(
    (risk: RiskLevel) => {
