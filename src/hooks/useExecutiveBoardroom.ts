...
export default function useExecutiveBoardroom() {
  ...
  useEffect(() => {
    const defaultExecutives = [
      ...
      .map((name, index) => {
        const role =
          Object.entries(executiveBots).find(([_, names]) =>
            names.includes(name),
          )?.[0] || "ceo";
  ...
  const simulateDebateInteractions = async (
    topic: string,
    previousResponses: DebateMessage[],
  ) => {
    try {
      for (let i = 0; i < 3; i++) {
        await new Promise((resolve) =>
        ...
        const previousMessage = previousResponses.find(
          (m) => m.senderId === respondent.id,
        );
        ...
        const observersIndices = Array.from(
  ...
  const generateThoughtReaction = (
    speaker: DebateParticipant,
    observer: DebateParticipant,
    topic: string,
  ) => {
    const thoughts = [
      "Interesting point.",
  ...
  const generateDebateSummary = (topic: string) => {
    const summary: DebateSummary = {
      winningStrategy:
        votes.filter((v) => v.choice === "option_a").length >
      ...
      executivePerformance: participants.reduce(
        (acc, exec) => {
          acc[exec.id] = {
  ...
  const generateMockResponse = (
    ...
    const roleResponses =
      responses[executive.role as keyof typeof responses] || responses.strategy;
    return (
      roleResponses[riskAppetite as keyof typeof roleResponses] ||
