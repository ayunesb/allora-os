...
export function useLeadSelection() {
    ...
    const toggleSelectAll = useCallback((leads) => {
    ...
    const isLeadSelected = useCallback((leadId) => {
        return selectedLeads.includes(leadId);
    }, [selectedLeads]);
    const getSelectedLeads = useCallback((allLeads) => {
    ...
    const handleSelectAll = useCallback((leads, isSelected) => {
        if (isSelected) {
            selectAllLeads(leads);
        }
        else {
    ...
    const handleBulkStatusUpdate = useCallback((status) => __awaiter(this, void 0, void 0, function* () {
