...
export const useSocialMedia = () => {
    ...
    const refreshPosts = useCallback(async () => {
        await fetchPosts({ search_query: searchQuery, platform: selectedPlatform, status: selectedStatus });
    ...
    const updatePost = useCallback(async (postId, postData) => {
        ...
        try {
            // In real app, this would be an API call
            setPosts(prev => prev.map(post => post.id === postId ? { ...post, ...postData } : post));
            setIsLoading(false);
    ...
    const approve = useCallback(async (postId) => {
        try {
    ...
    const schedule = useCallback(async (postId, scheduledDate) => {
        try {
