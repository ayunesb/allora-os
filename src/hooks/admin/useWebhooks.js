...
export function useWebhooks() {
    ...
    const handleSaveWebhooks = useCallback(async (isStripeValid, isZapierValid, isGithubValid, isSlackValid, isCustomValid) => {
        setIsSaving(true);
        try {
    ...
    const testWebhookUrl = useCallback(async (url, webhookType) => {
        if (!url) {
    ...
    const handleTestStripeWebhook = useCallback(async (isValid) => {
        if (!isValid) {
    ...
    const handleTestZapierWebhook = useCallback(async (isValid) => {
        if (!isValid) {
    ...
    const handleTestGithubWebhook = useCallback(async (isValid) => {
        if (!isValid) {
    ...
    const handleTestSlackWebhook = useCallback(async (isValid) => {
        if (!isValid) {
    ...
    const handleTestCustomWebhook = useCallback(async (isValid) => {
        if (!isValid) {
