"use strict";
var __assign =
  (this && this.__assign) ||
  function () {
    __assign =
      Object.assign ||
      function (t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
          s = arguments[i];
          for (var p in s)
            if (Object.prototype.hasOwnProperty.call(s, p)) t[p] = s[p];
        }
        return t;
      };
    return __assign.apply(this, arguments);
  };
var __awaiter =
  (this && this.__awaiter) ||
  function (thisArg, _arguments, P, generator) {
    function adopt(value) {
      return value instanceof P
        ? value
        : new P(function (resolve) {
            resolve(value);
          });
    }
    return new (P || (P = Promise))(function (resolve, reject) {
      function fulfilled(value) {
        try {
          step(generator.next(value));
        } catch (e) {
          reject(e);
        }
      }
      function rejected(value) {
        try {
          step(generator["throw"](value));
        } catch (e) {
          reject(e);
        }
      }
      function step(result) {
        result.done
          ? resolve(result.value)
          : adopt(result.value).then(fulfilled, rejected);
      }
      step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
  };
var __generator =
  (this && this.__generator) ||
  function (thisArg, body) {
    var _ = {
        label: 0,
        sent: function () {
          if (t[0] & 1) throw t[1];
          return t[1];
        },
        trys: [],
        ops: [],
      },
      f,
      y,
      t,
      g = Object.create(
        (typeof Iterator === "function" ? Iterator : Object).prototype,
      );
    return (
      (g.next = verb(0)),
      (g["throw"] = verb(1)),
      (g["return"] = verb(2)),
      typeof Symbol === "function" &&
        (g[Symbol.iterator] = function () {
          return this;
        }),
      g
    );
    function verb(n) {
      return function (v) {
        return step([n, v]);
      };
    }
    function step(op) {
      if (f) throw new TypeError("Generator is already executing.");
      while ((g && ((g = 0), op[0] && (_ = 0)), _))
        try {
          if (
            ((f = 1),
            y &&
              (t =
                op[0] & 2
                  ? y["return"]
                  : op[0]
                    ? y["throw"] || ((t = y["return"]) && t.call(y), 0)
                    : y.next) &&
              !(t = t.call(y, op[1])).done)
          )
            return t;
          if (((y = 0), t)) op = [op[0] & 2, t.value];
          switch (op[0]) {
            case 0:
            case 1:
              t = op;
              break;
            case 4:
              _.label++;
              return { value: op[1], done: false };
            case 5:
              _.label++;
              y = op[1];
              op = [0];
              continue;
            case 7:
              op = _.ops.pop();
              _.trys.pop();
              continue;
            default:
              if (
                !((t = _.trys), (t = t.length > 0 && t[t.length - 1])) &&
                (op[0] === 6 || op[0] === 2)
              ) {
                _ = 0;
                continue;
              }
              if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) {
                _.label = op[1];
                break;
              }
              if (op[0] === 6 && _.label < t[1]) {
                _.label = t[1];
                t = op;
                break;
              }
              if (t && _.label < t[2]) {
                _.label = t[2];
                _.ops.push(op);
                break;
              }
              if (t[2]) _.ops.pop();
              _.trys.pop();
              continue;
          }
          op = body.call(thisArg, _);
        } catch (e) {
          op = [6, e];
          y = 0;
        } finally {
          f = t = 0;
        }
      if (op[0] & 5) throw op[1];
      return { value: op[0] ? op[1] : void 0, done: true };
    }
  };
Object.defineProperty(exports, "__esModule", { value: true });
exports.useAiModelPreferences = useAiModelPreferences;
var react_1 = require("react");
var client_1 = require("@/integrations/supabase/client");
var AuthContext_1 = require("@/context/AuthContext");
var sonner_1 = require("sonner");
var defaultPreferences = {
  modelPreference: "gpt-4o-mini",
  enableMemory: true,
  enableLearning: true,
  enableVectorSearch: false,
  maxMemoryItems: 5,
  lastUpdated: new Date().toISOString(),
  // Add default values for new properties
  enableDebate: false,
  maxDebateParticipants: 3,
};
function useAiModelPreferences() {
  var _this = this;
  var user = (0, AuthContext_1.useAuth)().user;
  var _a = (0, react_1.useState)(defaultPreferences),
    preferences = _a[0],
    setPreferences = _a[1];
  var _b = (0, react_1.useState)(true),
    isLoading = _b[0],
    setIsLoading = _b[1];
  var _c = (0, react_1.useState)(false),
    isSaving = _c[0],
    setIsSaving = _c[1];
  // Fetch user AI model preferences
  (0, react_1.useEffect)(
    function () {
      var fetchPreferences = function () {
        return __awaiter(_this, void 0, void 0, function () {
          var _a, data, error, error_1;
          return __generator(this, function (_b) {
            switch (_b.label) {
              case 0:
                if (!(user === null || user === void 0 ? void 0 : user.id))
                  return [2 /*return*/];
                setIsLoading(true);
                _b.label = 1;
              case 1:
                _b.trys.push([1, 8, 9, 10]);
                return [
                  4 /*yield*/,
                  client_1.supabase
                    .from("user_ai_preferences")
                    .select("*")
                    .eq("user_id", user.id)
                    .single(),
                ];
              case 2:
                (_a = _b.sent()), (data = _a.data), (error = _a.error);
                if (!error) return [3 /*break*/, 6];
                if (!(error.code === "PGRST116")) return [3 /*break*/, 4];
                // Create default preferences
                return [4 /*yield*/, savePreferences(defaultPreferences)];
              case 3:
                // Create default preferences
                _b.sent();
                return [3 /*break*/, 5];
              case 4:
                throw error;
              case 5:
                return [3 /*break*/, 7];
              case 6:
                if (data) {
                  setPreferences(data.preferences);
                }
                _b.label = 7;
              case 7:
                return [3 /*break*/, 10];
              case 8:
                error_1 = _b.sent();
                console.error("Error fetching AI model preferences:", error_1);
                return [3 /*break*/, 10];
              case 9:
                setIsLoading(false);
                return [7 /*endfinally*/];
              case 10:
                return [2 /*return*/];
            }
          });
        });
      };
      fetchPreferences();
    },
    [user === null || user === void 0 ? void 0 : user.id],
  );
  // Save user AI model preferences
  var savePreferences = (0, react_1.useCallback)(
    function (newPreferences) {
      return __awaiter(_this, void 0, void 0, function () {
        var updatedPreferences, error, error_2;
        return __generator(this, function (_a) {
          switch (_a.label) {
            case 0:
              if (!(user === null || user === void 0 ? void 0 : user.id))
                return [2 /*return*/, false];
              setIsSaving(true);
              _a.label = 1;
            case 1:
              _a.trys.push([1, 3, 4, 5]);
              updatedPreferences = __assign(__assign({}, newPreferences), {
                lastUpdated: new Date().toISOString(),
              });
              return [
                4 /*yield*/,
                client_1.supabase.from("user_ai_preferences").upsert(
                  {
                    user_id: user.id,
                    preferences: updatedPreferences,
                  },
                  {
                    onConflict: "user_id",
                  },
                ),
              ];
            case 2:
              error = _a.sent().error;
              if (error) throw error;
              setPreferences(updatedPreferences);
              sonner_1.toast.success("AI preferences saved");
              return [2 /*return*/, true];
            case 3:
              error_2 = _a.sent();
              console.error("Error saving AI model preferences:", error_2);
              sonner_1.toast.error("Failed to save AI preferences");
              return [2 /*return*/, false];
            case 4:
              setIsSaving(false);
              return [7 /*endfinally*/];
            case 5:
              return [2 /*return*/];
          }
        });
      });
    },
    [user === null || user === void 0 ? void 0 : user.id],
  );
  // Update a specific preference
  var updatePreference = (0, react_1.useCallback)(
    function (key, value) {
      var _a;
      var newPreferences = __assign(
        __assign({}, preferences),
        ((_a = {}),
        (_a[key] = value),
        (_a.lastUpdated = new Date().toISOString()),
        _a),
      );
      setPreferences(newPreferences);
      return savePreferences(newPreferences);
    },
    [preferences, savePreferences],
  );
  // Update multiple preferences at once
  var updatePreferences = (0, react_1.useCallback)(
    function (updates) {
      var newPreferences = __assign(
        __assign(__assign({}, preferences), updates),
        { lastUpdated: new Date().toISOString() },
      );
      setPreferences(newPreferences);
      return savePreferences(newPreferences);
    },
    [preferences, savePreferences],
  );
  return {
    preferences: preferences,
    setPreferences: setPreferences,
    savePreferences: savePreferences,
    updatePreference: updatePreference,
    updatePreferences: updatePreferences,
    isLoading: isLoading,
    isSaving: isSaving,
  };
}
