"use strict";
var __awaiter =
  (this && this.__awaiter) ||
  function (thisArg, _arguments, P, generator) {
    function adopt(value) {
      return value instanceof P
        ? value
        : new P(function (resolve) {
            resolve(value);
          });
    }
    return new (P || (P = Promise))(function (resolve, reject) {
      function fulfilled(value) {
        try {
          step(generator.next(value));
        } catch (e) {
          reject(e);
        }
      }
      function rejected(value) {
        try {
          step(generator["throw"](value));
        } catch (e) {
          reject(e);
        }
      }
      function step(result) {
        result.done
          ? resolve(result.value)
          : adopt(result.value).then(fulfilled, rejected);
      }
      step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
  };
var __generator =
  (this && this.__generator) ||
  function (thisArg, body) {
    var _ = {
        label: 0,
        sent: function () {
          if (t[0] & 1) throw t[1];
          return t[1];
        },
        trys: [],
        ops: [],
      },
      f,
      y,
      t,
      g = Object.create(
        (typeof Iterator === "function" ? Iterator : Object).prototype,
      );
    return (
      (g.next = verb(0)),
      (g["throw"] = verb(1)),
      (g["return"] = verb(2)),
      typeof Symbol === "function" &&
        (g[Symbol.iterator] = function () {
          return this;
        }),
      g
    );
    function verb(n) {
      return function (v) {
        return step([n, v]);
      };
    }
    function step(op) {
      if (f) throw new TypeError("Generator is already executing.");
      while ((g && ((g = 0), op[0] && (_ = 0)), _))
        try {
          if (
            ((f = 1),
            y &&
              (t =
                op[0] & 2
                  ? y["return"]
                  : op[0]
                    ? y["throw"] || ((t = y["return"]) && t.call(y), 0)
                    : y.next) &&
              !(t = t.call(y, op[1])).done)
          )
            return t;
          if (((y = 0), t)) op = [op[0] & 2, t.value];
          switch (op[0]) {
            case 0:
            case 1:
              t = op;
              break;
            case 4:
              _.label++;
              return { value: op[1], done: false };
            case 5:
              _.label++;
              y = op[1];
              op = [0];
              continue;
            case 7:
              op = _.ops.pop();
              _.trys.pop();
              continue;
            default:
              if (
                !((t = _.trys), (t = t.length > 0 && t[t.length - 1])) &&
                (op[0] === 6 || op[0] === 2)
              ) {
                _ = 0;
                continue;
              }
              if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) {
                _.label = op[1];
                break;
              }
              if (op[0] === 6 && _.label < t[1]) {
                _.label = t[1];
                t = op;
                break;
              }
              if (t && _.label < t[2]) {
                _.label = t[2];
                _.ops.push(op);
                break;
              }
              if (t[2]) _.ops.pop();
              _.trys.pop();
              continue;
          }
          op = body.call(thisArg, _);
        } catch (e) {
          op = [6, e];
          y = 0;
        } finally {
          f = t = 0;
        }
      if (op[0] & 5) throw op[1];
      return { value: op[0] ? op[1] : void 0, done: true };
    }
  };
Object.defineProperty(exports, "__esModule", { value: true });
exports.generateSelfCoachingNote = generateSelfCoachingNote;
exports.saveCoachingNoteToMemory = saveCoachingNoteToMemory;
var client_1 = require("@/integrations/supabase/client");
var loggingService_1 = require("@/utils/loggingService");
var selfCoachingPromptTemplate =
  "\nYou are {executiveName}, a {role} at Allora AI.\n\nYou recently completed the following task:\nTask: {task}\n\nThe result was: {outcome}\nPerformance notes: {performanceNotes}\n\nAs part of your personal development:\n- Reflect honestly on what went well.\n- Reflect honestly on what went wrong (if anything).\n- Suggest one improvement you should make next time.\n- Write it like a personal self-coaching note.\n\nOutput in a motivating tone.\n";
/**
 * Generates a self-coaching note based on the outcome of an executive action
 */
function generateSelfCoachingNote(
  executiveName,
  role,
  task,
  outcome,
  performanceNotes,
) {
  return __awaiter(this, void 0, void 0, function () {
    var filledPrompt, _a, data, error, error_1;
    return __generator(this, function (_b) {
      switch (_b.label) {
        case 0:
          _b.trys.push([0, 2, , 3]);
          loggingService_1.logger.info(
            "Generating self-coaching note for "
              .concat(executiveName, " on task: ")
              .concat(task),
          );
          filledPrompt = selfCoachingPromptTemplate
            .replace("{executiveName}", executiveName)
            .replace("{role}", role)
            .replace("{task}", task)
            .replace("{outcome}", outcome)
            .replace("{performanceNotes}", performanceNotes);
          return [
            4 /*yield*/,
            client_1.supabase.functions.invoke("generate-text", {
              body: {
                prompt: filledPrompt,
                temperature: 0.3,
              },
            }),
          ];
        case 1:
          (_a = _b.sent()), (data = _a.data), (error = _a.error);
          if (error) {
            loggingService_1.logger.error(
              "Failed to generate self-coaching note",
              { error: error },
            );
            return [
              2 /*return*/,
              "Unable to generate coaching note due to API error: ".concat(
                error.message,
              ),
            ];
          }
          loggingService_1.logger.info(
            "Self-coaching note generated successfully",
          );
          return [2 /*return*/, data.content];
        case 2:
          error_1 = _b.sent();
          loggingService_1.logger.error("Error generating self-coaching note", {
            error: error_1,
          });
          return [
            2 /*return*/,
            "Error in self-coaching process: ".concat(
              error_1 instanceof Error ? error_1.message : "Unknown error",
            ),
          ];
        case 3:
          return [2 /*return*/];
      }
    });
  });
}
/**
 * Saves a self-coaching note to the executive memory
 */
function saveCoachingNoteToMemory(userId, executiveName, task, coachingNote) {
  return __awaiter(this, void 0, void 0, function () {
    var error, error_2;
    return __generator(this, function (_a) {
      switch (_a.label) {
        case 0:
          _a.trys.push([0, 2, , 3]);
          return [
            4 /*yield*/,
            client_1.supabase.from("executive_memory").insert([
              {
                user_id: userId,
                executive_name: executiveName,
                task: "Self-Coaching on: ".concat(task),
                decision: coachingNote,
              },
            ]),
          ];
        case 1:
          error = _a.sent().error;
          if (error) {
            loggingService_1.logger.error(
              "Failed to save coaching note to memory",
              { error: error },
            );
            return [2 /*return*/, false];
          }
          loggingService_1.logger.info(
            "Coaching note saved to memory successfully",
          );
          return [2 /*return*/, true];
        case 2:
          error_2 = _a.sent();
          loggingService_1.logger.error(
            "Error saving coaching note to memory",
            { error: error_2 },
          );
          return [2 /*return*/, false];
        case 3:
          return [2 /*return*/];
      }
    });
  });
}
