 ...
 */
export async function forecastExecutiveResources(
 ...
 */
export async function trainMultiForecastModels(
  ...
  try {
    ...
    for (const kpiType in kpiData) {
      ...
      if (data.length < 3) {
        logger.warn(
          `Not enough data points for KPI type ${kpiType}. Skipping model training.`,
        );
        continue;
      }
 ...
 */
export async function forecastMultipleFuture(
  ...
  try {
    ...
    for (const kpiType in models) {
      ...
      logger.info(
        `Forecasted future value for KPI type ${kpiType}: ${forecasts[kpiType]}`,
      );
    }
