 ...
 */
export function createAuthCompatibilityLayer(
  authContext: any,
): AuthContextProps {
  if (!authContext)
    return {
  ...
  return {
    ...
    refreshProfile:
      authContext.refreshProfile || (async () => Promise.resolve()),
    refreshSession:
      authContext.refreshSession || (async () => Promise.resolve(true)),
    signOut:
      authContext.signOut ||
      authContext.logout ||
...
export function normalizeUserObject(userObject: any): User | null {
  ...
  const userMetadata =
    userObject.user_metadata || userObject.metadata?.user || {};
  ...
  const normalizedUser: User = {
    ...
    role: userObject.role || (appMetadata?.is_admin ? "admin" : "user"),
