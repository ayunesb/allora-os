 ...
 */
export async function generateVideo(text, avatarId, voiceId, companyName, campaignId, strategyId, retryCount = 0) {
    try {
...
export async function getVideoStatus(videoId, retryCount = 0) {
    try {
        const { data, error } = await supabase.functions.invoke('heygen', {
 ...
 */
export async function pollVideoStatus(videoId, onStatusChange) {
    let attempts = 0;
