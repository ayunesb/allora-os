...
export async function createKPIMetric(metric: Omit<KPIMetric, 'id'>): Promise<KPIMetric | null> {
  try {
    const { data, error } = await supabase
      .from('kpi_metrics')
      .insert(metric)
...
export async function updateKPIMetric(id: string, updates: Partial<KPIMetric>): Promise<KPIMetric | null> {
  try {
    const { data, error } = await supabase
      .from('kpi_metrics')
      .update(updates)
