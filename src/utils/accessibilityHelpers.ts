 ...
 */
export function announceToScreenReader(message: string, assertive: boolean = false) {
 ...
 */
export function makeKeyboardAccessible(callback: () => void) {
 ...
 */
export function focusElement(elementId: string, preventScroll: boolean = false) {
  const element = document.getElementById(elementId);
  if (element) {
    // Add tabindex if the element isn't natively focusable
    const needsTabIndex = !['a', 'button', 'input', 'select', 'textarea'].includes(
