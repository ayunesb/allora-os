 ...
 */
export async function validateAndCleanProductionData() {
    const results = {
        success: true,
        validRecords: 0,
        errors: [],
 ...
 */
async function validateCompanies(results) {
    try {
 ...
 */
async function validateLeads(results) {
    try {
        // First, get total count for reporting
        const { count: totalCount, error: countError } = await supabase
            .from('leads')
 ...
 */
async function validateStrategies(results) {
    try {
        // First, get total count for reporting
        const { count: totalCount, error: countError } = await supabase
            .from('strategies')
 ...
 */
async function validateCampaigns(results) {
    try {
        // First, get total count for reporting
        const { count: totalCount, error: countError } = await supabase
            .from('campaigns')
