...
export async function fetchStrategyMilestones(strategyId: string): Promise<StrategyMilestone[]> {
  try {
...
export async function createMilestone(milestone: Omit<StrategyMilestone, 'id' | 'created_at'>): Promise<StrategyMilestone | null> {
  try {
    const { data, error } = await supabase
      .from('strategy_milestones')
      .insert([{
...
// Update a milestone
export async function updateMilestone(id: string, updates: Partial<Omit<StrategyMilestone, 'id' | 'created_at'>>): Promise<boolean> {
  try {
...
export function calculateStrategyProgress(milestones: StrategyMilestone[]): number {
