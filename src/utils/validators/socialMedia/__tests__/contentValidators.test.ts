...
describe("contentValidators", () => {
  describe("validateHashtags", () => {
    it("validates correct hashtags", () => {
      expect(
        validateHashtags(["#marketing", "#business2023", "#growth_strategy"]),
      ).toBe(true);
    });

    it("invalidates incorrect hashtags", () => {
      expect(
        validateHashtags(["marketing", "#business-2023", "##growth"]),
      ).toBe(false);
    });
  ...
  describe("validateContentLength", () => {
    it("validates content within platform limits", () => {
      ...
      expect(validateContentLength(content, "Twitter" as SocialPlatform)).toBe(
        true,
      );
    });
    ...
    it("invalidates content exceeding platform limits", () => {
      ...
      expect(
        validateContentLength(longContent, "Twitter" as SocialPlatform),
      ).toBe(false);
    });
  });
});
