 ...
 */
export const validateWebhookUrlFormat = (url, type) => {
        ...
        switch (type) {
            case 'stripe':
                return url.startsWith('https://api.stripe.com/') ||
                    ...
                    url.includes('webhook.site/'); // For testing purposes
            case 'zapier':
                return url.startsWith('https://hooks.zapier.com/') ||
                    url.includes('zapier.com/hooks/') ||
                    url.includes('webhook.site/'); // For testing
            case 'github':
 ...
 */
export const sanitizeWebhookUrl = (url, type) => {
    if (!url || !url.trim())
        return null;
    try {
