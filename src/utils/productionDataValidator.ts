...
async function validateCompanies(results: ValidationResults): Promise<void> {
  try {
    ...
    const orConditions = TEST_PATTERNS.map(
...
async function validateLeads(results: ValidationResults): Promise<void> {
  try {
    ...
    const nameConditions = TEST_PATTERNS.map(
      (pattern) => `name.ilike.${pattern}`,
    ).join(", ");
    const emailConditions = TEST_PATTERNS.map(
      (pattern) => `email.ilike.${pattern}`,
    ).join(", ");
...
async function validateStrategies(results: ValidationResults): Promise<void> {
  try {
    ...
    const titleConditions = TEST_PATTERNS.map(
      (pattern) => `title.ilike.${pattern}`,
    ).join(", ");
    const descConditions = TEST_PATTERNS.map(
      (pattern) => `description.ilike.${pattern}`,
    ).join(", ");
