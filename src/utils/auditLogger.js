...
export async function logSecurityEvent(eventTypeOrDetails, details, userId, severity = 1, metadata) {
    try {
        // Handle new format
 ...
 */
export async function logAuditEvent(eventType, details, userId, metadata) {
    try {
        // Log to console in development
        if (process.env.NODE_ENV === 'development') {
 ...
 * @returns Success status
 */
export async function logComplianceChange(userId, details, metadata) {
    try {
        // Log to console in development
        if (process.env.NODE_ENV === 'development') {
