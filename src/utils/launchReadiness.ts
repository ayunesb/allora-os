...
export async function checkLaunchReadiness(): Promise<LaunchReadinessStatus> {
  ...
  try {
    ...
    if (secretsResult.success) {
      ...
      const connectedApiCount = Object.values(status.apis).filter(
        (s) => s === "connected",
      ).length;
    ...
    const apiReadiness = Object.values(status.apis).filter(
      (s) => s === "connected",
    ).length;
    const featuresReady = Object.values(status.features).filter(
      (f) => f,
    ).length;
    const complianceReady = Object.values(status.compliance).filter(
      (c) => c === true || c === "high" || c === "medium",
    ).length;
