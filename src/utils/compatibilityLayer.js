...
export function normalizeCampaign(campaign) {
    return Object.assign(Object.assign({}, campaign), { name: campaign.title || campaign.name, ad_platform: campaign.platform, startDate: campaign.start_date });
}
export function normalizeUser(user) {
    if (!user)
        return null;
    // Ensure user has expected properties
    return Object.assign(Object.assign({}, user), { name: user.name ||
            (user.user_metadata
        ...
        company_id: user.company_id || null, company: user.company || null, industry: user.industry || null });
}
export function normalizeWebhookEvent(event) {
    return Object.assign(Object.assign({}, event), { eventType: event.eventType || event.event_type || "unknown", event_type: event.event_type || event.eventType || "unknown", webhookType: event.webhookType || event.webhook_type || "custom", webhook_type: event.webhook_type || event.webhookType || "custom" });
}
