 ...
 */
export function initStripeClient(apiKey) {
    if (!apiKey) {
        ...
        func: (input) => __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (normalized.includes("active subscriptions")) {
                    const subs = yield stripe.subscriptions.list({
                        status: "active",
                        limit: 10,
                    });
                    ...
                    const names = subs.data
                        .map((sub) => `• ${sub.customer} (${sub.items.data[0].price.nickname || "Default Plan"})`)
                        .join("\n");
                ...
                if (normalized.includes("refunds")) {
                    ...
                    const refunds = yield stripe.refunds.list({
                        limit: 20,
