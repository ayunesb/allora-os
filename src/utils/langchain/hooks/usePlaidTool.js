"use strict";
var __awaiter =
  (this && this.__awaiter) ||
  function (thisArg, _arguments, P, generator) {
    function adopt(value) {
      return value instanceof P
        ? value
        : new P(function (resolve) {
            resolve(value);
          });
    }
    return new (P || (P = Promise))(function (resolve, reject) {
      function fulfilled(value) {
        try {
          step(generator.next(value));
        } catch (e) {
          reject(e);
        }
      }
      function rejected(value) {
        try {
          step(generator["throw"](value));
        } catch (e) {
          reject(e);
        }
      }
      function step(result) {
        result.done
          ? resolve(result.value)
          : adopt(result.value).then(fulfilled, rejected);
      }
      step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
  };
var __generator =
  (this && this.__generator) ||
  function (thisArg, body) {
    var _ = {
        label: 0,
        sent: function () {
          if (t[0] & 1) throw t[1];
          return t[1];
        },
        trys: [],
        ops: [],
      },
      f,
      y,
      t,
      g = Object.create(
        (typeof Iterator === "function" ? Iterator : Object).prototype,
      );
    return (
      (g.next = verb(0)),
      (g["throw"] = verb(1)),
      (g["return"] = verb(2)),
      typeof Symbol === "function" &&
        (g[Symbol.iterator] = function () {
          return this;
        }),
      g
    );
    function verb(n) {
      return function (v) {
        return step([n, v]);
      };
    }
    function step(op) {
      if (f) throw new TypeError("Generator is already executing.");
      while ((g && ((g = 0), op[0] && (_ = 0)), _))
        try {
          if (
            ((f = 1),
            y &&
              (t =
                op[0] & 2
                  ? y["return"]
                  : op[0]
                    ? y["throw"] || ((t = y["return"]) && t.call(y), 0)
                    : y.next) &&
              !(t = t.call(y, op[1])).done)
          )
            return t;
          if (((y = 0), t)) op = [op[0] & 2, t.value];
          switch (op[0]) {
            case 0:
            case 1:
              t = op;
              break;
            case 4:
              _.label++;
              return { value: op[1], done: false };
            case 5:
              _.label++;
              y = op[1];
              op = [0];
              continue;
            case 7:
              op = _.ops.pop();
              _.trys.pop();
              continue;
            default:
              if (
                !((t = _.trys), (t = t.length > 0 && t[t.length - 1])) &&
                (op[0] === 6 || op[0] === 2)
              ) {
                _ = 0;
                continue;
              }
              if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) {
                _.label = op[1];
                break;
              }
              if (op[0] === 6 && _.label < t[1]) {
                _.label = t[1];
                t = op;
                break;
              }
              if (t && _.label < t[2]) {
                _.label = t[2];
                _.ops.push(op);
                break;
              }
              if (t[2]) _.ops.pop();
              _.trys.pop();
              continue;
          }
          op = body.call(thisArg, _);
        } catch (e) {
          op = [6, e];
          y = 0;
        } finally {
          f = t = 0;
        }
      if (op[0] & 5) throw op[1];
      return { value: op[0] ? op[1] : void 0, done: true };
    }
  };
var __spreadArray =
  (this && this.__spreadArray) ||
  function (to, from, pack) {
    if (pack || arguments.length === 2)
      for (var i = 0, l = from.length, ar; i < l; i++) {
        if (ar || !(i in from)) {
          if (!ar) ar = Array.prototype.slice.call(from, 0, i);
          ar[i] = from[i];
        }
      }
    return to.concat(ar || Array.prototype.slice.call(from));
  };
Object.defineProperty(exports, "__esModule", { value: true });
exports.usePlaidTool = usePlaidTool;
var react_1 = require("react");
var client_1 = require("@/integrations/supabase/client");
var loggingService_1 = require("@/utils/loggingService");
var sonner_1 = require("sonner");
var sanitizers_1 = require("@/utils/sanitizers");
function usePlaidTool() {
  var _this = this;
  var _a = (0, react_1.useState)(false),
    isLoading = _a[0],
    setIsLoading = _a[1];
  var _b = (0, react_1.useState)(null),
    error = _b[0],
    setError = _b[1];
  /**
   * Get account balances from Plaid
   */
  var getAccountBalances = function () {
    return __awaiter(_this, void 0, void 0, function () {
      var _a, data, error_1, err_1, message;
      return __generator(this, function (_b) {
        switch (_b.label) {
          case 0:
            setIsLoading(true);
            setError(null);
            _b.label = 1;
          case 1:
            _b.trys.push([1, 3, 4, 5]);
            loggingService_1.logger.info("Fetching Plaid account balances");
            return [
              4 /*yield*/,
              client_1.supabase.functions.invoke("plaid-tool", {
                body: { action: "get_balances" },
              }),
            ];
          case 2:
            (_a = _b.sent()), (data = _a.data), (error_1 = _a.error);
            if (error_1) {
              throw new Error(error_1.message);
            }
            return [2 /*return*/, data.result];
          case 3:
            err_1 = _b.sent();
            message =
              err_1 instanceof Error
                ? err_1.message
                : "Failed to fetch Plaid account balances";
            loggingService_1.logger.error(
              "Error fetching Plaid account balances",
              err_1,
            );
            setError(message);
            sonner_1.toast.error("Failed to fetch balances", {
              description: message,
            });
            return [2 /*return*/, null];
          case 4:
            setIsLoading(false);
            return [7 /*endfinally*/];
          case 5:
            return [2 /*return*/];
        }
      });
    });
  };
  /**
   * Get recent transactions from Plaid
   */
  var getRecentTransactions = function () {
    var args_1 = [];
    for (var _i = 0; _i < arguments.length; _i++) {
      args_1[_i] = arguments[_i];
    }
    return __awaiter(
      _this,
      __spreadArray([], args_1, true),
      void 0,
      function (days) {
        var _a, data, error_2, err_2, message;
        if (days === void 0) {
          days = 7;
        }
        return __generator(this, function (_b) {
          switch (_b.label) {
            case 0:
              setIsLoading(true);
              setError(null);
              _b.label = 1;
            case 1:
              _b.trys.push([1, 3, 4, 5]);
              loggingService_1.logger.info(
                "Fetching Plaid recent transactions",
                { days: days },
              );
              return [
                4 /*yield*/,
                client_1.supabase.functions.invoke("plaid-tool", {
                  body: {
                    action: "get_transactions",
                    days: (0, sanitizers_1.sanitizeInput)(days.toString()),
                  },
                }),
              ];
            case 2:
              (_a = _b.sent()), (data = _a.data), (error_2 = _a.error);
              if (error_2) {
                throw new Error(error_2.message);
              }
              return [2 /*return*/, data.result];
            case 3:
              err_2 = _b.sent();
              message =
                err_2 instanceof Error
                  ? err_2.message
                  : "Failed to fetch Plaid transactions";
              loggingService_1.logger.error(
                "Error fetching Plaid transactions",
                err_2,
              );
              setError(message);
              sonner_1.toast.error("Failed to fetch transactions", {
                description: message,
              });
              return [2 /*return*/, null];
            case 4:
              setIsLoading(false);
              return [7 /*endfinally*/];
            case 5:
              return [2 /*return*/];
          }
        });
      },
    );
  };
  return {
    getAccountBalances: getAccountBalances,
    getRecentTransactions: getRecentTransactions,
    isLoading: isLoading,
    error: error,
  };
}
