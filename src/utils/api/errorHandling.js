 ...
 */
export function generateUserFriendlyMessage(error) {
    switch (error.code) {
        case "NETWORK_ERROR":
...
export function handleApiError(error, options = {}) {
    const { showToast = true, customMessage, logError = true, rethrow = false, toastDuration = 5000, a11yContext, } = options;
    ...
    if (error instanceof Error) {
        ...
        if (name === "AbortError") {
            errorDetails = Object.assign(Object.assign({}, errorDetails), { code: "TIMEOUT", message: "Request timed out", a11yContext });
        }
        else if (message === "Network Error" || !navigator.onLine) {
            errorDetails = Object.assign(Object.assign({}, errorDetails), { code: "NETWORK_ERROR", message: "Network connection error", a11yContext });
        }
        else {
            errorDetails = Object.assign(Object.assign({}, errorDetails), { message, context: { stack } });
        }
    }
        ...
        if ("status" in errorObj || "statusCode" in errorObj) {
            ...
            errorDetails = Object.assign(Object.assign({}, errorDetails), { code: mapHttpStatusToErrorCode(status), message: typeof errorObj.message === "string"
                    ? errorObj.message
                    : typeof errorObj.error === "string"
                        ? errorObj.error
                        : "An error occurred", statusCode: status, context: (errorObj.details || errorObj.context), a11yContext });
        }
        else if ("message" in errorObj && typeof errorObj.message === "string") {
            errorDetails = Object.assign(Object.assign({}, errorDetails), { message: errorObj.message, context: errorObj });
        }
    }
    else if (typeof error === "string") {
        errorDetails = Object.assign(Object.assign({}, errorDetails), { message: error });
    }
    ...
    if (showToast) {
        ...
        toast.error(toastMessage, Object.assign({ id: `error-${errorDetails.code}-${Date.now()}`, duration: toastDuration }, toastContext));
