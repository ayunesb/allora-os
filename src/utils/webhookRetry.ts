...
export const executeWebhook = async (
  ...
  try {
    ...
    logger.error(
      `Webhook execution failed with error: ${error.message}`,
      errorDetails,
    );
    return {
      success: false,
    ...
    while (attempt <= maxRetries) {
      try {
        ...
        if (result.success) {
          if (attempt > 0) {
            logger.info(
              `Successfully executed ${type} webhook after ${attempt} retries`,
            );
          }
          return result;
        }
      ...
      logger.warn(
        `Retrying ${type} webhook, attempt ${attempt + 1} of ${maxRetries} after ${actualDelay}ms delay`,
        {
          webhookType: type,
          eventType,
          attempt: attempt + 1,
