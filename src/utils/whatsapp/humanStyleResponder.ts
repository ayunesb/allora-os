...
// Communication styles based on sales experts
type CommunicationStyle = 'consultative' | 'relationship' | 'outbound' | 'balanced';
...
const generateThanks = (context?: string): string => {
  const options = [
...
const generateFollowupQuestion = (inquiryType: string): string => {
  ...
  const typeQuestions = questions[inquiryType as keyof typeof questions] || questions.general;
  return typeQuestions[Math.floor(Math.random() * typeQuestions.length)];
};

// Generate meeting proposals
const generateMeetingProposal = (): string => {
  const proposals = [
...
const getStyleForExecutive = (executiveBot?: string): CommunicationStyle => {
  if (!executiveBot) return 'balanced';
  
  const executiveStyles: Record<string, CommunicationStyle> = {
...
export async function processWhatsAppMessage(
  ...
  return generateHumanResponse(incomingMessage, executiveBot, {
    ...
    inquiryType: lowerMessage.includes('price') ? 'pricing' 
               : lowerMessage.includes('help') ? 'support'
               : lowerMessage.includes('feature') ? 'product'
               : 'general'
  });
}
