 ...
 */
export function recordApiUsage(companyId_1, usageType_1) {
    return __awaiter(this, arguments, void 0, function* (companyId, usageType, amount = 1) {
        try {
            // Always increment the total API calls
            const { data: usageData, error: fetchError } = yield supabase
                .from("api_usage")
                .select("*")
            ...
            yield checkUsageLimits(companyId, newUsage);
            return true;
        }
        catch (error) {
 ...
 */
export function getApiUsage(companyId) {
...
function checkUsageLimits(companyId, currentUsage) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const tier = (companyData === null || companyData === void 0 ? void 0 : companyData.subscription_tier) || "starter";
            ...
            if (currentUsage.total_api_calls >= limits.maxApiCalls * warningThreshold) {
                approachingLimits.push(`API calls (${currentUsage.total_api_calls}/${limits.maxApiCalls})`);
            }
            // Check OpenAI tokens
            if (currentUsage.openai_tokens_used >=
                limits.maxOpenAiTokens * warningThreshold) {
                approachingLimits.push(`OpenAI tokens (${currentUsage.openai_tokens_used}/${limits.maxOpenAiTokens})`);
            }
            // Check strategy generations
            if (currentUsage.strategy_generations >=
                limits.maxStrategyGenerations * warningThreshold) {
                approachingLimits.push(`Strategy generations (${currentUsage.strategy_generations}/${limits.maxStrategyGenerations})`);
            }
            // Check campaign generations
            if (currentUsage.campaign_generations >=
                limits.maxCampaignGenerations * warningThreshold) {
                approachingLimits.push(`Campaign generations (${currentUsage.campaign_generations}/${limits.maxCampaignGenerations})`);
            }
            // Check script generations
            if (currentUsage.script_generations >=
                limits.maxScriptGenerations * warningThreshold) {
                approachingLimits.push(`Script generations (${currentUsage.script_generations}/${limits.maxScriptGenerations})`);
            }
            // Check AI debate runs
            if (currentUsage.ai_debate_runs >=
                limits.maxAiDebateRuns * warningThreshold) {
                approachingLimits.push(`AI debate runs (${currentUsage.ai_debate_runs}/${limits.maxAiDebateRuns})`);
            }
            // If any limits are approaching, notify admins and suggest upgrade
            if (approachingLimits.length > 0) {
                // Log a system alert
                reportWarning("API Usage Limit Approaching", `Company ${companyId} is approaching usage limits: ${approachingLimits.join(", ")}`, { companyId, tier, currentUsage, limits });
                // Store notification for admins
                yield supabase.from("admin_notifications").insert({
                    type: "usage_limit",
...
function suggestUpgrade(companyId, currentTier) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield supabase.from("user_notifications").insert({
            ...
            // Log the suggestion
            console.log(`Upgrade suggested for company ${companyId}: ${currentTier} -> ${nextTier}`);
        }
        catch (error) {
