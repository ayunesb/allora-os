 ...
 */
export async function recordApiUsage(companyId, usageType, amount = 1) {
    try {
 ...
 */
export async function getApiUsage(companyId) {
    try {
 ...
 */
async function checkUsageLimits(companyId, currentUsage) {
    try {
        ...
        // Check OpenAI tokens
        if (currentUsage.openai_tokens_used >= limits.maxOpenAiTokens * warningThreshold) {
            approachingLimits.push(`OpenAI tokens (${currentUsage.openai_tokens_used}/${limits.maxOpenAiTokens})`);
        }
        // Check strategy generations
        if (currentUsage.strategy_generations >= limits.maxStrategyGenerations * warningThreshold) {
        ...
        if (currentUsage.campaign_generations >= limits.maxCampaignGenerations * warningThreshold) {
        ...
        if (currentUsage.script_generations >= limits.maxScriptGenerations * warningThreshold) {
        ...
        if (currentUsage.ai_debate_runs >= limits.maxAiDebateRuns * warningThreshold) {
 ...
 */
async function suggestUpgrade(companyId, currentTier) {
    try {
        // Get next tier
