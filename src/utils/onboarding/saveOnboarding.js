 ...
 */
export async function saveOnboardingInfo(userId, companyName, industry, goals, companyDetails) {
    try {
        if (!userId) {
        ...
        if (companyId) {
            ...
            try {
                const integrationResult = await setupCompanyIntegrations(companyId, companyName, industry, userEmail);
