...
export function getExecutiveEnhancements(botName: string) {
  ...
  if (!boostInfo) {
    logger.warn(
      `No boost information found for executive: ${botName}. Using defaults.`,
    );
 ...
 */
export function determinePersonalityTraits(
  name: string,
  role: string,
): string[] {
 ...
 */
export function determineCognitiveLayers(role: string) {
  const roleLower = role.toLowerCase();
 ...
 */
export function determineMentalModels(role: string) {
  const roleLower = role.toLowerCase();
  const mentalModels = [] as { name: string; description: string }[];
 ...
 */
export function generateDailyMission(role: string, companyGoals?: string[]) {
  const roleLower = role.toLowerCase();
