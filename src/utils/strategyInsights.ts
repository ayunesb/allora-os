...
import { analyzeStrategy } from "@/utils/strategyAnalyzer"; // ✅ Adjust to the correct path
...
export const estimateTimeToResults = (
  ...
  if ("riskLevel" in strategy && !("risk_level" in strategy)) {
    ...
    // This is a Strategy
    const riskLevel =
      (strategy as Strategy).riskLevel || (strategy as Strategy).risk_level;
...
export const analyzeStrategyFactors = (
  ...
  if (!("riskLevel" in strategy) || "risk_level" in strategy) {
    ...
    const riskLevel =
      (strategy as Strategy).riskLevel ||
      (strategy as Strategy).risk_level ||
      "Medium";
...
export function getAnalyzedStrategy(strategy: PatchedStrategy) {
  ...
  return analyzeStrategy({
    ...strategy,
    riskLevel: normalizedRisk as "Low" | "Medium" | "High",
  });
}
