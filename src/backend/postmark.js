 ...
 */
export const sendEmail = async ({ to, subject, companyName, htmlBody, textBody, templateId, templateModel, leadId }) => {
    try {
 ...
 * @returns A promise with the result of the operation
 */
export const sendCampaignEmail = async ({ campaignId, subject, companyName, htmlBody, textBody, templateId, templateModel, messageType = 'all' }) => {
    try {
        // Get the current auth session
        const { data: { session } } = await supabase.auth.getSession();
        if (!session) {
