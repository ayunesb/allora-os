...
describe("useDatabaseVerification Hook", () => {
  ...
  describe("Verification Process", () => {
    beforeEach(() => {
      ...
      vi.mocked(databaseVerification.verifyDatabaseTables).mockResolvedValue(
        mockTableResults,
      );
      vi.mocked(databaseVerification.verifyRlsPolicies).mockResolvedValue(
        mockPolicyResults,
      );
      vi.mocked(databaseVerification.verifyDatabaseFunctions).mockResolvedValue(
        mockFunctionResults,
      );
    });
    ...
    it("should perform full verification and update results", async () => {
      ...
      expect(databaseVerification.verifyDatabaseTables).toHaveBeenCalledTimes(
        1,
      );
  ...
  describe("Error Handling", () => {
    it("should handle errors during verification and show error toast", async () => {
      ...
      vi.mocked(databaseVerification.verifyDatabaseTables).mockRejectedValue(
        testError,
      );
      ...
      expect(toast.error).toHaveBeenCalledWith(
        expect.stringContaining("Test verification error"),
      );
    });
  });
