...
} from "@/components/admin/database-verification";
...
export default function DatabaseVerificationPage() {
  ...
  const tableSuccessCount = verificationData.tables.filter(
    (t) => t.exists,
  ).length;
  const policySuccessCount = verificationData.policies.filter(
    (p) => p.exists,
  ).length;
  const functionSuccessCount = verificationData.functions.filter(
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        ...
        >
          <RefreshCw
            className={`mr-2 h-4 w-4 ${verificationData.isVerifying ? "animate-spin" : ""}`}
          />
