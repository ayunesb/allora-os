...
export default function PlatformStability() {
    ...
    return (<div className="animate-fadeIn space-y-4">
      ...
      <div className="space-y-4">
        {items.map((item) => (<Card key={item.id} className="border p-4">
            <div className="flex items-start gap-3">
              <div>
                <input type="checkbox" id={item.id} checked={item.checked} onChange={() => handleToggleItem(item.id)} className="h-5 w-5 mt-1 rounded border-gray-300 text-primary focus:ring-primary"/>
              </div>
              <div className="flex flex-col flex-grow">
                <label htmlFor={item.id} className={`font-medium ${item.checked ? 'line-through text-muted-foreground' : ''}`}>
                  {item.text}
                </label>
                {item.priority === 'high' && (<span className="mt-1 text-xs px-2 py-1 bg-orange-100 text-orange-800 rounded-full w-fit">
                    HIGH
                  </span>)}
                {item.priority === 'critical' && (<span className="mt-1 text-xs px-2 py-1 bg-red-100 text-red-800 rounded-full w-fit">
                    CRITICAL
                  </span>)}
              </div>
            </div>
