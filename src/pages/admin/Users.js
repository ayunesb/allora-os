...
export default function AdminUsers() {
    ...
    return (<div className="animate-fadeIn">
      <UserManagementHeader companies={companies} loadingCompanies={loadingCompanies} onUserAdded={loadUsers}/>
      ...
      <Card className={`border-white/10 shadow-md ${isMobileView ? 'p-2' : ''} bg-[#111827]`}>
        ...
        <CardContent className={isMobileView ? 'p-3' : ''}>
          {isLoading ? (<UserListSkeleton />) : (<UserTable users={users} onUpdateUser={updateUser} onDeleteUser={handleDeleteUser}/>)}
        </CardContent>
      </Card>
    </div>);
}
