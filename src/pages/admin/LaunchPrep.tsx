      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            <Button onClick={runValidation} disabled={isValidating} className="w-full sm:w-auto">
              {isValidating ? "Running Validation..." : "Run Production Readiness Check"}
            </Button>
            
            {error && (<div className="mt-4 p-4 bg-destructive/10 border border-destructive/20 rounded-md text-destructive">
            ...
            {validationResult && (<div className="mt-4 border rounded-md divide-y">
                <div className="p-4 bg-muted/30">
                  <div className="flex items-center justify-between">
                    <h3 className="font-medium">
                      System Readiness: {validationResult.ready ? "Ready" : "Not Ready"}
                    </h3>
                    {validationResult.ready ? (<span className="bg-green-100 text-green-800 px-2 py-1 rounded-full text-xs font-medium flex items-center gap-1">
                        <CheckCircle className="h-3 w-3"/> Ready
                      </span>) : (<span className="bg-amber-100 text-amber-800 px-2 py-1 rounded-full text-xs font-medium flex items-center gap-1">
                ...
                {/* Issues section */}
                {validationResult.issues.length > 0 && (<div className="p-4">
                    ...
                    <ul className="space-y-2">
                      {validationResult.issues.map((issue, index) => (<li key={index} className="bg-destructive/10 border border-destructive/20 p-3 rounded-md text-sm">
                          <p className="font-medium">{issue.message}</p>
                          {issue.details && (<div className="mt-1 text-muted-foreground">
                ...
                {validationResult.passedChecks.length > 0 && (<div className="p-4">
                    <h4 className="font-medium mb-2 text-green-600 flex items-center gap-1">
                      <CheckCircle className="h-4 w-4"/> Passed Checks
                    </h4>
                    <ul className="space-y-2">
                      {validationResult.passedChecks.map((check, index) => (<li key={index} className="bg-green-50 border border-green-200 p-3 rounded-md text-sm text-green-800">
                          {check.message}
                        </li>))}
                    </ul>
                  </div>)}
