      ...
      <div className="grid gap-6">
          ...
          <CardContent>
            {scanResults && (<div className="mb-6">
                <Alert variant={scanResults.success ? "default" : "destructive"}>
                  ...
                  <AlertDescription>
                    Scan completed at {new Date(scanResults.timestamp).toLocaleTimeString()}
                  </AlertDescription>
                </Alert>
                
                <div className="grid grid-cols-2 gap-4 mt-4">
                  <div className="flex items-center gap-2">
                    <Badge variant={scanResults.details.authentication ? "outline" : "destructive"}>
                      {scanResults.details.authentication ? "Pass" : "Fail"}
                    </Badge>
                    <span>Authentication</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge variant={scanResults.details.database ? "outline" : "destructive"}>
                      {scanResults.details.database ? "Pass" : "Fail"}
                    </Badge>
                    <span>Database</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge variant={scanResults.details.permissions ? "outline" : "destructive"}>
                      {scanResults.details.permissions ? "Pass" : "Fail"}
                    </Badge>
                    <span>Permissions</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge variant={scanResults.details.routing ? "outline" : "destructive"}>
            ...
            <Tabs defaultValue="instructions">
              ...
              <TabsContent value="instructions">
                <div className="text-sm space-y-4">
                  ...
                  <p className="font-medium">Click "Run Deep Scan" to begin the diagnostics process.</p>
                </div>
              </TabsContent>
              
              <TabsContent value="authentication">
                <div className="text-sm">
                  <h3 className="font-medium mb-2">Authentication Diagnostics</h3>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Troubleshooting for frequently encountered problems</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div>
                <h3 className="text-sm font-medium">404 Errors on Admin Pages</h3>
                <p className="text-sm text-muted-foreground mt-1">
                  This typically indicates a routing issue or missing component. Check route definitions in admin-routes.tsx
                  and verify that all required components are imported correctly.
                </p>
              </div>
              
              <div>
                <h3 className="text-sm font-medium">Database Connection Issues</h3>
                <p className="text-sm text-muted-foreground mt-1">
                  Verify your Supabase credentials and ensure your IP is whitelisted. Check console logs for specific
                  error details that might indicate permission or configuration problems.
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  If authentication is failing, try signing out and back in. Check your role in the profiles
                  table to ensure you have the correct permissions for admin functionality.
                </p>
              </div>
            </div>
          </CardContent>
