...
export default function ServiceStatusList({ services, showViewAllButton = true, onViewAllClick }) {
    ...
    return (<div className="space-y-4">
      {services.map((service) => (<div key={service.name} className="flex items-center justify-between">
      ...
      {services.length > 0 && showViewAllButton && (<Button variant="outline" size="sm" className="w-full mt-2" onClick={onViewAllClick}>
          View All Services
        </Button>)}
    </div>);
}
