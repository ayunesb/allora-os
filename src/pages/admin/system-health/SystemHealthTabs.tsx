          ...
          <CardContent>
            <div className="flex flex-col gap-4">
              ...
              <div className="w-full bg-muted rounded-full h-4">
                <div className={`h-4 rounded-full ${systemHealth.status === 'healthy'
            ? 'bg-green-500'
            : systemHealth.status === 'degraded'
                ? 'bg-amber-500'
                : 'bg-destructive'}`} style={{ width: `${systemHealth.percentage}%` }}/>
              </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {services.map((service) => (<div key={service.id} className="border rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <Server className="h-5 w-5 text-primary"/>
                      <div className="font-semibold">{service.name}</div>
                    </div>
                    <div className={`px-2 py-1 rounded-full text-xs ${service.status === 'healthy'
                ...
                : service.status === 'degraded'
                  ...
                  {service.status !== 'healthy' && (<div className="mt-3 flex gap-2 justify-end">
                      <Button variant="outline" size="sm" className="gap-1" onClick={() => handleFix(service.id)}>
