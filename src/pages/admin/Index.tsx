...
export default function AdminIndex() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const convertedLeads = leadsStatusData?.filter(lead => lead.status === 'converted' || lead.status === 'customer' || lead.status === 'closed').length || 0;
                ...
                try {
                    ...
                    if (sessionData && sessionData.length > 0) {
                        ...
                        const sessions = sessionData.map(s => s.activity_data?.duration || 0);
                        averageSessionTime = sessions.reduce((sum, duration) => sum + duration, 0) / sessions.length;
                        ...
                        const recentAvg = recentSessions.reduce((sum, duration) => sum + duration, 0) / recentSessions.length;
                        const olderAvg = olderSessions.reduce((sum, duration) => sum + duration, 0) / olderSessions.length || 1;
    ...
    if (!isProductionReady) {
        return (<div className={isMobileView ? "animate-fadeIn" : "container mx-auto animate-fadeIn"}>
        <AdminHeader />
        ...
        <Card className="mb-6 border-amber-500 bg-amber-50 dark:bg-amber-950/20">
          ...
          <CardContent>
            ...
            }} variant="outline" className="bg-amber-100 hover:bg-amber-200 text-amber-700 border-amber-300">
              Run Data Validation
            </Button>
          </CardContent>
        </Card>
    ...
    if (error) {
        ...
        <Card className="mb-6 border-red-500 bg-red-50 dark:bg-red-950/20">
          <CardHeader>
            <CardTitle className="text-red-700 dark:text-red-400">Error Loading Admin Data</CardTitle>
          ...
          <CardContent>
            ...
            <Button onClick={() => window.location.reload()} variant="outline" className="bg-red-100 hover:bg-red-200 text-red-700 border-red-300">
              Retry Loading
            </Button>
          </CardContent>
        </Card>
