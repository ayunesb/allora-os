...
export default function RunAudit() {
    ...
    return (<div className="container mx-auto py-8">
      ...
      {auditResults.length > 0 && (<div className="mt-8 space-y-4">
          <h2 className="text-2xl font-semibold">Audit Results</h2>
          {auditResults.map((result, index) => (<Card key={index} className="border">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">
                  {result.type} Check
                </CardTitle>
                {result.valid ? (<CheckCircle className="text-green-500 h-4 w-4"/>) : (<AlertTriangle className="text-red-500 h-4 w-4"/>)}
              </CardHeader>
              <CardContent>
