      ...
      <div className="max-w-md mx-auto space-y-6">
        <Card>
            ...
            {user ? (<>
                <div className="bg-muted/40 p-3 rounded-md">
                  ...
                  <div className="text-sm mt-2">
                    ...
                    {profile?.role === "admin" && (<span className="ml-2 text-green-500 text-xs">(Admin access granted)</span>)}
                  </div>
                </div>

                <div className="pt-2">
                  <Button onClick={makeUserAdmin} className="w-full" disabled={updatingRole || profile?.role === "admin"}>
          ...
          <CardFooter className="flex flex-col gap-2">
            <Button variant="outline" onClick={runDatabaseChecks} className="w-full" disabled={!user || loading || isValidating}>
              <Database className="h-4 w-4 mr-2"/>
              {loading || isValidating ? "Verifying..." : "Run Database Verification"}
            </Button>
            
            <Button variant="outline" onClick={() => navigate("/admin")} className="w-full" disabled={!user || profile?.role !== "admin"}>
