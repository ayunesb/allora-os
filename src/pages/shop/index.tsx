...
export default function ShopAssistant() {
    ...
    return (<div className="container py-8">
      ...
      <div className="flex gap-2 max-w-xl mb-8">
        <Input placeholder="I need a desk for long hours and back support..." value={query} onChange={(e) => setQuery(e.target.value)} onKeyPress={handleKeyPress} className="flex-1"/>
      ...
      {!loading && searched && results.length === 0 && (<Card>
      ...
      {!loading && results.length > 0 && (<>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {results.map(product => (<a key={product.id} href={product.link} target="_blank" rel="noopener noreferrer" className="border rounded-xl overflow-hidden hover:shadow-md bg-white/5 transition-all">
                <div className="relative">
                  <img src={product.image_url} alt={product.title} className="w-full h-48 object-cover"/>
                  {product.price && (<span className="absolute top-2 right-2 bg-primary/90 text-white px-2 py-1 rounded text-sm font-medium">
                      {product.price}
                    </span>)}
                </div>
                <div className="p-4">
