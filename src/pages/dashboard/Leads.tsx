  ...
  return (
    <div className={`animate-fadeIn space-y-6 ${isMobileView ? "px-0" : ""}`}>
      ...
      {isLoading || isAnyPending ? (
        ...
        <LeadsErrorState onRetry={() => startTransition(() => refetchLeads())} />
      ) : leads.length === 0 ? (
        <LeadsEmptyState />
      ) : (
        <>
          <LeadsContentComponent 
            ...
            setActiveFilter={(filter) => startTransition(() => setActiveFilter(filter))}
            selectedLeads={selectedLeads}
            handleLeadSelect={(leadId, isSelected) => startTransition(() => handleLeadSelect(leadId, isSelected))}
            handleSelectAll={(leads, isSelected) => startTransition(() => handleSelectAll(isSelected))}
            handleBulkStatusUpdate={(status) => startTransition(() => handleBulkStatusUpdate(status))}
            handleViewLead={(lead) => startTransition(() => handleViewLead(lead))}
            handleLeadStatusUpdate={(leadId, status) => startTransition(() => handleLeadStatusUpdate(leadId, status))}
            handleLeadDelete={(leadId) => startTransition(() => handleLeadDelete(leadId))}
          ...
          {selectedLead && (
            <LeadProfileDrawer
              open={isDrawerOpen}
              onOpenChange={(open) => startTransition(() => setIsDrawerOpen(open))}
              lead={selectedLead}
              onStatusUpdate={(leadId, status) => startTransition(() => handleLeadStatusUpdate(leadId, status))}
              onDelete={(leadId) => startTransition(() => handleLeadDelete(leadId))}
              getLeadScore={getLeadScore}
              getNextBestAction={getNextBestAction}
            />
          )}
