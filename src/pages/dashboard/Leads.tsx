...
export default function DashboardLeads() {
    ...
    return (<ErrorBoundary>
      <Suspense fallback={<LeadsLoading />}>
        <LeadsPageContent startTransition={startTransition} isPending={isPending}/>
      </Suspense>
    </ErrorBoundary>);
}
...
function LeadsPageContent({ startTransition, isPending: transitionPending }) {
    ...
    return (<div className={`animate-fadeIn space-y-6 ${isMobileView ? "px-0" : ""}`}>
      <LeadsHeader isMobileView={isMobileView}/>
      
      {isLoading || isAnyPending ? (<LeadsLoading />) : leadsError ? (<LeadsErrorState onRetry={() => startTransition(() => refetchLeads())}/>) : leads.length === 0 ? (<LeadsEmptyState />) : (<>
          <LeadsContentComponent leads={leads} isMobileView={isMobileView} filteredLeads={filteredLeads} searchQuery={searchQuery} setSearchQuery={(query) => startTransition(() => setSearchQuery(query))} activeFilter={activeFilter} setActiveFilter={(filter) => startTransition(() => setActiveFilter(filter))} selectedLeads={selectedLeads} handleLeadSelect={(leadId, isSelected) => startTransition(() => handleLeadSelect(leadId, isSelected))} handleSelectAll={(leads, isSelected) => startTransition(() => handleSelectAll(isSelected))} handleBulkStatusUpdate={(status) => startTransition(() => handleBulkStatusUpdate(status))} handleViewLead={(lead) => startTransition(() => handleViewLead(lead))} handleLeadStatusUpdate={(leadId, status) => startTransition(() => handleLeadStatusUpdate(leadId, status))} handleLeadDelete={(leadId) => startTransition(() => handleLeadDelete(leadId))} refetchLeads={() => startTransition(() => refetchLeads())} sortBy={sortBy} sortOrder={sortOrder} toggleSort={(column) => startTransition(() => toggleSort(column))} getLeadScore={getLeadScore} getNextBestAction={getNextBestAction} campaigns={formattedCampaigns.map(c => ({ id: c.value, name: c.label }))}/>
          
          {selectedLead && (<LeadProfileDrawer open={isDrawerOpen} onOpenChange={(open) => startTransition(() => setIsDrawerOpen(open))} lead={selectedLead} onStatusUpdate={(leadId, status) => startTransition(() => handleLeadStatusUpdate(leadId, status))} onDelete={(leadId) => startTransition(() => handleLeadDelete(leadId))} getLeadScore={getLeadScore} getNextBestAction={getNextBestAction}/>)}
        </>)}
    </div>);
}
