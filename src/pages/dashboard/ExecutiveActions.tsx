...
const ExecutiveActions = () => {
    ...
    return (<div className="container px-4 py-6">
      <PageTitle title="Executive Actions" description="Track and manage executive decisions and automated actions"/>
      
      <div className="mt-8 space-y-4">
        {actions.map(action => (<Card key={action.id}>
            ...
            <CardContent>
              <div className="space-y-2">
                ...
                {action.completedAt && (<div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Completed:</span>
                    <span>{new Date(action.completedAt).toLocaleString()}</span>
                  </div>)}
                
                {action.triggeredBy && (<div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Triggered by:</span>
                    <span>{action.triggeredBy}</span>
                  </div>)}
                
                {action.result && (<div className="mt-4 p-3 bg-green-50 text-green-800 rounded-md text-sm">
                    {action.result}
                  </div>)}
                
                {action.error && (<div className="mt-4 p-3 bg-red-50 text-red-800 rounded-md text-sm">
                    Error: {action.error}
                  </div>)}
              </div>
            </CardContent>
