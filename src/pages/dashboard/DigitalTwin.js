    ...
    const handleRefresh = () => {
        ...
        setTimeout(() => {
            ...
            toast.success(t.digitalTwin.refreshSuccess || "Data refreshed successfully", {
                description: t.digitalTwin.refreshDescription || "Latest KPI data has been loaded"
            });
        }, 1500);
    };
    return (<div className="space-y-6 p-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <PageTitle title={t.digitalTwin.title} description={t.digitalTwin.description}/>
      ...
      <Card className="overflow-hidden border-primary/20 shadow-md">
        <CardHeader className="pb-2 flex flex-row justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="outline" size="icon" className="h-8 w-8 text-muted-foreground" onClick={() => setShowPerformancePanel(!showPerformancePanel)}>
        ...
        {showPerformancePanel && (<div className="px-6 pt-2 pb-4 bg-muted/30 flex flex-wrap items-center justify-between gap-2 text-sm">
            <div className="flex items-center gap-6">
              <div>
                <span className="text-muted-foreground mr-2">FPS:</span>
                <span className={performanceStats.fps >= 50 ? "text-green-500" :
                (performanceStats.fps >= 30 ? "text-amber-500" : "text-red-500")}>
              ...
              <div>
                ...
                <span className={performanceStats.loadTime < 300 ? "text-green-500" :
                (performanceStats.loadTime < 1000 ? "text-amber-500" : "text-red-500")}>
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="text-muted-foreground text-xs">
                {t.digitalTwin.performanceTip || "Higher FPS means smoother visualization"}
              </span>
            </div>
            
            <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => setShowPerformancePanel(false)}>
          ...
          <div className="h-[70vh] w-full relative">
            ...
            <div className="absolute bottom-4 right-4 flex gap-2">
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button variant="secondary" size="icon" className="backdrop-blur-sm bg-background/80 h-10 w-10">
              ...
              <Button variant="secondary" size="sm" className="backdrop-blur-sm bg-background/80" onClick={handleRefresh} disabled={isRefreshing}>
                <RefreshCw className={`h-4 w-4 mr-2 ${isRefreshing ? 'animate-spin' : ''}`}/>
            ...
            <div className="absolute top-4 left-4">
              <Badge variant="outline" className={`px-2 py-1 backdrop-blur-sm ${performanceStats.fps >= 50 ? "bg-green-500/10 text-green-400 border-green-500/30" :
            performanceStats.fps >= 30 ? "bg-amber-500/10 text-amber-400 border-amber-500/30" :
