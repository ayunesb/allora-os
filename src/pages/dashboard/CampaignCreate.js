...
export default function CampaignCreate() {
    ...
    if (!metaConnected && !tiktokConnected) {
        ...
        <div className="flex flex-col items-center">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="w-full max-w-2xl">
            <ConnectPlatformsCard metaConnected={metaConnected} tiktokConnected={tiktokConnected} isLoading={false} onProceed={handleConnectAccounts}/>
          </motion.div>
        </div>
      </div>);
    }
