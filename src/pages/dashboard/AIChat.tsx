...
const AIChat = () => {
    ...
    const executives = {
        ...
        cfo: { id: '2', name: 'Morgan Patel', role: 'CFO', avatarUrl: '/assets/executives/cfo-avatar.png' },
        cmo: { id: '3', name: 'Sarah Johnson', role: 'CMO', avatarUrl: '/assets/executives/cmo-avatar.png' },
        cto: { id: '4', name: 'David Miller', role: 'CTO', avatarUrl: '/assets/executives/cto-avatar.png' }
    };
    ...
    const generateResponse = (message, executive) => {
        ...
        const roleResponses = responses[executive.id === '1' ? 'ceo' :
            executive.id === '2' ? 'cfo' :
    ...
    return (<div className="container px-4 py-6 h-full flex flex-col">
      <PageTitle title="AI Executive Chat" description="Chat with your AI executives for strategic insights and guidance"/>
          ...
          <Card className="flex-1 flex flex-col h-[calc(100vh-260px)]">
            <CardHeader className="pb-2 border-b">
              <div className="flex items-center gap-2">
                <Avatar className="h-8 w-8">
                  ...
                  <AvatarFallback>{activeExecutive.name.charAt(0)}</AvatarFallback>
                </Avatar>
                <div>
            ...
            <CardContent className="flex-1 overflow-auto py-4 px-4">
              {messages.length === 0 ? (<div className="h-full flex items-center justify-center text-center">
                  <div className="max-w-md">
                    ...
                    <h3 className="text-lg font-medium mb-2">Chat with {activeExecutive.name}</h3>
                    <p className="text-sm text-muted-foreground">
                      Start a conversation with your {activeExecutive.role} to get strategic insights and guidance tailored to your business.
                    </p>
                  </div>
                  ...
                  {messages.map((message) => (<div key={message.id} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    ...
                    : 'bg-muted'}`}>
                        {message.role === 'assistant' && (<div className="flex items-center gap-2 mb-1">
                            <Avatar className="h-6 w-6">
                              <AvatarImage src={activeExecutive.avatarUrl}/>
                              <AvatarFallback>{message.executiveName?.charAt(0)}</AvatarFallback>
                            </Avatar>
                            <span className="text-xs font-semibold">{message.executiveName}</span>
                            <span className="text-xs text-muted-foreground">• {message.executiveRole}</span>
                          </div>)}
                        
                        <div className="text-sm">
                          {message.isLoading ? (<div className="flex items-center gap-2">
                              <RefreshCw className="h-4 w-4 animate-spin"/>
                              <span>{message.content}</span>
                            </div>) : (message.content)}
                        </div>
                        
                        {message.role === 'assistant' && !message.isLoading && (<div className="flex justify-end gap-1 mt-1">
                            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => provideFeedback(message.id, true)}>
                              <ThumbsUp className="h-4 w-4"/>
                            </Button>
                            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => provideFeedback(message.id, false)}>
            ...
            <div className="p-4 border-t">
              <div className="flex items-end gap-2">
                <Textarea placeholder="Type your message..." value={inputMessage} onChange={(e) => setInputMessage(e.target.value)} onKeyDown={handleKeyDown} className="flex-1 min-h-[60px] resize-none" disabled={isLoading}/>
                <div className="flex flex-col gap-2">
                  <Button size="icon" variant="outline" type="button" disabled={isLoading} onClick={() => toast.info("Voice input feature coming soon!")}>
                    <Mic className="h-5 w-5"/>
                  </Button>
                  <Button size="icon" type="button" disabled={!inputMessage.trim() || isLoading} onClick={handleSendMessage}>
