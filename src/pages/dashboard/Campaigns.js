...
export default function Campaigns() {
    ...
    useEffect(() => {
        // Track page view
        trackAction('view_page', 'page_view', 'campaigns', 'page', { page: 'campaigns' });
    }, [trackAction]);
    const onSubmit = (data) => {
    ...
    return (<Tabs defaultValue="main">
      <TabsContent value="main">
        <div className="container mx-auto px-4 py-8">
          ...
          <CampaignAnalytics campaignName="All Campaigns Overview" isComparison={false} campaigns={campaigns} isLoading={isLoading}/>
          
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-semibold">All Campaigns</h2>
            
            {campaigns.length > 0 && (<div className="flex space-x-2">
          ...
          <Suspense fallback={<Skeleton className="h-96 w-full"/>}>
            <CampaignsList campaigns={campaigns} isLoading={isLoading} handleEditCampaign={handleEditCampaign} deleteCampaign={deleteCampaign} onCreateCampaign={handleNewCampaign} onApproveCampaign={handleApproveCampaign} onExportCampaign={handleExportCampaign}/>
          </Suspense>
          
          <Suspense fallback={null}>
            <CampaignWizard open={isDialogOpen} onOpenChange={setIsDialogOpen} onSubmit={onSubmit} defaultValues={getWizardDefaultValues()} isSubmitting={isCreating || isUpdating} isEditing={!!editingCampaignId}/>
          </Suspense>
        </div>
      </TabsContent>
    </Tabs>);
