...
export default function Approvals() {
    ...
    return (<div className="w-full max-w-full px-4 sm:px-6 md:px-8 mx-auto pb-6">
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-0 mb-4 sm:mb-6">
        <Button variant="ghost" size="sm" className="mr-0 sm:mr-4 w-fit" onClick={() => navigate(-1)}>
      ...
      {approvals.length === 0 ? (<Card>
          ...
          {approvals.map((item) => (<Card key={item.id} className="overflow-hidden">
              ...
              <CardFooter className="flex flex-col sm:flex-row justify-between gap-3 border-t p-4 sm:p-6">
                <div className="flex w-full sm:w-auto space-x-2">
                  <Button onClick={() => handleApprove(item.id)} className="bg-green-600 hover:bg-green-700 flex-1 sm:flex-none">
                    <ThumbsUp className="mr-2 h-4 w-4"/>
                    Approve
                  </Button>
                  <Button variant="outline" onClick={() => handleDecline(item.id)} className="flex-1 sm:flex-none">
