...
export default function GalaxyPage() {
    ...
    return (<div className="p-6 flex">
      <div className="flex-1">
        ...
        <GalaxyGraph onNodeClick={handleNodeClick} linkDirectionalParticles={2} linkDirectionalParticleSpeed={(d) => d.value * 0.001} linkDirectionalArrowLength={4} linkDirectionalArrowRelPos={1} nodeColor={(node) => (node.type === 'plugin' ? '#6366f1' : '#22d3ee')} nodeVal={(node) => node.xp || 1} // XP influences node size
     nodeCanvasObject={(node, ctx, globalScale) => {
        ...
        }} linkWidth={(link) => link.impact || 1} linkColor={(link) => (link.type === 'plugin-strategy' ? '#7dd3fc' : '#f472b6')} onNodeHover={handleNodeHover} // Update hovered node on hover
    />
      </div>
      <InspectorSidebar data={hoveredNodeData} // Pass hovered node data to sidebar
     onClose={() => setHoveredNodeId(null)} onNodeHover={handleNodeHover}>
        {hoveredNodeData && hoveredNodeData.type === 'strategy' && (<AgentVotePanel logId={hoveredNodeData.logId} // Assuming logId is part of node data
         ...
         renderVersionBadge={(version) => (<span className="bg-gradient-to-r from-green-400 to-blue-500 text-white text-xs font-bold px-2 py-1 rounded-full">
                {version}
              </span>)} renderXPBar={(xp) => (<div className="w-full bg-gray-200 rounded-full h-2.5">
                <div className="bg-gradient-to-r from-blue-400 to-purple-500 h-2.5 rounded-full transition-all duration-200" style={{ width: `${Math.min(xp / 100, 1) * 100}%` }}/>
              </div>)}/>)}
      </InspectorSidebar>
    </div>);
}
