...
export default function ExecutiveLeaderboard() {
    ...
    if (loading) {
        ...
        <PageTitle title="Executive Leaderboard" description="Top performing AI executives by decisions, risk, and priorities"/>
        ...
        <TabsContent value="decisions">
          <Card className="p-0 overflow-hidden">
            <Table>
              ...
              <TableBody>
                {byDecisions.map((entry, index) => (<TableRow key={entry.executiveName} className={index < 3 ? 'bg-muted/20' : ''}>
                    ...
                    <TableCell>
                      <Link to={`/dashboard/executives/${encodeURIComponent(entry.executiveName)}`} className="hover:underline hover:text-primary font-medium">
                        {entry.executiveName}
                      </Link>
        ...
        <TabsContent value="risk">
          <Card className="p-0 overflow-hidden">
            <Table>
              ...
              <TableBody>
                {byRisk.map((entry, index) => (<TableRow key={entry.executiveName} className={index < 3 ? 'bg-muted/20' : ''}>
                    <TableCell className="font-medium">
                      <div className="flex items-center">
                        {getMedal(index)}
                        <span className="ml-2">#{index + 1}</span>
        ...
        <TabsContent value="priority">
          <Card className="p-0 overflow-hidden">
            <Table>
              ...
              <TableBody>
                {byPriority.map((entry, index) => (<TableRow key={entry.executiveName} className={index < 3 ? 'bg-muted/20' : ''}>
                    <TableCell className="font-medium">
                      <div className="flex items-center">
                        {getMedal(index)}
                        <span className="ml-2">#{index + 1}</span>
