...
const SomeComponent: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
...
export default function ExecutiveLeaderboard() {
  ...
  useEffect(() => {
    async function fetchLeaderboardData() {
      try {
        ...
        const leaderboardData = Array.from(executiveMap.entries()).map(
          ([_, value]) => ({
            executiveName: value.name,
            executiveRole: value.role,
            decisionCount: value.decisions,
            averageRisk:
              value.decisions > 0 ? value.totalRisk / value.decisions : 0,
            priorityScore:
              value.decisions > 0 ? value.priorityTotal / value.decisions : 0,
          }),
        );
  ...
  const byDecisions = [...leaderboard].sort(
    (a, b) => b.decisionCount - a.decisionCount,
  );
  const byRisk = [...leaderboard].sort((a, b) => b.averageRisk - a.averageRisk);
  const byPriority = [...leaderboard].sort(
    ...
    <div className="container mx-auto p-4">
              ...
              <TableBody>
                {byDecisions.map((entry, index) => (
                  ...
                  >
                    ...
                    <TableCell
                      className={`text-right ${getRiskColor(entry.averageRisk)}`}
                    >
                      {entry.averageRisk.toFixed(1)}
                    </TableCell>
                    <TableCell
                      className={`text-right ${getPriorityColor(entry.priorityScore)}`}
                    >
              ...
              <TableBody>
                {byRisk.map((entry, index) => (
                  ...
                  >
                    ...
                    <TableCell
                      className={`text-right font-bold ${getRiskColor(entry.averageRisk)}`}
                    >
                      {entry.averageRisk.toFixed(1)}
                    </TableCell>
              ...
              <TableBody>
                {byPriority.map((entry, index) => (
                  ...
                  >
                    ...
                    <TableCell
                      className={`text-right font-bold ${getPriorityColor(entry.priorityScore)}`}
                    >
                      {entry.priorityScore.toFixed(1)}
                    </TableCell>
                    <TableCell className="text-right">
      ...
      <Card className="p-6">
        ...
        <p className="text-muted-foreground mb-4">
          View performance metrics for your AI executive team. The leadership
          board ranks executives based on decision volume, risk tolerance, and
          priority levels.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
          ...
          <div className="bg-muted/30 p-4 rounded-lg">
            <h3 className="font-medium flex items-center">
              <ShieldAlert className="h-4 w-4 mr-2 text-orange-500" /> Risk
              Score
            </h3>
            <p className="text-sm text-muted-foreground mt-1">
          ...
          <div className="bg-muted/30 p-4 rounded-lg">
            <h3 className="font-medium flex items-center">
              <TrendingUp className="h-4 w-4 mr-2 text-purple-500" /> Priority
              Score
            </h3>
            <p className="text-sm text-muted-foreground mt-1">
