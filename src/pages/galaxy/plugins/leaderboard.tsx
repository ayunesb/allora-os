...
const MOCK_PLUGINS: PluginEntry[] = [
  {
    name: "StripeSync",
    installs: 91,
    roi: 112.3,
...
export default function PluginLeaderboard() {
  const [plugins, setPlugins] = useState<PluginEntry[]>(MOCK_PLUGINS);
  const [isLoading, setIsLoading] = useState(false);
  const [activeTab, setActiveTab] = useState("all");
  const [sortBy, setSortBy] = useState<"installs" | "roi">("installs");
  ...
  useEffect(() => {
    const loadPlugins = async () => {
      setIsLoading(true);
      try {
        const result = await optimizedApiClient.fetch<PluginEntry[]>('/api/plugin-usage', {
  ...
  const filteredPlugins = plugins.filter(plugin => 
    activeTab === "all" || plugin.category === activeTab
  ).sort((a, b) => {
    if (sortBy === "installs") {
  ...
  const columns = [
    ...
    { 
      ...
      title: 'Installs',
      render: (plugin: PluginEntry) => (
        <div className="font-medium">{plugin.installs.toLocaleString()}</div>
      )
    },
    { 
      key: 'roi', 
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button 
            variant="outline" 
            size="sm"
            onClick={() => setSortBy(sortBy === "installs" ? "roi" : "installs")}
            className="flex items-center gap-1"
          >
        ...
        <div className="flex items-center justify-between">
          <TabsList>
            ...
            {categories.map(category => (
              <TabsTrigger key={category} value={category as string} className="hidden md:inline-flex">
                {category}
              </TabsTrigger>
            ))}
          </TabsList>
        ...
        <TabsContent value={activeTab} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredPlugins.slice(0, 3).map((plugin) => (
              <Card key={plugin.name} className="overflow-hidden border-white/10 bg-white/5 hover:shadow-md transition">
                <CardContent className="p-5">
                  <div className="flex justify-between items-start">
                    ...
                    <Badge 
                      className={`${
                        plugin.roi > 100 ? 'bg-green-500/10 text-green-500' : 
