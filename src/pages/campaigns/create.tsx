...
export default function CampaignBuilder() {
  ...
  const updateBlockDelay = (id, delay) => {
    setBlocks(
      blocks.map((block) => (block.id === id ? { ...block, delay } : block)),
    );
  };
  const getBlockIcon = (type) => {
    switch (type) {
      ...
      case "meta":
        return (
          <div className="h-5 w-5 rounded-full bg-blue-500 flex items-center justify-center text-xs text-white font-bold">
            f
          </div>
        );
      case "tiktok":
        return (
          <div className="h-5 w-5 rounded-full bg-black flex items-center justify-center text-xs text-white font-bold">
            T
          </div>
        );
      default:
      ...
      <div className="grid md:grid-cols-[1fr_3fr] gap-6">
        <div>
          ...
          <div className="space-y-2">
            {["email", "sms", "meta", "tiktok"].map((type) => (
              <Button
                key={type}
                variant="outline"
