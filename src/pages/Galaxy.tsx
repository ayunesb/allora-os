...
export default function GalaxyPage() {
  ...
  useEffect(() => {
    const fetchGraphData = async () => {
      ...
      const pluginNodes =
        plugins?.map((p) => ({ id: p.name, group: "plugin" })) || [];
      const strategyNodes =
        strategies?.map((s) => ({ id: s.title, group: "strategy" })) || [];
      const pluginToStrategyLinks =
      ...
      const agentToPluginLinks =
        agentLinks
          ...
          .filter((l) => l.target) || [];
      const agentNodes = [
        ...new Set(agentLinks?.map((l) => `agent:${l.agent_id}`)),
      ].map((id) => ({
        id,
        group: "agent",
      }));
