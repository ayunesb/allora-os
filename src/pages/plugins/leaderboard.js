    ...
    return (<div className="container mx-auto py-8 px-4 sm:px-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <Card>
          ...
          <CardContent>
            {loading ? (<div className="h-6 bg-muted animate-pulse rounded"/>) : impactData.length > 0 ? (<div>
              ...
              </div>) : (<div className="text-sm text-muted-foreground">No data available</div>)}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            {loading ? (<div className="h-6 bg-muted animate-pulse rounded"/>) : (<div className="text-2xl font-bold">
                {formatCurrency(impactData.reduce((sum, plugin) => sum + (plugin.total_value || 0), 0))}
              </div>)}
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent>
          {loading ? (<div className="flex justify-center items-center py-12">
            ...
            </div>) : impactData.length === 0 ? (<div className="text-center py-12">
              ...
              <Table>
                ...
                <TableBody>
                  {impactData.map((item, idx) => (<TableRow key={`${item.plugin_name}-${idx}`} className={idx === 0 ? "bg-primary/5" : undefined}>
                      <TableCell>
                      ...
                      <TableCell>
                        <Badge variant={item.average_value > 10 ? "success" :
