    ...
    async function fetchData() {
      try {
        ...
        const sortedData = [...data].sort(
          (a, b) => (b.average_value || 0) - (a.average_value || 0),
        );
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Compare plugin performance across your organization
          </p>
        </div>
      </div>
          ...
          <CardContent>
            {loading ? (
              ...
              <div className="text-2xl font-bold">
                {formatCurrency(
                  impactData.reduce(
                    (sum, plugin) => sum + (plugin.total_value || 0),
                    0,
                  ),
                )}
              </div>
              ...
              <Table>
                ...
                <TableBody>
                  {impactData.map((item, idx) => (
                    <TableRow
                      key={`${item.plugin_name}-${idx}`}
                      className={idx === 0 ? "bg-primary/5" : undefined}
                    >
                      <TableCell>
                        <Badge
                          variant={idx < 3 ? "success" : "outline"}
