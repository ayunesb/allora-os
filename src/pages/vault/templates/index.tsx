...
export default function VaultTemplatesPage() {
    ...
    return (<div className="container py-8">
      ...
      {/* Industry filters */}
      {industries.length > 0 && (<div className="mb-6">
          <div className="flex flex-wrap gap-2">
            <Button variant={selectedIndustry === null ? "default" : "outline"} size="sm" onClick={() => setSelectedIndustry(null)}>
              All Industries
            </Button>
            {industries.map(industry => (<Button key={industry} variant={selectedIndustry === industry ? "default" : "outline"} size="sm" onClick={() => setSelectedIndustry(industry)}>
                {industry}
              </Button>))}
          </div>
        </div>)}
      ...
      {isLoading ? (<div className="space-y-4">
        ...
        </div>) : filteredTemplates.length > 0 ? (<ul className="space-y-4">
          {filteredTemplates.map((tpl) => (<Card key={tpl.id} className="border border-border hover:border-primary/50 transition-colors relative">
              {/* Featured badge */}
              {tpl.is_public && (<div className="absolute top-2 right-2 bg-green-600 text-white px-2 py-1 text-xs rounded-md flex items-center gap-1">
              ...
              <CardContent className="p-4">
                <div className="flex justify-between items-start">
                  <div>
                    {tpl.is_public && (<span className="text-xs text-green-500 font-semibold inline-block mb-1">
                        🌟 Featured
                      </span>)}
                  ...
                  <div className="ml-4">
                    ...
                    <Button className="mt-2 text-xs" size="sm" onClick={() => handleRemix(tpl.id, tpl.title)}>
                      Remix →
                    </Button>
                  </div>
                </div>
