...
export default function PublishStrategyTemplates() {
    ...
    const handlePublish = async (id) => {
        setPublishingId(id);
        try {
            await fetchApi(`/api/vault/templates/publish?id=${id}`, { method: 'POST' });
    ...
    return (<AdminOnly>
      <div className="container py-8">
        ...
        <div className="flex items-center justify-between gap-2 mb-6">
          <div>
            <h1 className="text-3xl font-bold">🚀 Publish Strategy Templates</h1>
            <p className="text-muted-foreground">Review and publish tenant-created strategies to the public Vault.</p>
          </div>
        ...
        {loading ? (<div className="space-y-4">
          ...
          </div>) : templates.length > 0 ? (<ul className="space-y-4">
            {templates.map((tpl) => (<Card key={tpl.id} className={`border ${tpl.is_public ? 'border-green-500/30' : 'border-border'}`}>
                <CardContent className="p-4">
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <h2 className="font-semibold text-lg">{tpl.title}</h2>
                        {tpl.is_public && (<Badge variant="success" className="text-xs">Published</Badge>)}
                      </div>
                    ...
                    <div className="ml-4">
                      ...
                      {!tpl.is_public && (<Button className="mt-2 w-full" size="sm" onClick={() => handlePublish(tpl.id)} disabled={publishingId === tpl.id}>
                          {publishingId === tpl.id ? 'Publishing...' : '📢 Publish'}
                        </Button>)}
                    </div>
                  </div>
                </CardContent>
